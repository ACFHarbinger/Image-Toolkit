...
"agent bus." Current roster: Codex, Gemini/Antigravity, Grok, Cursor,
Meta's Muse, Claude — and the user is still evaluating more (a persistent
Perplexity agent in a TUI, among others) to see which stick as a
permanent part of the R&D workflow. If you're Cursor or Muse: you've
replaced the two opencode-based agents (DeepSeek and Opencode/mimo) who
did most of Phase 0's work (see D19 in the report) — their attribution on
already-shipped work stands as historical record, you're picking up
where they left off on what's still open, not redoing anything. If you're
some other newer arrival: welcome, and don't assume you're the only "new"
one — check the bus for how recently others joined too.
...
**Where it stands (decisions D1-D19 locked; check the report/bus for
anything more recent than this snapshot):**

- **Phase 0 — invariant lock: ✅ complete, merged to `main`.** All four
  sub-issues (#521-#524) shipped and closed, verified via a live-desktop
  pass against real large/adversarial data, not just unit-test-green.
  Two more bugs (GIF thumbnail disk-caching, a session-recovery freeze)
  were found and fixed along the way — see report §5.18E/F in
  `docs/moon/roadmaps/architecture.md`.
- **Phase 1 — six parallel contract epics, now unblocked (D2):**
  `PreferenceStore` (#525, Gemini/Antigravity), `ThumbnailScheduler`
  interface (#526, Grok), `ModuleDescriptor`+`ModuleHost` pilot (#527,
  Gemini/Antigravity, Log Panel), `WindowManager` (#528, **Cursor**),
  backend Qt-decoupling `Observable` (#529, **Meta's Muse**), CI
  import-boundary guardrails (#530, **Meta's Muse**). Per D6, these run
  concurrently across owners, not serialized — accepted bus-coordination
  overhead.
...
issues **#520-#532**. Full owner table is report §6. If you're Cursor or
Muse, your assigned Phase 1 epic is the place to start — read its issue
body and report §3 (the proposals) for the contract shape before writing
code. If you're picking up other work, the honest options are: wait for
an owner to finish and hand off, help review someone else's in-progress
branch, or propose new scope the team hasn't covered — post it to the bus
and the report's Q&A section (§4) rather than just starting.
...
- Check the bus for the latest status on #525-#532 before assuming a
  piece is idle or done — ownership and status change fast, faster than
  any snapshot in this file.
- Reviewing someone else's in-progress Phase 1 branch is genuinely useful
  work if you don't have (or don't want) a piece of your own yet.
- If you find something the analysis passes missed, or want to challenge
  a locked decision (D1-D19 in report §5) with new evidence, post it —
