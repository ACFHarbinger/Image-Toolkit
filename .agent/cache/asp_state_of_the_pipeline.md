...
**S81 (2026-06-13):** Multi-modal HITL (Issue 10). `backend/src/animation/data_serialization.py` (new) — `COCOAnnotationBuilder` (fg segmentation, seam-exclusion, frame-selection annotations; atomic JSON write) + `LabelStudioExporter` (model predictions + human annotations arrays for RLHF delta) + `create_session_serializers()` factory. `backend/src/animation/grounding.py` (new) — lazy GroundingDINO wrapper (`_detect_objects`, `_detect_best_box`, `_detect_exclusion_mask`); graceful ImportError fallback; `GROUNDING_DINO_CKPT`/`CFG` env vars. `masking.py` extended: `_compute_fg_masks_grounded_sam2()` (DINO bbox + SAM-2 propagation, BiRefNet per-frame fill, exception fallback) + `_refine_masks_with_clicks()` (pos/neg click → SAM-2 re-propagate). `compositing.py` extended: `_build_seam_cost_map()` + `_composite_foreground()` gain `exclusion_masks: Optional[List[np.ndarray]]` param — masks auto-resized, cost=1e6 hard barrier for NL seam routing. `gui/src/dialogs/mask_review_dialog.py` (new) — `MaskReviewDialog` with `_ClickOverlay` (left=pos, right=neg SAM-2 prompts) + `_RefinementWorker(QThread)`. `stitch_worker.py` — `sig_review_masks` signal + `set_mask_override()` + HITL checkpoint 1.5. 44 new tests → **542 tests passing**.

**S80 (2026-06-13):** §1A per-pair Otsu bg-only phase correlation (`_otsu_bg_mask_pair()` in `frame_selection.py`, `ASP_OTSU_BG_CORR=1`). §5A/C background zero-coverage fill (`complete_background()` in new `bg_complete.py`; NN fill default, ProPainter hook when installed; Stage 10.2 in `pipeline.py`, `ASP_BG_COMPLETE=1/2`). §8 recommended defaults shipped in `asp_config.toml` (8 flags enabled: ADAPTIVE_SP_SOFT/THRESH, SEAM_SMOOTH_WINDOW, SEAM_MARGIN, SEAM_FG_PENETRATION_MAX, ZONE_MIN_HEIGHT, SEAM_INSTABILITY_THRESH, STATIC_INPUT_MAX_MAD). `animation/__init__.py` auto-loads TOML before module flags are read. SAM-2 wiring completed (`ASP_USE_SAM2=1` in `pipeline.py`). Issue report marked with implementation status. 498 tests passing.
...
**Files:** `backend/benchmark/bench_anime_stitch.py` → `_smart_select_frames()`;  `backend/src/animation/frame_selection.py` → `smart_select_frames()` (pipeline-usable backend module added in session 3)
...
**Backend module (`frame_selection.py`):** Extracted to `backend/src/animation/frame_selection.py` as a clean, pipeline-usable API so the GUI and pipeline can call `smart_select_frames()` without re-implementing the logic.
...
The 13-stage pipeline in `backend/src/animation/pipeline.py`:
...
**File:** `backend/src/animation/fg_register.py`
...
**File:** `backend/src/animation/rendering.py`
...
Verified by 3 unit tests: `backend/test/animation/test_rendering.py::TestForegroundExcludedMedian`.
...
**File:** `backend/src/animation/validation.py`
...
- **ToonCrafter** (`animation/anim_fill.py`): anime-style generative inbetweening — wired to worst seam in `compositing.py` via `ASP_TOONCRAFTER_SEAM=1` (session 9); see §2.15
- **SRStitcher** (`animation/sr_stitcher.py`): diffusion-based seam/border inpainting (`sr_mode=True`)
- **Real-ESRGAN anime_6B** (`animation/super_res.py`): post-process 2–4× upscaling
- **Unit tests**: **292 passing** in `backend/test/animation/` (S37: +5 `TestFilterHighConfEdges`; S36: +5 `TestAdaptiveMinGap`; S35: +5 `TestGhostingScoreV2`; S34: +5 `TestComputeAdaptiveMinDisp`; S33: +10 `TestSeamCostColumnFilter`/`TestDetectScrollAxisModule`; S32: +5 `TestRejectStaticEdges`; earlier sessions: fg_register, rendering, bundle_adjust, filter_edges, affine_validation, frame_selection, bench_metrics, config, pipeline, canvas, compositing)
...
**Files:** `backend/benchmark/bench_anime_stitch.py`, `backend/src/animation/pipeline.py`
...
**Files:** `backend/src/animation/frame_selection.py`, `backend/benchmark/bench_anime_stitch.py`
...
**File:** `backend/src/animation/frame_selection.py` → `_detect_hold_blocks()`
...
**File:** `backend/src/animation/bundle_adjust.py`
...
**File:** `backend/src/animation/fg_register.py` → `_slic_sgm_proxy()`
...
**File:** `backend/src/animation/pipeline.py` (between Stage 12 and Stage 13)
...
**File:** `backend/src/animation/compositing.py`

Wires `_generate_canonical_cel()` from `animation/anim_fill.py` to the **single worst seam** (max `post_warp_diff` among single-pose-escalated seams). Instead of the hard dominant-frame partition, a synthesised intermediate pose is used for the fg pixels at that seam, structurally eliminating the most severe ghost.
...
**File:** `backend/src/animation/compositing.py` — `_seam_cut()`
...
**Files:** `backend/benchmark/bench_anime_stitch.py`, `backend/src/animation/compositing.py`, `backend/src/animation/pipeline.py`
...
**Files:** `backend/src/animation/compositing.py`, `backend/test/animation/test_compositing.py`
...
**Files:** `backend/src/animation/pipeline.py`, `backend/test/animation/test_canvas.py`
...
| Metric | S12 | S13 |
|--------|-----|-----|
| Tests passing (animation suite) | 149 | 155 |
...
| Metric | S14 | S15 |
|--------|-----|-----|
| Tests passing (animation suite) | 163 | 170 |
...
| Metric | S15 | S16 |
|--------|-----|-----|
| Tests passing (animation suite) | 170 | 177 |
...
| Metric | S16 | S17 |
|--------|-----|-----|
| Tests passing (animation suite) | 177 | 183 |
...
| Metric | S17 | S18 |
|--------|-----|-----|
| Tests passing (animation suite) | 183 | 194 |
...
| Metric | S18 | S19 |
|--------|-----|-----|
| Tests passing (animation suite) | 194 | 201 |
...
| Metric | S19 | S20 |
|--------|-----|-----|
| Tests passing (animation suite) | 201 | 203 |
...
**File:** `backend/src/animation/compositing.py` — `_poisson_seam_blend()`
...
| Metric | S20 | S21 |
|--------|-----|-----|
| Tests passing (animation suite) | 203 | 208 |
...
**File:** `backend/src/animation/compositing.py` — `_gain_to_min_feather()`
...
| Metric | S21 | S22 |
|--------|-----|-----|
| Tests passing (animation suite) | 208 | 214 |
...
**File:** `backend/src/animation/canvas.py` — `_telea_fill_gaps()`; `backend/src/animation/pipeline.py` — P1.8 block
...
| Metric | S22 | S23 |
|--------|-----|-----|
| Tests passing (animation suite) | 214 | 219 |
...
**File:** `backend/src/animation/compositing.py` — `_adaptive_gain_clamp()`
...
| Metric | S23 | S24 |
|--------|-----|-----|
| Tests passing (animation suite) | 219 | 222 |
...
| Metric | S24 | S25 |
|--------|-----|-----|
| Tests passing (animation suite) | 222 | 227 |
...
**File:** `backend/src/animation/pipeline.py` — `_reject_static_edges()` + wired into `_filter_edges()`
**Constant:** `backend/src/constants/animation.py` — `STATIC_EDGE_MIN_DISP_PX = 50`
...
| Metric | S31 | S32 |
|--------|-----|-----|
| Tests passing (animation suite) | 257 | 262 |
...
| S34 | §1.2C adaptive min-step threshold | `pipeline.py` | `_compute_adaptive_min_disp(edges)` = `max(50, 0.10 × median_adjacent_step)`; wired into `_filter_edges` before `_reject_static_edges`. `ADAPTIVE_MIN_DISP_FRAC=0.10` in `constants/animation.py`. +5 tests, 277 total. |
...
| S50 | §1.4F per-frame exposure outlier rejection | `compositing.py` / `constants/animation.py` / `config.py` | `_reject_exposure_outliers(frame_lums, max_deviation_lum) → List[bool]` — median bg-lum computed from all valid frame lums; returns True for frames with `|lum − median| > threshold`. Fallback all-False when < 3 valid frames. `_EXPOSURE_OUTLIER_THRESH` flag (default 0.0=off, `ASP_EXPOSURE_OUTLIER_THRESH=60.0`); OR'd into `_skip_norm` after `_coherence_skip_mask`. `EXPOSURE_OUTLIER_THRESH=60.0` constant; `ASP_EXPOSURE_OUTLIER_THRESH` in `_CONFIG_SCHEMA`. Exported in `__all__`. +5 tests, 357 total. |
| S51 | §1.13 scene-change edge pre-filter | `pipeline.py` / `constants/animation.py` / `config.py` | `_reject_scene_change_edges(edges, frames, max_luma_diff)` — computes 64×64 thumbnail mean grayscale luma for each pair of frames (i, j); rejects edge when `|lum(i)−lum(j)| > max_luma_diff`. Safe fallback for out-of-bounds frame indices (kept). Gate disabled by default (`_SCENE_CHANGE_LUMA_THRESH=0.0`; set `ASP_SCENE_CHANGE_LUMA_THRESH=60.0` to enable). Wired as first check in `_filter_edges` before §1.2A+C static-edge rejection. `SCENE_CHANGE_LUMA_THRESH=60.0` constant; `ASP_SCENE_CHANGE_LUMA_THRESH` in `_CONFIG_SCHEMA`. Exported in `__all__`. +5 tests, 362 total. |
...
| S54 | §1.3C scale normalisation before BA | `pipeline.py` / `constants/animation.py` | `_normalize_frame_scales(frames, edges, scale_thresh=0.05) → (List[np.ndarray], List[Dict])` — extracts per-edge scale `s_ij=sqrt(a²+b²)` from matched affines; BFS spanning tree from frame 0 propagates absolute per-frame scale; resizes frames by `1/scale[i]` (Lanczos-4); resets edge M 2×2 block to identity and divides tx/ty by `scale[i]`; no-op when dev<thresh or graph disconnected. `SCALE_NORM_THRESH=0.05` constant; `_SCALE_NORM_THRESH` flag (default 0.0=off, `ASP_SCALE_NORM_THRESH=0.05`). Exported in `__all__`. +5 tests, 377 total. |
...
| Metric | S30 | S31 |
|--------|-----|-----|
| Tests passing (animation suite) | 252 | 257 |
...
**File:** `backend/src/animation/bundle_adjust.py` — `_compute_adaptive_f_scale()` + adaptive re-solve in `_bundle_adjust_affine()`
...
| Metric | S29 | S30 |
|--------|-----|-----|
| Tests passing (animation suite) | 247 | 252 |
...
| Metric | S28 | S29 |
|--------|-----|-----|
| Tests passing (animation suite) | 242 | 247 |
...
**File:** `backend/src/animation/pipeline.py` — `_spatial_dedup_frames()` + `run()` refactor
...
| Metric | S27 | S28 |
|--------|-----|-----|
| Tests passing (animation suite) | 237 | 242 |
...
**File:** `backend/src/animation/config.py` — `load_asp_config()`
...
| Metric | S26 | S27 |
|--------|-----|-----|
| Tests passing (animation suite) | 232 | 237 |
...
**File:** `backend/src/animation/frame_selection.py` — `_near_dup_luma_filter()`; `backend/src/constants/animation.py` — `NEAR_DUP_LUMA_THRESH`
...
**Constants change:** `NEAR_DUP_LUMA_THRESH = 3.0` added to `constants/animation.py` and imported in `pipeline.py`, replacing the hardcoded magic number `3.0` in the pre-stage-5 dedup block.
...
| Metric | S25 | S26 |
|--------|-----|-----|
| Tests passing (animation suite) | 227 | 232 |
...
**Infrastructure in place:** `backend/src/animation/frame_selection.py` has the two-pass architecture ready to accept any pose similarity metric via `_fg_center_diff()`. Replacing the gradient computation with foreground-only flow or pose embedding is the only change needed.
...
| **Tests** | 322 passing in animation suite (S43: §3.4A dHash hold detection; S22–S43 cumulatively +108 tests) | — | §1.10B Bayesian parameter search (needs calibrated reward model) |
...
- **`VideoIngestionStream`** (`backend/src/animation/video_ingestion.py`) — wraps `av.open()`. Exposes proxy stream (I-frame-only at ¼-res for fast selection pass) and full-res frame decode via `get_frame(idx)`. `decimate_duplicates(mad_threshold)` drops telecine pull-down duplicates on the proxy tensor stream before `smart_select_frames()`.
...
1. **COCO JSON** (`COCOAnnotationBuilder` in `backend/src/animation/data_serialization.py`) — `images`/`annotations` arrays with RLE masks or polygon contours (pycocotools). Stored at `~/.image-toolkit/hitl_annotations/session_{timestamp}.json`.
...
## OpenCV Stitcher Gap Analysis (2026-06-22)

*Analysis based on cv2 4.13.0 (`cv2.detail` module), the full 97-test benchmark (`anime_stitch_20260621_193956.json`), and source inspection of `backend/src/animation/`.*

### Benchmark: Where ASP Loses to Simple Stitch

Full-corpus result over 97 tests (S142 baseline, all flags default OFF):

| Metric | ASP mean | Simple mean | Δ | % tests ASP worse |
|--------|----------|-------------|---|-------------------|
| **strip_banding_score** | high | 0.0 | +25.4 | **97.9%** |
| **ghosting_score** | 38.66 | 27.19 | +11.7 | **93.8%** |
| **seam_visibility** | high | 4.0 | +19.4 | **88.5%** |
| seam_coherence | high | lower | +0.7 | 57.3% |
| sharpness | 108.9 | 63.8 | +45.1 | 6.2% (ASP better) |
| GT-SSIM | 0.659 | 0.699 | −0.040 | — |
| ghosting_siqe | lower | higher | −36.6 | 9.4% (ASP better FFT) |

**Verdict counts** (97 tests): `simple_better=46, comparable=41, asp_better=9`.  
**GT-verdict** (55 tests): `simple_better=26, comparable=22, asp_better=6`.

The `strip_banding_score` (97.9% of tests, additive +25.4 lum units) is the dominant failure mode and is near-universal. It measures the maximum luminance jump between frame-strip transition zones — the simple stitch (uniform `np.vstack`) scores 0.0 by construction (no blending). ASP's compositing occasionally amplifies rather than suppresses inter-strip luminance differences.

### OpenCV Stitcher Pipeline (cv2 4.13.0)

The OpenCV `cv2.Stitcher` exposes the full pipeline through the `cv2.detail` module. Introspection with `cv2.Stitcher_create(mode)`:

**PANORAMA mode** (`Stitcher_PANORAMA`):
- Feature detection: `cv2.SIFT_create()` (internally; exposed via `cv2.detail.computeImageFeatures(featuresFinder, images)`)
- Feature matching: `detail_BestOf2NearestMatcher(match_conf=0.3)` — Lowe ratio test + cross-check, all-pairs graph
- Estimation: `detail_HomographyBasedEstimator(is_focals_estimated=False)` — full 8-DOF homography per pair → focal lengths from DLT
- Bundle adjustment: `detail_BundleAdjusterRay()` — projects feature points onto unit sphere, minimises angular reprojection error (numerically more stable than pixel-space; avoids scale ambiguity)
- Wave correction: `detail.waveCorrect(rmats, WAVE_CORRECT_HORIZ)` — post-BA rotation-matrix straightening to eliminate accumulated vertical drift in the panorama midline
- Warper: `PyRotationWarper("spherical", scale)` or `"cylindrical"` — maps images onto sphere/cylinder surface before compositing
- Exposure compensation: `detail_BlocksGainCompensator(bl_width=32, bl_height=32, nr_feeds=1, nr_iterations=2)` — divides each image into 32×32 blocks, fits per-block gain, applies 2 rounds of Gaussian smoothing to avoid block boundaries
- Seam finding: `detail_GraphCutSeamFinder("COST_COLOR_GRAD")` — solves global min-cut on the full image graph simultaneously (NOT pairwise DP); minimises colour + gradient cost across all N images jointly
- Blending: `detail_MultiBandBlender(num_bands=5)` — 5-level Laplacian pyramid; each level blended with its own feather width (2^l pixels); combines into full-resolution result

**SCANS mode** (`Stitcher_SCANS`) — more relevant for ASP:
- Wave correction: **disabled** (`waveCorrection=False`)
- Otherwise uses `detail_AffineBasedEstimator` + `detail_BundleAdjusterAffinePartial` (4-DOF: tx, ty, rotation, scale)
- Same seam finder and blender as PANORAMA

Additional `cv2.detail` classes available but not used in ASP:
- `detail_ChannelsCompensator` — per-channel (B, G, R) gain compensation, catches white-balance shifts
- `detail_BlocksChannelsCompensator` — spatial blocks × per-channel (most thorough)
- `detail_VoronoiSeamFinder` — fast Voronoi-based seam (no energy minimization; approximation)
- `detail_DpSeamFinder("COLOR_GRAD")` — DP seam (similar to ASP's DP, but operates on warped images in canvas space over all images simultaneously rather than pairwise)
- `detail_FeatherBlender(sharpness=0.02)` — simple linear feather (faster than MultiBand, lower quality)
- `detail.waveCorrect(rmats, WAVE_CORRECT_VERT)` / `WAVE_CORRECT_AUTO` — vertical wave correction for portrait panoramas

### Gap Analysis

| Pipeline Stage | OpenCV Stitcher | ASP Current | Gap / Opportunity |
|---|---|---|---|
| Feature detection | SIFT/ORB + `computeImageFeatures` | EfficientLoFTR → ALIKED → Phase Corr | ASP advantage (dense correspondences) |
| Feature matching | `BestOf2NearestMatcher` (all-pairs) | Pairwise + bg-filter + spread gate | Comparable; ASP has domain-specific bg filtering |
| Camera estimation | `HomographyBasedEstimator` (8-DOF) | Phase correlation median (2-DOF Tx,Ty) | OpenCV richer model; ASP translation-only matches scroll assumption |
| Bundle adjustment | `BundleAdjusterRay` (unit sphere) | L-M affine/translation (pixel space) | OpenCV sphere projection avoids pixel-scale nonlinearity; ASP GNC+spanning-tree is custom but less principled for rotation |
| **Wave correction** | `detail.waveCorrect(WAVE_CORRECT_HORIZ)` | None (trajectory smooth §3.16A is partial) | **ASP MISSING** — accumulated vertical drift in panorama midline not corrected |
| Warping | `PyRotationWarper` (cylindrical/spherical) | `cv2.warpAffine` (pure 2D affine) | Both valid for near-planar scroll; affine simpler/faster |
| **Exposure compensation (spatial)** | `BlocksGainCompensator` (32×32 blocks) | Global scalar gain + smooth (§1.4B, §1.98) | **ASP MAJOR GAP** — per-block spatial compensation corrects panel vignetting, spatially-varying lighting; global scalar cannot. Root cause of `strip_banding_score` |
| **Exposure compensation (channels)** | `ChannelsCompensator` or `BlocksChannelsCompensator` | Luminance-based scalar only | **ASP GAP** — per-channel correction catches white-balance shifts between frames |
| **Seam finding (global)** | `GraphCutSeamFinder` (global min-cut on all N images) | `_seam_cut` (pairwise DP per boundary) | **ASP MAJOR GAP** — GraphCut finds globally optimal seams across all images simultaneously; pairwise DP can create conflicting seams that both want the same background corridor |
| Blending | `MultiBandBlender` (5-level Laplacian pyramid) | Custom Laplacian + DSFN feather | ASP DSFN adapts to similarity; OpenCV bands are principled multi-scale |
| Post-process | Auto-crop result ROI | `_crop_to_valid` (§1.7C) | Comparable |

### Ranked Improvement Avenues

**1. Spatial blocks gain compensation (highest impact — targets strip_banding 97.9%)**  
Replace the per-frame global scalar gain with `cv2.detail_BlocksGainCompensator(32, 32)`. Feed warped frames and their masks; retrieve per-block gain maps; apply before compositing. This directly addresses the `strip_banding_score` which is near-universally worse. OpenCV's implementation uses least-squares across overlapping block pairs with Gaussian smoothing between iterations. Pure Python/cv2 — no new deps. New section: **§4.1**.

**2. GraphCut seam finding (highest seam quality — targets ghosting 93.8% + seam_visibility 88.5%)**  
Replace pairwise `_seam_cut` with `cv2.detail_GraphCutSeamFinder("COST_COLOR_GRAD")`. Takes warped images and masks in canvas space; produces per-image binary seam masks (ownership maps) across all N images simultaneously. Eliminates pairwise conflicts. Requires warped images to be available in canvas coordinates (already the case after Stage 10 render). New section: **§4.2**.

**3. Wave correction (medium impact — alignment fidelity)**  
Apply `cv2.detail.waveCorrect(rmats, cv2.detail.WAVE_CORRECT_HORIZ)` to the BA-solved rotation matrices (or affines) after Stage 7. For pure-translation scroll the rotation matrices are near-identity, so the input would be `[np.eye(3, dtype=np.float32)] * N` with tx/ty encoded separately — but the straightening effect is the same. New section: **§4.3**.

**4. Per-channel spatial gain compensation (medium impact — white-balance correction)**  
Use `cv2.detail_BlocksChannelsCompensator(bl_width=32, bl_height=32)` instead of or in addition to luminance-only gain correction. Applies per-block per-channel gain (3× more parameters). Particularly effective when scanner/camera temperature shifts between frames. New section: **§4.4**.

**5. DpSeamFinder in canvas space (lower overhead than GraphCut, higher quality than pairwise DP)**  
`cv2.detail_DpSeamFinder("COLOR_GRAD")` is the same DP as ASP but operates in canvas space on all N images simultaneously. Drop-in upgrade over pairwise `_seam_cut` without the GraphCut computational overhead. New section: **§4.5**.

**6. MultiBandBlender confidence-map weighting (quality)**  
...
- §4.3 `_wave_correct_affines(affines, axis='vertical')` in `pipeline.py` — `np.polyfit(frame_idx, tx_list, 1)` linear trend subtraction post-BA; anchor first frame; only fires when range > `WAVE_CORRECT_MIN_TX_RANGE=5.0 px`; `ASP_WAVE_CORRECT=vertical|horizontal`. Wired between §3.16 trajectory smoother and Stage 7b validation.
- §4.4 `_blocks_lum_compensate(fa_zone, fb_zone, block_size=32)` in `compositing.py` — LAB L-channel scalar block gain (avoids BGR color cast from near-zero channels); clamp [0.5, 2.0]; `ASP_BLOCKS_LUM_COMP=1`. Wired after `_blocks_gain_compensate` in `_fb_for_blend` chain.
- §3.31 `_canvas_gain_uniformity(img, n_strips=8)` in `bench_anime_stitch.py` — strip-CV luminance uniformity metric; `std(strip_means)/mean(strip_means)`; 0=uniform, high=banded; degenerate → 0.0. Added as `canvas_gain_uniformity` to all benchmark result dicts.
- **1272 backend tests (9 skipped, 5 pre-existing ptlflow failures).**
