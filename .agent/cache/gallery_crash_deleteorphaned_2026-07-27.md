...
## Addendum 30 (2026-08-18) — why cores never materialized (root-caused + fixed); first telemetry-correlated crash found; a controlled repro driver built; repro-environment caveat

This round (opencode, live display access) targeted the untried avenue Addendum 29 explicitly named: actually getting a real core dump. Three concrete outcomes, plus one important caveat about repro data quality.

**1. Root cause of "cores never materialize" found, and the pipeline fixed.** Two independent reasons no `core.<pid>` ever appeared despite hs_err claiming it would:
- `/proc/sys/kernel/core_pattern` pipes every kernel core to apport (`|/usr/share/apport/apport -p%p ...`) — so even with `ulimit -c unlimited` (which `dev/run_with_gdb.sh` already set), the kernel never writes a file; apport absorbs it (and drops these, since `/var/crash` holds none for this app).
- Once gdb is attached, the kernel never writes a core at all: gdb owns the SIGABRT, and the script's `continue` re-delivers it to the JVM handler — no core generation happens anywhere in that flow.

Fix (commit `ed8ec499`): `dev/run_with_gdb.sh` now runs gdb's `generate-core-file $OUT_DIR/core.<timestamp>` at the SIGABRT stop, **before** re-delivering the signal. This is the only path that produces a real core on this box (kernel core-pattern aside, a traced process never lets the kernel dump). The gdb-written core captures the corrupted heap at the abort moment — exactly what inspecting the damaged QObject/ConnectionData requires. Verified working: the pipeline emitted a 9.8 GB core on demand (of a false-positive crash — see caveat below). Future capture runs should use the updated script as-is.

**2. First telemetry-correlated crash found (the gap Addenda 18/20/29 kept flagging).** Today's artifacts include `hs_err_pid59193.log` + `telemetry-59193.jsonl` — same pid, so this crash *does* have live JSONL correlation. Problematic frame is a genuinely **new offset family**, not a Qt one:
```
# C [libshiboken6.abi3.so.6.10+0x22d40] Shiboken::BindingManager::retrieveWrapper(void const*) const+0x30
```
- `si_addr: 0x0` (null deref inside `retrieveWrapper`), `RDI`/`RBP = 0x5e94fc9dfd38` (the heap QObject pointer shiboken was trying to wrap). `retrieveWrapper` looks up the wrapper in the binding-manager hash; a null deref there is consistent with the binding hash being corrupted — i.e. the same underlying heap/QObject corruption, surfacing this time in PySide6/Shiboken's own bookkeeping during `main_window.construction.start` (telemetry seq 13, t=26.5s), not in Qt.
- Telemetry for the run: `login.succeeded` → `main_window.construction.start` repeats 3x within one pid (the repeated login/main-window cycle), last event `main_window.construction.start` at t=26.5s before the crash. The crash happened during MainWindow construction — same phase as the connect() crashes.
- This is consistent with `_scan_pipeline.py`'s own note that every observed crash lands "inside PySide6/Shiboken's own binding-layer bookkeeping" under heavy concurrent QObject churn.

**3. Controlled repro driver built (commit `184f585e`): `dev/repro_guest_startup.py`.** Replicates `backend/main.py`'s full process setup (QT_FFMPEG pin, libpulse/libfontconfig preloads, submodule registration, media-backend pin, file-dialog patch), then auto-logs-in as an anonymous guest and injects a crafted `session_recovery_data` payload (volatile-memory only) so the real `_restore_session_recovery()` path fires the two-panel `populate_scan_image_gallery` wallpaper race — the documented crash shape — without any GUI interaction. Run under the fixed capture pipeline:
```
RUN_ARGS="dev/repro_guest_startup.py" bash dev/run_with_gdb.sh
```
Verified the race shape fires: both linked panels start their own `ImageScannerWorker` for the same directory concurrently. **No native SIGSEGV captured yet across ~8 race firings** (stochastic); a rapid-switch "hammer" added to raise odds instead trips Python-level exceptions (uncaught exceptions in QTimer callbacks killing the process), not the native crash. A controlled deterministic reproduction of the *native* crash remains elusive.

**4. Repro-environment caveat (Harbinger, opencode).** During the gdb capture runs, the app window opens **at the cursor position**, so a mid-click lands on the window's close button — producing clean "normal exits" (and plausibly inflating some historical "crashes" too). Genuine native crashes (QObjectPrivate::connect/deleteOrphaned, QEventDispatcherGlibPrivate, shiboken retrieveWrapper) are separately confirmed by real hs_err SIGSEGV logs and are unaffected by this. Future capture runs should position the window away from the cursor (or accept the noise).

**Recommendation**: the capture pipeline is now fixed and one-key to run; the best remaining avenue is Harbinger (or a session that can reach a real-account login) running `dev/run_with_gdb.sh` during a real crash — the script will now emit a real core for the first time, and a gdb backtrace + heap inspection of the corrupted QObject would finally replace offset-guessing.

## Addendum 31 (2026-08-18) — first family crash captured during JVM/app SHUTDOWN (fresh hs_err, pid 138048)

New artifact: `hs_err_pid138048.log` (repo root), crash 2026-08-18 14:29:34, pid 138048, JVM OpenJDK 25.0.3+9 (Ubuntu 26.04), elapsed 682s (11m22s). Same crash family — `Shiboken::BindingManager::retrieveWrapper(void const*) const+0x30` (libshiboken6.abi3.so.6.10+0x22d40). But unlike every prior hs_err in this document, this one fired during **shutdown**, not startup:

- **`VM state: at safepoint (shutting down)`** — the JVM was in graceful shutdown at the fault moment.
- **`SIGTERM handler` JavaThread (id 151520)** present in the 11-thread list — this thread only exists after a SIGTERM was delivered and the JVM began running its shutdown hooks. (`Command Line:` empty; no `-Xint`/`-Xdebug`.)
- Current thread is the `JavaThread "main"` (tid = pid) in `_thread_in_native`, native frame = shiboken `retrieveWrapper`. `si_addr: 0x0000606faf9ff470` (non-null garbage — a hash lookup over corrupted state, unlike Addendum 30's clean 0x0 null deref).
- Concurrent shutdown evidence in the event log: class-loading (System$Logger$Level, WeakHashMap) at ~681,8xx ms + an `InterruptedException` thrown on a sibling thread at 681,814 ms — shutdown hooks running and interrupt-waiters being torn down.
- Registers: live image-pixel data — `XMM[8]=0x00ff001e001e001e` (BGR 30,30,30 dark grey + 0xff0000 blue), `XMM[9..14]` hold descending counters ~0x39f→0x398 (927→920) — the main thread was mid an image format-conversion / scanline-fill loop (a gallery thumbnail/viewport render) when it faulted inside `retrieveWrapper`.

**Interpretation.** A SIGTERM reached the process. Python installs `signal.signal(SIGTERM, handle_interrupt)` early in `backend/src/app.py:main()` (`handle_interrupt` → `active_window.close()` → closeEvent → `app.quit()`); the embedded JVM starts *later* (login/`VaultManager`), and JNI re-installs the OS-level SIGTERM handler, so the JVM's native handler (running graceful shutdown hooks) won over Python's. Result: the JVM began shutting down on its `SIGTERM handler` thread while the main thread kept executing Python/PySide6 — mid gallery-image render — and the next shiboken binding-hash access faulted on the main thread as the VM tore down beneath it.

**Shutdown-path context for anyone correlating.** `app.aboutToQuit.connect(vault_manager.shutdown_jvm)` (backend/src/app.py:286) fires `jpype.shutdownJVM()` as the event loop exits (Bug 1 follow-up fix). MainWindow closeEvent (`gui/src/windows/main/_lifecycle.py:129`) closes all tabs (worker cancellation) then `shutdown()` (skipped while Qt alive) then `super().closeEvent()`. If the OS-level SIGTERM goes to the JVM instead of Python's `handle_interrupt`, closeEvent never runs and the app never gets a clean shutdown — leaving Qt + scan workers running against a dying JVM. Open question (no core): is this (a) a JVM-shutdown-vs-main-thread-shiboken re-entrancy race, or (b) the same underlying QObject/heap corruption from the startup crashes merely surfacing at a different lifecycle point? The frame alone can't decide.

**No core again (expected).** Kernel `core_pattern` pipes to apport, which drops cores for this non-packaged app (nothing new in `/var/crash`, no `core.<pid>` in the repo root). The gdb pipeline (Addendum 30, commit `ed8ec499`) remains the only path to a ground-truth core.

**ComfyUI hint check (closed).** `vendor/ComfyUI` submodule is healthy at the pinned `0ea188c6` (gitlink valid after the earlier local re-init; `_submodule_bootstrap.py` registers only ASP/CSG/HIE — ComfyUI is not touched at startup). No connection between the ComfyUI submodule and this crash family.

**Status.** Diagnosis only, no code changes. The repro drivers (guest login) never start the JVM, so they cannot exercise this shutdown path; a real-account session or a JVM-active variant is required to capture this under gdb.

## Addendum 32 (2026-08-18) — JVM removed entirely (#435): the shutdown-path mechanism no longer exists; startup crashes still need the gdb pipeline

Per Harbinger's directive, `#435` (commit `a04a0952`) retired the embedded JVM cryptography module — `VaultManager` is now pure ctypes/C over `libitk_crypto.so`, no `jpype`, no JVM, anywhere in the process.

**What this closes.** Addendum 31's shutdown-path crash was root-caused to a race between Python's `SIGTERM` handler (`handle_interrupt`, installed early in `app.py:main()`) and the JVM's JNI-installed native `SIGTERM` handler winning the OS-level registration, so the JVM ran shutdown hooks on a `SIGTERM handler` thread while the main thread kept touching PySide6/shiboken bindings mid-teardown. **With no JVM in the process, there is no second native signal handler to race against — `handle_interrupt` is now the only `SIGTERM` handler, full stop.** This crash mechanism is structurally gone, not just harder to hit. `app.aboutToQuit.connect(vault_manager.shutdown_jvm)` and the "skip shutdown while Qt alive" branch (`shutdown()`/`shutdown_jvm()` are now no-ops per #435) are dead code paths by construction.

**What this does NOT close.** The startup-path crashes (Addenda 24-30: `QObjectPrivate::connect`, `deleteOrphaned`, `QEventDispatcherGlibPrivate::~QEventDispatcherGlibPrivate`, and Addendum 30's clean-null-deref `Shiboken::BindingManager::retrieveWrapper`) were captured with the JVM already up (login always starts it, pre-#435) but none of that analysis depended on JVM shutdown-hook interference — the working theory across those addenda is a QObject/binding-hash heap corruption from the wallpaper-panel dual-scan race (Addenda 16/21/22/26/28), unrelated to the JVM's presence. #435 removes one confirmed crash mechanism and one long-standing piece of investigative noise (two different native handlers touching process-wide signal state), but the primary open thread — ground-truth heap inspection of the corrupted binding via the fixed gdb core-capture pipeline (Addendum 30, commit `ed8ec499`) — is unaffected and still the next concrete step for whoever picks this back up.

**Practical effect for future repro rounds:** any *new* crash captured post-#435 that still shows a `SIGTERM handler`/`shutting down` VM state can be ruled out immediately (no JVM = impossible now) — if that shape recurs, it's a different bug, not this one. Startup-path repro attempts (`dev/repro_guest_startup.py`) are otherwise unaffected; guest-login sessions never started the JVM anyway (Addendum 30/31's own noted repro gap), so the driver's behavior doesn't change. **`dev/run_with_gdb.sh` itself did need a fix post-#435 — see Addendum 33.**

No code changes this round beyond documentation — #435 was landed and committed separately, this addendum just closes the loop between that work and this investigation.

## Addendum 33 (2026-08-18) — `run_with_gdb.sh`'s SIGSEGV-passthrough was stale post-#435 (fixed); three more repro attempts, still no crash

Following on from Addendum 32: `dev/run_with_gdb.sh` deliberately passed SIGSEGV through untouched (`handle SIGSEGV nostop noprint pass`) specifically because the (now-removed) JVM raised SIGSEGV internally for benign safepoint/null-check purposes, and its own crash handler converted a real fatal SIGSEGV into SIGABRT+hs_err — which is how every prior SIGSEGV in this investigation (Addenda 29-31, `Shiboken::BindingManager::retrieveWrapper`) actually got caught by this script at all (gdb only stopped once the JVM turned it into a SIGABRT). **With the JVM gone, that conversion no longer happens: a real native SIGSEGV now passes straight through gdb with zero diagnostics** — the opposite of this tool's purpose, and a regression `#435` introduced as a side effect (not a bug in #435 itself, since #435 wasn't touching this file, just an assumption of this script's that #435 invalidated).

**Fixed** (working tree, this round): `handle SIGSEGV stop print` (matching `SIGABRT`'s existing handling) — both signals now get an all-thread backtrace + `generate-core-file` before `continue` re-delivers to the process's own default handler (there's no JVM handler left to re-deliver to). Comments updated to explain the JVM-removal context for future readers. No other behavior change.

**Three more repro attempts this round** (`RUN_ARGS="dev/repro_guest_startup.py" bash dev/run_with_gdb.sh`, ~60-90s each, verified the fixed script runs clean end-to-end: guest login, recovery injection, wallpaper hammer all fire as designed) — **no native crash landed**, consistent with the race's known stochastic rate (Addendum 30: "no native SIGSEGV captured yet across ~8 race firings"). Not a negative result worth reading into; just more attempts logged so the next round knows the count. The fixed script is otherwise verified working (clean startup/shutdown each time, no leaked processes).

**Status.** `run_with_gdb.sh` fix committed (dev-tool only, no product code touched). Next concrete step unchanged from Addendum 30: more repro volume (ideally on a machine/session that can leave it running longer, or a real-account login instead of the guest-login driver) is the only way to actually land a crash under the now-correct gdb config.
