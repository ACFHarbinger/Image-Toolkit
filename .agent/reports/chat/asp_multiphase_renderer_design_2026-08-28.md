...
**Status:** §7 signed off 2026-08-28. §4 → MIDDLE branch. Steps 2–5 landed (`84af1be`), fixed (`7e352ba`), re-reviewed GO (`9b6116a2`). **Sweep DONE 2026-08-29** (`asp_multiphase_sweep_2026-08-29.md`): memory cleared (~5 GB peak); mechanically sound; 2 stable wins (test05, test41); the rest fail the seam gates on the `_blend_phase_plates` join; Option B flips nothing → drop it. **Stays default-off.** Next: fix the band-join seam. Also fixed BiRefNet batch VRAM (`e720ccd1`).
...
6. ~~**First render sweep**~~ — **DONE 2026-08-29 (Claude, conditional Harbinger auth). See `.agent/reports/chat/asp_multiphase_sweep_2026-08-29.md`.** Both arms (piecewise P1; + `ASP_PHASE_COMPOSITE=1`) on the 20-case set, RTX 4080 laptop / 31 GB. **Memory review: cleared** (peak RSS ~5 GB, `2.0+3.0·n_phases` is a loose bound not a prediction, no cap needed). Verdicts **identical** between arms: 3 raw_asp / 7 safe_asp-fallback / 10 scans. **Two stable multi-phase wins: `test05` (3ph), `test41` (2ph).** The other engaged cases fail `seam_vis_gate` / `composite_gate_sb` (asp_sv 44–145 vs limit 35) → the `_blend_phase_plates` band join produces visible seams. **Option B flips zero verdicts** (lowers the seam metric ~20–35% but sub-threshold) → **drop it.** ~10/20 frozen-`RAW_ASP` cases now fall back pre-composite (`disconnected_edge_graph`/`affine_invalid`) — separate registration regression. Also landed: BiRefNet batch VRAM fix (root `e720ccd1`, 11.8→7.8 GB, 0 OOM).
7. **No default-ON** without a full-97 run and Phase 0.1 human coherence ratings, per roadmap Ground Rules. **Next work item: the `_blend_phase_plates` seam** must survive the seam gates (feather width / gradient-domain join / photometric seam-band alignment) before piecewise-P1 is a quality win.
