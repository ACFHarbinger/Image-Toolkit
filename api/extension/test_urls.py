"""Test-only URLConf: mounts just ``api.extension.urls`` under its real
...
actually for. ``api.extension``'s test cases point ``ROOT_URLCONF`` at this
module instead (``@override_settings(ROOT_URLCONF="api.extension.test_urls")``
...
urlpatterns = [
    path("api/extension/", include("api.extension.urls")),
]
