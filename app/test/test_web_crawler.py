...
class TestImageCrawler:
    ...
    @patch('src.web.image_crawler.os.path.join', side_effect=lambda *args: '/'.join(args))
    @patch('src.web.image_crawler.os.path.basename', return_value="image_from_url.jpg?q=1")
    @patch('src.web.image_crawler.os.path.splitext', return_value=("image_from_url", ".jpg"))
    @patch('src.web.image_crawler.os.path.exists', return_value=False)
    @patch('builtins.open', new_callable=MagicMock)
    def test_download_image_from_url_success(self, mock_open, mock_exists, mock_splitext, mock_basename, mock_join, crawler, mock_requests):
        """Tests successful image download and file writing."""
        
        result = crawler._download_image_from_url("http://example.com/path/image_from_url.jpg?q=1")
        ...
        crawler.on_image_saved.emit.assert_called_once_with('/tmp/test_downloads/image_from_url.jpg')
    ...
    def test_download_image_from_url_failure(self, crawler, mock_requests):
        ...
        with patch('src.web.image_crawler.print') as mock_print:
            ...
            mock_print.assert_called_with(
                "Failed http://example.com/fail.jpg: Network Error"
            )
    ...
    def test_process_data_filtering_and_download(self, crawler):
        ...
        EXPECTED_DOWNLOAD_URLS = {
            ...
            urljoin('http://example.com/page-X.html', '/img3.jpg'), # Mocked to FAIL download
        ...
        # Assign side effects to mock elements
        mock_images[0].get_attribute.side_effect = lambda attr: "data:image/gif" if attr == "src" else None
        mock_images[1].get_attribute.side_effect = lambda attr: "/img1.jpg" if attr == "src" else None
        mock_images[2].get_attribute.side_effect = lambda attr: "http://cdn/img2.jpg" if attr == "src" else None
        mock_images[3].get_attribute.side_effect = lambda attr: "/img3.jpg" if attr == "src" else None
        mock_images[4].get_attribute.side_effect = lambda attr: None if attr == "src" else None # Test None src
        mock_images[5].get_attribute.side_effect = lambda attr: "/img5.jpg" if attr == "src" else None
        mock_images[6].get_attribute.side_effect = lambda attr: "/img6.jpg" if attr == "src" else None
        mock_images[7].get_attribute.side_effect = lambda attr: "/img7.jpg" if attr == "src" else None
        mock_images[8].get_attribute.side_effect = lambda attr: "/img8.jpg" if attr == "src" else None
        mock_images[9].get_attribute.side_effect = lambda attr: "/img9.jpg" if attr == "src" else None 
        ...
        with patch.object(crawler, '_download_image_from_url', side_effect=lambda url: url != mock_img3_url) as mock_download:
            
            # Mock URL for relative paths (Required to fix previous AttributeError on current_url property)
            type(crawler.driver).current_url = MagicMock(return_value='http://example.com/page-X.html')
            ...
            crawler.on_status.assert_any_call("Scanning for images...")
            # The number of unique URLs is 7.
            crawler.on_status.assert_any_call("Downloading 7 unique images from page 1...")
    ...
    def test_run_main_loop(self, crawler):
        ...
        with patch.object(crawler, 'process_data', side_effect=[3, 5]) as mock_process_data:
            ...
            crawler.on_status.emit.assert_any_call("Crawl complete. Downloaded 8 total images.")
    ...
    def test_run_cancellation(self, crawler):
        ...
        with patch.object(crawler, 'process_data', side_effect=process_data_side_effect):
            
            total_downloaded = crawler.run()
            
            # Assertions
            with patch.object(crawler, 'process_data', side_effect=[1]) as mock_process_data:
                assert total_downloaded == 2
                assert mock_process_data.call_args_list == [call('url1'), call('url2')]
                
                # Loop should break after url2 because crawler.driver is now None
                crawler.on_status.emit.assert_not_called_with(
                    "Crawl complete. Downloaded 1 total images."
                )
