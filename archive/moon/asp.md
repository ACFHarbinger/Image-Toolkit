...
## 0.1 Foreground Pose Registration — The Core Fix [Priority 0] {: #01-foreground-pose-registration--the-core-fix-priority-0 }
...
<a id="02-pose-consistency-aware-frame-selection-priority-1"></a><a id="02-pose-consistency-aware-frame-selection-priority-1--infrastructure-built-disabled"></a>## 0.2 Pose-Consistency-Aware Frame Selection [Priority 1 — Infrastructure Built, Disabled]
...
## 0.5 min_gap Threshold Calibration [Priority 2 — Quick Win] {: #05-min_gap-threshold-calibration-priority-2--quick-win }
...
## 2.0 ASP Human-in-the-Loop Augmentation [Priority: Medium — Unique Multiplier] {: #20-asp-human-in-the-loop-augmentation-priority-medium--unique-multiplier }
...
### 2.1 Frame Selection Assistant [Quick Win] \[Option A ✅ Session 79\] {: #21-frame-selection-assistant-quick-win }
...
### 2.2 Edge Graph Inspector & Editor [Quick Win] \[Read-Only Viewer ✅ Session 62\] {: #22-edge-graph-inspector--editor-quick-win }
...
### 2.3 Anchor Frame & Canvas Layout Inspector [Priority: Medium] \[Read-Only Viewer ✅ Session 63\] {: #23-anchor-frame--canvas-layout-inspector-priority-medium }
...
### 2.4 Seam Registration Inspector [Highest Impact] \[Option A ✅ Session 95–96\] {: #24-seam-registration-inspector-highest-impact }
...
### 2.6 Output Scale & Crop Assistant [✅ Stage 12.5 shipped — Session 7] {: #26-output-scale--crop-assistant-priority-medium---test27-fix }
...
### 2.7 Architecture: StitchWorker Staged Execution [Implementation Foundation] \[✅ Session 79\] {: #27-architecture-stitchworker-staged-execution-implementation-foundation }
...
### 3.1 AnimeInterp SGM: Segment-Guided Matching for Flat-Region Correspondence [Research — Highest Aperture-Problem Impact] {: #31-animeinterp-sgm-segment-guided-matching-for-flat-region-correspondence-research--highest-aperture-problem-impact }
...
### 3.3 DINOv2 + SigLIP Submodular Frame Selection [✅ Option A shipped — Session 8] {: #33-dinov2--siglip-submodular-frame-selection-priority-high--directly-addresses-gt-coupling }
...
### 3.4 FD-Means Animation Hold Detection [Quick Win — preprocessing] {: #34-fd-means-animation-hold-detection-quick-win--preprocessing }
...
### 3.6 ToonCrafter Seam Synthesis — Wiring the Generative Fallback [✅ Option B shipped — Session 9] {: #36-tooncrafter-seam-synthesis--wiring-the-generative-fallback-priority-medium }
...
### 3.7 UDIS++ / UDTATIS Diffusion-Based Seam Composition [Long-term — End-to-End Replacement] {: #37-udis--udtatis-diffusion-based-seam-composition-long-term--end-to-end-replacement }
...
### 3.8 SIQE No-Reference Ghosting Detection [Quick Win — metric upgrade] {: #38-siqe-no-reference-ghosting-detection-quick-win--metric-upgrade }
...
### 3.9 SI-FID: Stitched-Image Fréchet Distance for Reference-Free Evaluation [Research] {: #39-si-fid-stitched-image-fréchet-distance-for-reference-free-evaluation-research }
...
### 3.10 MLLM Semantic Quality Scoring [Research — Autonomous Quality Assurance] {: #310-mllm-semantic-quality-scoring-research--autonomous-quality-assurance }
...
<a id="111-animation-hold-detection--preprocessing--option-a-shipped--session-6"></a><a id="111-animation-hold-detection--preprocessing-quick-win--session-6"></a>## 1.11 Animation Hold Detection — Preprocessing [✅ Option A shipped — session 6]
...
## 1.32 GNC-TLS Bundle Adjustment [Quick Win] ✅ Shipped S76 {: #132-gnc-tls-bundle-adjustment-quick-win--shipped-s76 }
...
## 2.9 BigWarp / Fourier-Mellin Manual Registration Fallback [Priority: High HITL] {: #29-bigwarp--fourier-mellin-manual-registration-fallback-priority-high-hitl }
...
## 2.10 SAM2Flow / FlowVid Interactive Optical Flow Kinematics [Research — HITL] {: #210-sam2flow--flowvid-interactive-optical-flow-kinematics-research--hitl }
...
## 2.11 Intelligent Scissors Seam Routing [Quick Win — replaces DP seam] {: #211-intelligent-scissors-seam-routing-quick-win--replaces-dp-seam }
...
## 3.11 SAM 2 — Interactive Masking Upgrade [Research — HITL] {: #311-sam-2--interactive-masking-upgrade-research--hitl }
...
## 3.12 Overmix Sub-Pixel Averaging — Maximal Frame Ingestion Philosophy [Research] {: #312-overmix-sub-pixel-averaging--maximal-frame-ingestion-philosophy-research }
...
### 3.13 ProPainter Background Completion [Research — Highest Background Plate Impact] {: #313-propainter-background-completion-research--highest-background-plate-impact }
...
### 3.14 Horizontal and Diagonal Scroll — 2D Canvas Support [Engineering — Unblocks Category F/H] {: #314-horizontal-and-diagonal-scroll--2d-canvas-support-engineering--unblocks-category-fh }
...
### 3.15 OBJ-GSP + SemanticStitch Mesh-Based Seam Barrier [Research — Character-Preserving Seam Routing] {: #315-obj-gsp--semanticstitch-mesh-based-seam-barrier-research--character-preserving-seam-routing }
...
### 3.16 StabStitch++ Trajectory Smoothing for Multi-Axis Scroll [Engineering — Fixes Genuine Category F Fallbacks] {: #316-stabstitch-trajectory-smoothing-for-multi-axis-scroll-engineering--fixes-genuine-category-f-fallbacks }
...
### §5.29 Pipeline Ghosting SIQE Gate ✅ [Priority: Medium — ghost artifact SCANS fallback]

**Problem**: The existing GhostGate (§4.3) is a comparative bench gate only. The pipeline had no per-run ghost detection that could divert to SCANS without running the full bench comparison.

**Implementation** *(shipped S176)*:
- `_canvas_ghosting_siqe(img)` in `canvas.py` — FFT autocorrelation of column-mean Sobel gradient; secondary peak at lag D in [5, H//4]; score 0–100
- Stage 11.28 in `pipeline.py`: fires when `siqe > _SIQE_GATE_FLOOR` (default 30.0) → SCANS fallback
- Flags: `_SIQE_GATE_ENABLED` (`ASP_GATE_GHOSTING_SIQE`), `_SIQE_GATE_FLOOR` (`ASP_GATE_GHOSTING_SIQE_FLOOR`)
- `SIQE_GATE_FLOOR = 30.0` in `constants/animation.py`
- 5 tests in `TestSiqeGatePipeline` (`test_pipeline.py`)

---

### §5.30 Benchmark Ghosting SIQE Comparative Gate ✅ [Priority: Low — bench ghost parity]

**Problem**: Bench had no comparative SIQE ghost check analogous to the pipeline's Stage 11.28.

**Implementation** *(shipped S176)*:
- GhostSiqeGate block after ChromaSeamGate in `run_dataset()` — reuses `_ghosting_score_v2`
- Fires when `asp_siqe > max(_GHOST_SIQE_ABS_FLOOR, _GHOST_SIQE_RATIO_LIMIT × sim_siqe)`
- `_GHOST_SIQE_RATIO_LIMIT=2.0` (`ASP_GATE_GHOST_SIQE_RATIO`), `_GHOST_SIQE_ABS_FLOOR=30.0`
- Schema entry `ASP_GATE_GHOST_SIQE_RATIO` in `config.py`
- 5 tests in `TestGhostSiqeGate` (`test_bench_metrics.py`)

---

### §5.31 Pipeline Seam Band NCC Gate ✅ [Priority: Medium — seam discontinuity SCANS fallback]

**Problem**: A seam can be numerically coherent (SC gate passes) yet visually discontinuous if pixel content above and below the boundary have very different texture structure.

**Implementation** *(shipped S176)*:
- `_seam_band_ncc_min(img, n_strips=8, band_px=10)` in `canvas.py` — minimum NCC between ±band_px bands above/below each strip boundary
- Stage 11.29 in `pipeline.py`: **fires when `ncc < floor`** (LOW NCC = bad — inverted from other gates)
- Flags: `_SEAM_BAND_NCC_GATE_ENABLED` (`ASP_GATE_SEAM_BAND_NCC`), `_SEAM_BAND_NCC_GATE_FLOOR` (`ASP_GATE_SEAM_BAND_NCC_FLOOR`, default 0.30)
- `SEAM_BAND_NCC_GATE_FLOOR = 0.30` in `constants/animation.py`
- 5 tests in `TestSeamBandNccGatePipeline` (`test_pipeline.py`)

---

### §5.32 Pipeline Strip Gradient CV Gate ✅ [Priority: Medium — sharpness consistency SCANS fallback]

**Problem**: A stitched canvas can have strips of inconsistent sharpness — one strip sharp, the next blurred — indicating alignment/blending failures that no luminance-based gate catches.

**Implementation** *(shipped S176)*:
- `_strip_gradient_cv(img, n_strips=8)` in `canvas.py` — coefficient of variation of per-strip mean Laplacian energy
- Stage 11.30 in `pipeline.py`: fires when `cv > _STRIP_GRAD_CV_GATE_FLOOR` (default 0.50) → SCANS fallback
- Flags: `_STRIP_GRAD_CV_GATE_ENABLED` (`ASP_GATE_STRIP_GRAD_CV`), `_STRIP_GRAD_CV_GATE_FLOOR` (`ASP_GATE_STRIP_GRAD_CV_FLOOR`)
- `STRIP_GRAD_CV_GATE_FLOOR = 0.50` in `constants/animation.py`
- 5 tests in `TestStripGradCvGatePipeline` (`test_pipeline.py`)

---

### §5.33 Pipeline Seam Gradient Ratio Gate ✅ [Priority: Medium — hard seam cut SCANS fallback]

**Problem**: Stitched canvases can have hard luminance/gradient cuts at strip boundaries (visible as distinct horizontal lines), undetected by CGU or SC gates.

**Implementation** *(shipped S177)*:
- `_strip_seam_gradient_score(img, n_strips=8)` in `canvas.py` — max ratio of boundary-row Laplacian gradient (±5px window) to middle-third interior gradient across all strip boundaries; capped at 10.0
- Stage 11.31 in `pipeline.py`: fires when `ratio > _SEAM_GRAD_RATIO_GATE_FLOOR` (default 3.0) → SCANS fallback
- Flags: `_SEAM_GRAD_RATIO_GATE_ENABLED` (`ASP_GATE_SEAM_GRAD_RATIO`), `_SEAM_GRAD_RATIO_GATE_FLOOR` (`ASP_GATE_SEAM_GRAD_RATIO_FLOOR`)
- `SEAM_GRAD_RATIO_GATE_FLOOR = 3.0` in `constants/animation.py`
- 5 tests in `TestSeamGradRatioGatePipeline` (`test_pipeline.py`)

---

### §5.34 Pipeline Canvas Aspect-Ratio Gate ✅ [Priority: Medium — wrong scroll axis SCANS fallback]

**Problem**: A canvas that is landscape (W > H) is almost certainly the result of wrong scroll axis detection (horizontal instead of vertical). The existing `_detect_scroll_axis` gate fires earlier, but this gate catches cases where the canvas grew sideways due to bad affine solutions.

**Implementation** *(shipped S177)*:
- `_canvas_aspect_ratio(img)` in `canvas.py` — H/W ratio of the output canvas; portrait = correct; landscape = failure
- Stage 11.32 in `pipeline.py`: **fires when `ratio < floor`** (LOW = landscape = failure); dynamic floor = `max(_CANVAS_ASPECT_GATE_FLOOR, N * 0.3)`
- Flags: `_CANVAS_ASPECT_GATE_ENABLED` (`ASP_GATE_CANVAS_ASPECT`), `_CANVAS_ASPECT_GATE_FLOOR` (`ASP_GATE_CANVAS_ASPECT_FLOOR`)
- `CANVAS_ASPECT_GATE_FLOOR = 1.2` in `constants/animation.py`
- 5 tests in `TestCanvasAspectGatePipeline` (`test_pipeline.py`)

---

### §5.35 Benchmark Seam Band NCC Comparative Gate ✅ [Priority: Low — bench NCC parity]

**Problem**: The bench suite lacked a NCC-based comparative gate analogous to the pipeline's Stage 11.29; mismatched seam boundaries could pass all luminance gates but fail NCC.

**Implementation** *(shipped S177)*:
- SeamBandNccGate block after GhostSiqeGate in `run_dataset()` — reuses `_seam_band_ncc_min` from canvas.py
- Fires when `asp_ncc < _SEAM_NCC_ABS_FLOOR` (default 0.10) OR `asp_ncc < 0.5 × sim_ncc` when sim_ncc > 0.1
- Module constants: `_SEAM_NCC_ABS_FLOOR=0.10`, `_SEAM_NCC_RATIO=0.5`; schema entries `ASP_GATE_SEAM_NCC_FLOOR`, `ASP_GATE_SEAM_NCC_RATIO`
- 5 tests in `TestSeamBandNccGateBench` (`test_bench_metrics.py`)

---

### §5.36 Pipeline Strip Histogram Intersection Gate ✅ [Priority: Medium — color mismatch SCANS fallback]

**Problem**: Color-step artifacts between strips (different color temperatures, saturation mismatches) that pass luminance-based gates but are visually obvious to the human eye.

**Implementation** *(shipped S177)*:
- `_strip_hist_intersection_min(img, n_strips=8)` in `canvas.py` — minimum normalized histogram intersection (64-bin greyscale, cv2.HISTCMP_INTERSECT) between all adjacent strip pairs; 0=completely different histograms, 1=identical
- Stage 11.33 in `pipeline.py`: **fires when `intersection < floor`** (LOW = color mismatch); default floor 0.35
- Flags: `_HIST_INTERSECT_GATE_ENABLED` (`ASP_GATE_HIST_INTERSECT`), `_HIST_INTERSECT_GATE_FLOOR` (`ASP_GATE_HIST_INTERSECT_FLOOR`)
- `HIST_INTERSECT_GATE_FLOOR = 0.35` in `constants/animation.py`
- 5 tests in `TestHistIntersectGatePipeline` (`test_pipeline.py`)

---

### §5.37 Bench Histogram Intersection Comparative Gate ✅ [Priority: Low — bench color-mismatch parity]

**Problem**: The pipeline's Stage 11.33 HistIntersectGate triggers a SCANS fallback when per-strip color distributions diverge. The benchmark has no corresponding comparative check to record when ASP's histogram intersection is worse than SCANS, so comparative analysis is blind to this failure mode.

**Implementation** *(shipped S178)*:
- `HistIntersectGate` block in `bench_anime_stitch.py run_dataset()` after `SeamBandNccGate`
...
- Module constants: `_HIST_INTERSECT_ABS_FLOOR`, `_HIST_INTERSECT_RATIO`; schema entry `ASP_GATE_HIST_INTERSECT_RATIO` added to `config.py`
- 5 tests in `TestHistIntersectGateBench` (`test_bench_metrics.py`)

---

### §5.38 Pipeline Strip Saturation CV Gate ✅ [Priority: Medium — color saturation mismatch SCANS fallback]

**Problem**: Seam boundaries can induce color saturation mismatches between strips — one strip is vivid while adjacent strips appear washed-out. No gate catches this failure mode directly.

**Implementation** *(shipped S178)*:
- `_strip_sat_cv(img, n_strips=8)` in `canvas.py` — coefficient of variation (std/mean) of per-strip mean HSV S-channel saturation; returns 0.0 for monochrome/degenerate input
...
### §5.39 Pipeline Canvas Valid-Area Ratio Gate ✅ [Priority: Medium — underfilled canvas SCANS fallback]

**Problem**: When frame alignment fails catastrophically, the composite canvas may be mostly black (frames didn't overlap as expected). No gate fires on this underfilled canvas condition.

**Implementation** *(shipped S178)*:
- `_canvas_valid_area_ratio(img, black_threshold=8)` in `canvas.py` — fraction of pixels with grayscale > threshold; low ratio = large black/empty regions; returns 1.0 for degenerate input
- Stage 11.35 gate in `pipeline.py`: **fires when `ratio < floor`** (LOW = underfilled); default floor 0.55
...
### §5.40 Bench Seam Gradient Ratio Comparative Gate ✅ [Priority: Low — bench hard-seam parity]

**Problem**: The pipeline's Stage 11.31 `SeamGradRatioGate` triggers SCANS when ASP's seam gradient ratio exceeds the floor. The benchmark has no corresponding comparative gate to detect when ASP's seam cuts are disproportionately hard vs SCANS.

**Implementation** *(shipped S178)*:
- `SeamGradRatioGate` block in `bench_anime_stitch.py run_dataset()` after `HistIntersectGate`
- Dual condition: fires when `asp_sgr > _SEAM_GRAD_RATIO_ABS_FLOOR` (default 5.0) AND `asp_sgr > _SEAM_GRAD_RATIO_LIMIT × max(sim_sgr, 0.1)` (default 2.0×) — absolute floor prevents false positives on natural edge sequences
- Module constants: `_SEAM_GRAD_RATIO_ABS_FLOOR`, `_SEAM_GRAD_RATIO_LIMIT`; 2 schema entries in `config.py`
- 5 tests in `TestSeamGradRatioGateBench` (`test_bench_metrics.py`)

---

### §5.41 Pipeline Strip Hue CV Gate ✅ [Priority: Medium — hue-shift seam detection]

**Problem**: Seam-stitching can produce strips with inconsistent hue (hue shift across seam boundaries). No existing gate detects inter-strip hue inconsistency. The saturation CV gate (§5.38) catches saturation jumps but misses purely hue-shifted artifacts.

**Implementation** *(shipped S179)*:
- `_strip_hue_cv(img, n_strips=8)` in `canvas.py` — circular mean hue per strip via cos/sin averaging (handles OpenCV 0–179 hue wrap); returns std/mean of per-strip mean-hue angles; returns 0.0 for monochrome (mean sat < 1) or degenerate input
...
### §5.42 Pipeline Seam Boundary Sharpness Ratio Gate ✅ [Priority: Medium — hard-cut seam detection]

**Problem**: Hard seam cuts appear as unusually sharp rows at expected strip boundaries compared to strip interiors. The gradient ratio gate (§5.33) measures mean absolute gradient; this gate uses Laplacian variance — more sensitive to impulsive high-frequency edges.

**Implementation** *(shipped S179)*:
- `_seam_boundary_sharpness_ratio(img, n_strips=8, boundary_px=3)` in `canvas.py` — Laplacian variance in ±boundary_px around each seam boundary row vs interior (middle half of each strip); returns max ratio across boundaries, capped at 50.0; returns 0.0 when interior_var < 1.0 to avoid degenerate flat-image false positives
...
### §5.43 Bench Canvas Valid-Area Comparative Gate ✅ [Priority: Low — bench underfilled-canvas parity]

**Problem**: The pipeline's Stage 11.35 `ValidAreaGate` triggers SCANS when ASP's canvas valid-area ratio falls below floor. The benchmark has no corresponding comparative gate to detect when ASP's canvas fill is disproportionately low vs SCANS.

**Implementation** *(shipped S179)*:
- `ValidAreaGate` block in `bench_anime_stitch.py run_dataset()` after `SeamGradRatioGate`; reuses `_canvas_valid_area_ratio` from `canvas.py`
- Dual condition: fires when `asp_va < _VALID_AREA_ABS_FLOOR` (default 0.30) OR `asp_va < _VALID_AREA_RATIO × sim_va` (default 0.7×) when `sim_va > 0.5`
- Module constants: `_VALID_AREA_ABS_FLOOR`, `_VALID_AREA_RATIO` (env: `ASP_GATE_VALID_AREA_FLOOR`, `ASP_GATE_VALID_AREA_RATIO`)
- 2 schema entries in `config.py`; 5 tests in `TestValidAreaGateBench` (`test_bench_metrics.py`)

---

### §5.44 Bench Strip Saturation CV Comparative Gate ✅ [Priority: Low — bench saturation-banding parity]

**Problem**: The pipeline's Stage 11.34 `SatCvGate` triggers SCANS when ASP's strip saturation CV is too high. The benchmark has no corresponding comparative gate to detect when ASP's saturation variability is disproportionately high vs SCANS.

**Implementation** *(shipped S179)*:
- `SatCvGate` block in `bench_anime_stitch.py run_dataset()` after `ValidAreaGate`; reuses `_strip_sat_cv` from `canvas.py`
- Dual condition: fires when `asp_sc > _SAT_CV_ABS_FLOOR` (default 0.30) AND `asp_sc > _SAT_CV_RATIO × max(sim_sc, 0.001)` (default 2.0×) — absolute floor prevents false positives on genuinely saturated sequences
- Module constants: `_SAT_CV_ABS_FLOOR`, `_SAT_CV_RATIO` (env: `ASP_GATE_SAT_CV_ABS_FLOOR`, `ASP_GATE_SAT_CV_RATIO`)
- 2 schema entries in `config.py`; 5 tests in `TestSatCvGateBench` (`test_bench_metrics.py`)

---

### §5.45 Pipeline Strip Luma Range Gate ✅ [Priority: Medium — strip-level banding detection]

**Problem**: Seam-induced luminance banding creates large luma differences between horizontal strips. No gate existed to catch composite outputs with extreme strip-level luma range.

**Implementation** *(shipped S180)*:
- `_strip_luma_range(img, n_strips=8)` in `canvas.py` — absolute luminance range across horizontal strips (max − min strip mean); 0.0 for degenerate input
...
### §5.46 Pipeline Seam Edge Density Gate ✅ [Priority: Medium — noise/artifact overload detection]

**Problem**: Artifact-heavy or noise-overloaded composites show abnormally high edge density across strips. No gate existed to flag outputs with excessive Canny edge coverage.
...
### §5.47 Bench Strip Luma Range Comparative Gate ✅ [Priority: Low — bench strip-banding parity]

**Problem**: The pipeline's Stage 11.38 `LumaRangeGate` triggers SCANS when ASP's strip luma range is too high. The benchmark lacks a corresponding comparative gate to detect when ASP's luma range is disproportionately high vs SCANS.

**Implementation** *(shipped S180)*:
- `LumaRangeGate` block in `bench_anime_stitch.py run_dataset()` after `SatCvGate`; reuses `_strip_luma_range` from `canvas.py`
- Dual condition: fires when `asp_lr > _LUMA_RANGE_ABS_FLOOR` (default 30.0) AND (`sim_lr < 5.0` OR `asp_lr > 2.0 × max(sim_lr, 1.0)`)
- Module constants: `_LUMA_RANGE_ABS_FLOOR`, `_LUMA_RANGE_RATIO`; 2 schema entries in `config.py`
- `timings["render_gate_fallback"] += 128` when gate fires; 5 tests in `TestLumaRangeGateBench` (`test_bench_metrics.py`)

---

### §5.48 Bench Strip Edge Density Comparative Gate ✅ [Priority: Low — bench edge-density parity]

**Problem**: The pipeline's Stage 11.39 `EdgeDensityGate` triggers SCANS when ASP's edge density is too high. The benchmark lacks a corresponding comparative gate to detect when ASP's edge density is disproportionately high vs SCANS.

**Implementation** *(shipped S180)*:
- `EdgeDensityGate` block in `bench_anime_stitch.py run_dataset()` after `LumaRangeGate`; reuses `_seam_edge_density` from `canvas.py`
- Dual condition: fires when `asp_ed > _EDGE_DENSITY_ABS_FLOOR` (default 0.15) AND (`sim_ed < 0.01` OR `asp_ed > 2.5 × max(sim_ed, 0.001)`)
- Module constants: `_EDGE_DENSITY_ABS_FLOOR`, `_EDGE_DENSITY_RATIO`; 2 schema entries in `config.py`
- `timings["render_gate_fallback"] += 64` when gate fires; 5 tests in `TestEdgeDensityGateBench` (`test_bench_metrics.py`)

---

### §5.49 Pipeline Strip Luma MAD Gate ✅ [Priority: Medium — strip-level banding MAD detection]

**Problem**: `_strip_luma_range` (§5.45) captures the extreme high-low span but is insensitive to systematic banding where multiple strips are shifted uniformly away from the global mean. Mean absolute deviation (MAD) of per-strip luma means provides a complementary signal focused on population dispersion rather than extremes.

**Implementation** *(shipped S181)*:
- `_strip_luma_mad(img, n_strips=8)` in `canvas.py` — MAD of per-strip luma means from global mean; 0.0 for degenerate input
- Stage 11.40 gate in `pipeline.py`: fires when `_lmad_val > _LUMA_MAD_GATE_FLOOR` (default 20.0) → SCANS; reason `luma_mad_gate:{val:.2f}`
...
### §5.50 Pipeline Strip Sharpness CV Gate ✅ [Priority: Medium — mixed-frame sharpness detection]

**Problem**: When composite strips come from frames with mismatched sharpness (e.g., motion-blurred frame stitched with a sharp frame), the per-strip Laplacian variance varies dramatically across strips. The coefficient of variation (std/mean) of strip Laplacian variance flags this without requiring a reference image.

**Implementation** *(shipped S181)*:
- `_strip_sharpness_cv(img, n_strips=8)` in `canvas.py` — CV of per-strip Laplacian variance; 0.0 when mean sharpness < 1.0 (flat image guard) or degenerate
- Stage 11.41 gate in `pipeline.py`: fires when `_scv_val > _SHARPNESS_CV_GATE_FLOOR` (default 1.0) → SCANS; reason `sharpness_cv_gate:{val:.4f}`
...
### §5.51 Bench Strip Luma MAD Comparative Gate ✅ [Priority: Low — bench strip-MAD parity]

**Problem**: The pipeline's Stage 11.40 `LumaMadGate` triggers SCANS when ASP's per-strip luma MAD is too high. The benchmark lacks a corresponding comparative gate to detect when ASP's luma MAD is disproportionately high vs SCANS.

**Implementation** *(shipped S181)*:
- `LumaMadGate` block in `bench_anime_stitch.py run_dataset()` after `EdgeDensityGate`; reuses `_strip_luma_mad` from `canvas.py`
- Dual condition: fires when `asp_lmad > _LUMA_MAD_ABS_FLOOR` (default 10.0) AND (`sim_lmad < 2.0` OR `asp_lmad > 2.0 × max(sim_lmad, 1.0)`)
- Module constants: `_LUMA_MAD_ABS_FLOOR`, `_LUMA_MAD_RATIO`; 4 schema entries in `config.py` (abs floor + ratio for §5.51/§5.52)
- `timings["render_gate_fallback"] += 16` when gate fires; 5 tests in `TestLumaMadGateBench` (`test_bench_metrics.py`)

---

### §5.52 Bench Strip Sharpness CV Comparative Gate ✅ [Priority: Low — bench sharpness-CV parity]

**Problem**: The pipeline's Stage 11.41 `SharpnessCvGate` triggers SCANS when ASP's sharpness CV is too high. The benchmark lacks a corresponding comparative gate to detect when ASP's sharpness CV is disproportionately high vs SCANS.

**Implementation** *(shipped S181)*:
- `SharpnessCvGate` block in `bench_anime_stitch.py run_dataset()` after `LumaMadGate`; reuses `_strip_sharpness_cv` from `canvas.py`
- Dual condition: fires when `asp_scv > _SHARPNESS_CV_ABS_FLOOR` (default 0.60) AND (`sim_scv < 0.05` OR `asp_scv > 2.5 × max(sim_scv, 0.01)`)
- Module constants: `_SHARPNESS_CV_ABS_FLOOR`, `_SHARPNESS_CV_RATIO`; 2 schema entries in `config.py`
- `timings["render_gate_fallback"] += 8` when gate fires; 5 tests in `TestSharpnessCvGateBench` (`test_bench_metrics.py`)

---

### §5.53 Pipeline Strip Contrast CV Gate ✅ [Priority: Medium — mismatched-normalization detection]

**Problem**: Composite images from frames with mismatched contrast normalization (some strips high-contrast, others flat) indicate failed per-strip normalization. Existing gates don't catch this specific pattern.

**Implementation** *(shipped S182)*:
- `_strip_contrast_cv(img, n_strips=8)` in `canvas.py` — coefficient of variation (std/mean) of per-strip luma standard deviation; 0.0 when mean per-strip std < 1.0 (flat image guard) or degenerate input
...
### §5.54 Pipeline Seam Chroma Jump Gate ✅ [Priority: Medium — colour-step seam detection]

**Problem**: Poor inter-frame white-balance normalisation produces visible colour steps exactly at strip-seam boundaries. No existing gate targets per-boundary channel discontinuity directly.

**Implementation** *(shipped S182)*:
- `_seam_chroma_jump(img, n_strips=8, boundary_px=3)` in `canvas.py` — maximum per-channel mean absolute difference across ±`boundary_px` rows at each strip boundary; 0.0 for degenerate input
...
### §5.55 Bench Strip Contrast CV Comparative Gate ✅ [Priority: Low — bench contrast-CV parity]

**Implementation** *(shipped S182)*:
- `ContrastCvGate` block in `bench_anime_stitch.py run_dataset()` after `SharpnessCvGate`; reuses `_strip_contrast_cv` from `canvas.py`
...
- `timings["render_gate_fallback"] += 4` when gate fires; 5 tests in `TestContrastCvGateBench` (`test_bench_metrics.py`)

---

### §5.56 Bench Seam Chroma Jump Comparative Gate ✅ [Priority: Low — bench chroma-jump parity]

**Implementation** *(shipped S182)*:
- `ChromaJumpGate` block in `bench_anime_stitch.py run_dataset()` after `ContrastCvGate`; reuses `_seam_chroma_jump` from `canvas.py`
...
- `timings["render_gate_fallback"] += 2` when gate fires; 5 tests in `TestChromaJumpGateBench` (`test_bench_metrics.py`)

---

### §5.57 Pipeline Strip Noise CV Gate ✅ [Priority: Medium — mismatched encoding detection]

**Implementation** *(shipped S183)*:
- `_strip_noise_cv(img, n_strips=8)` in `canvas.py` — CV of per-strip HF noise estimate (strip vs Gaussian-blurred, σ=1.5); 0.0 for mean noise < 0.5 or degenerate
- Stage 11.44 gate: `_ncv_val > _NOISE_CV_GATE_FLOOR` (default 1.2) → SCANS fallback; reason `noise_cv_gate:{val:.4f}`
- Flags: `_NOISE_CV_GATE_ENABLED` (`ASP_GATE_NOISE_CV`), `_NOISE_CV_GATE_FLOOR` (`ASP_GATE_NOISE_CV_FLOOR`); `NOISE_CV_GATE_FLOOR = 1.2` in constants; 4 schema entries in `config.py`
- 5 tests in `TestStripNoiseCv` (`test_canvas.py`), 5 tests in `TestNoiseCvGatePipeline` (`test_pipeline.py`)

### §5.58 Pipeline Seam Luma Step CV Gate ✅ [Priority: Medium — uneven gain normalization detection]

**Implementation** *(shipped S183)*:
- `_seam_luma_step_cv(img, n_strips=8, boundary_px=3)` in `canvas.py` — CV of per-strip-boundary absolute luma step; 0.0 for mean step < 0.5, <2 steps, or degenerate
- Stage 11.45 gate: `_lscv_val > _LUMA_STEP_CV_GATE_FLOOR` (default 1.0) → SCANS fallback; reason `luma_step_cv_gate:{val:.4f}`
- Flags: `_LUMA_STEP_CV_GATE_ENABLED` (`ASP_GATE_LUMA_STEP_CV`), `_LUMA_STEP_CV_GATE_FLOOR` (`ASP_GATE_LUMA_STEP_CV_FLOOR`); `LUMA_STEP_CV_GATE_FLOOR = 1.0` in constants; 4 schema entries in `config.py`
- 5 tests in `TestSeamLumaStepCv` (`test_canvas.py`), 5 tests in `TestLumaStepCvGatePipeline` (`test_pipeline.py`)

### §5.59 Bench Strip Noise CV Comparative Gate ✅ [Priority: Low — bench noise-CV parity]

**Implementation** *(shipped S183)*:
- `NoiseCvGate` block in `bench_anime_stitch.py run_dataset()` after `ChromaJumpGate`
- Dual condition: `asp_ncv > 0.50` AND (`sim_ncv < 0.05` OR `asp_ncv > 2.5 × sim_ncv`) → SCANS fallback
- Constants: `_NOISE_CV_ABS_FLOOR = 0.50`, `_NOISE_CV_RATIO = 2.5`; schema entries `ASP_GATE_NOISE_CV_ABS_FLOOR/RATIO`
- 5 tests in `TestNoiseCvGateBench` (`test_bench_metrics.py`)

### §5.60 Bench Seam Luma Step CV Comparative Gate ✅ [Priority: Low — bench luma-step-CV parity]

**Implementation** *(shipped S183)*:
- `LumaStepCvGate` block in `bench_anime_stitch.py run_dataset()` after `NoiseCvGate`
- Dual condition: `asp_lscv > 0.40` AND (`sim_lscv < 0.05` OR `asp_lscv > 2.0 × sim_lscv`) → SCANS fallback
- Constants: `_LUMA_STEP_CV_ABS_FLOOR = 0.40`, `_LUMA_STEP_CV_RATIO = 2.0`; schema entries `ASP_GATE_LUMA_STEP_CV_ABS_FLOOR/RATIO`
- 5 tests in `TestLumaStepCvGateBench` (`test_bench_metrics.py`)

---

### §5.61 Pipeline Strip Entropy CV Gate ✅ [Priority: Medium — scene-complexity mismatch detection]

**Problem**: Strips from different frames can have very different information content — a complex background strip vs a flat sky or solid-colour cel region. High CV of per-strip Shannon entropy indicates composite segments from frames with mismatched scene complexity or strong background/foreground mismatch, which produces jarring visual discontinuities.

**Implementation** *(shipped S184)*:
- `_strip_entropy_cv(img, n_strips=8)` in `canvas.py` — CV of per-strip Shannon entropy (256-bin luma histogram, `-sum(p*log2(p+eps))`); 0.0 when mean_entropy < 0.5 (uniformly flat guard) or degenerate input; high CV = information-rich strips adjacent to flat/uniform strips
...
### §5.62 Pipeline Seam Chroma Step CV Gate ✅ [Priority: Medium — inconsistent white-balance detection]

**Problem**: Seam-stitching can apply white-balance normalization unevenly across composite zones, with some seams well-matched and others carrying large chroma steps. The §5.54 ChromaJumpGate detects the worst single seam; this gate detects inconsistency across all seams. High CV of per-seam chroma step (|ΔCb| + |ΔCr| in YCrCb) = unevenly corrected white-balance, which creates banding visible as alternating warm/cool strips.
...
### §5.63 Bench Strip Entropy CV Comparative Gate ✅ [Priority: Low — bench entropy-CV parity]

**Problem**: The pipeline's Stage 11.46 `EntropyCvGate` triggers SCANS when ASP's per-strip entropy CV is too high. The benchmark lacks a corresponding comparative gate to detect when ASP's entropy CV is disproportionately high vs SCANS.

**Implementation** *(shipped S184)*:
- `EntropyCvGate` block in `bench_anime_stitch.py run_dataset()` after `LumaStepCvGate`; reuses `_strip_entropy_cv` from `canvas.py`
- Dual condition: fires when `asp_ecv > _ENTROPY_CV_ABS_FLOOR` (default 0.30) AND (`sim_ecv < 0.05` OR `asp_ecv > 2.5 × max(sim_ecv, 0.01)`)
- Module constants: `_ENTROPY_CV_ABS_FLOOR`, `_ENTROPY_CV_RATIO`; 4 schema entries in `config.py`
- `timings["render_gate_fallback"] += 1` when gate fires; 5 tests in `TestEntropyCvGateBench` (`test_bench_metrics.py`)

---

### §5.64 Bench Seam Chroma Step CV Comparative Gate ✅ [Priority: Low — bench chroma-step-CV parity]

**Problem**: The pipeline's Stage 11.47 `ChromaStepCvGate` triggers SCANS when ASP's seam chroma step CV is too high. The benchmark lacks a corresponding comparative gate to detect when ASP's chroma step CV is disproportionately high vs SCANS.

**Implementation** *(shipped S184)*:
- `ChromaStepCvGate` block in `bench_anime_stitch.py run_dataset()` after `EntropyCvGate`; reuses `_seam_chroma_step_cv` from `canvas.py`
- Dual condition: fires when `asp_cscv > _CHROMA_STEP_CV_ABS_FLOOR` (default 0.30) AND (`sim_cscv < 0.05` OR `asp_cscv > 2.0 × max(sim_cscv, 0.01)`)
- Module constants: `_CHROMA_STEP_CV_ABS_FLOOR`, `_CHROMA_STEP_CV_RATIO`; 4 schema entries in `config.py`
- `timings["render_gate_fallback"] += 1` when gate fires; 5 tests in `TestChromaStepCvGateBench` (`test_bench_metrics.py`)

---

### §5.65 Pipeline Strip Chroma Energy CV Gate ✅ [Priority: Medium — colour-palette mismatch detection]

**Problem**: Strips from different animation segments can have wildly different colour saturation — a vivid cel-shaded character strip adjacent to a desaturated background or near-grayscale flashback scene. The §5.38 saturation CV gate uses HSV saturation; this gate uses YCrCb chroma magnitude (`sqrt((Cb-128)² + (Cr-128)²)`) which is more perceptually uniform, providing orthogonal coverage of the same class of defect.

**Implementation** *(shipped S185)*:
- `_strip_chroma_energy_cv(img, n_strips=8)` in `canvas.py` — CV of per-strip mean chroma magnitude in YCrCb space; 0.0 when mean < 1.0 (near-monochrome guard), grayscale input, or degenerate; high CV = some strips vivid, others desaturated
...
### §5.66 Pipeline Seam Gradient CV Gate ✅ [Priority: Medium — inconsistent blend-sharpness detection]

**Problem**: Seam blending can be inconsistent across zones — some seams are hard cuts (steep gradient, 1-pixel transition) while others are gradually feathered (gentle gradient, 6-pixel ramp). High CV of per-seam mean absolute row-to-row luma change within ±5 rows of each boundary detects this inconsistency. Orthogonal to §5.60 (luma step CV) which measures total step magnitude; this gate measures transition steepness independently of step size.

**Implementation** *(shipped S185)*:
- `_seam_gradient_cv(img, n_strips=8, band_px=5)` in `canvas.py` — CV of per-seam mean absolute row-to-row luma change in ±band_px rows; 0.0 when mean_grad < 0.1 (flat image guard), fewer than 2 seams, or degenerate; high CV = mix of hard cuts and feathered blends
...
### §5.67 Bench Strip Chroma Energy CV Comparative Gate ✅ [Priority: Low — bench chroma-energy-CV parity]

**Problem**: The pipeline's Stage 11.48 `ChromaEnergyCvGate` triggers SCANS when ASP's per-strip chroma energy CV is too high. The benchmark lacks a corresponding comparative gate.

**Implementation** *(shipped S185)*:
- `ChromaEnergyCvGate` block in `bench_anime_stitch.py run_dataset()` after `ChromaStepCvGate`; reuses `_strip_chroma_energy_cv` from `canvas.py`
- Dual condition: `asp_cecv > 0.30` AND (`sim_cecv < 0.05` OR `asp_cecv > 2.5 × sim_cecv`) → SCANS fallback
- Constants: `_CHROMA_ENERGY_CV_ABS_FLOOR = 0.30`, `_CHROMA_ENERGY_CV_RATIO = 2.5`; 4 schema entries in `config.py`
- `timings["render_gate_fallback"] += 1` when gate fires; 5 tests in `TestChromaEnergyCvGateBench` (`test_bench_metrics.py`)

---

### §5.68 Bench Seam Gradient CV Comparative Gate ✅ [Priority: Low — bench seam-gradient-CV parity]

**Problem**: The pipeline's Stage 11.49 `SeamGradientCvGate` triggers SCANS when ASP's seam gradient CV is too high. The benchmark lacks a corresponding comparative gate.

**Implementation** *(shipped S185)*:
- `SeamGradientCvGate` block in `bench_anime_stitch.py run_dataset()` after `ChromaEnergyCvGate`; reuses `_seam_gradient_cv` from `canvas.py`
- Dual condition: `asp_sgcv > 0.40` AND (`sim_sgcv < 0.05` OR `asp_sgcv > 2.0 × sim_sgcv`) → SCANS fallback
- Constants: `_SEAM_GRADIENT_CV_ABS_FLOOR = 0.40`, `_SEAM_GRADIENT_CV_RATIO = 2.0`; 4 schema entries in `config.py`
- `timings["render_gate_fallback"] += 1` when gate fires; 5 tests in `TestSeamGradientCvGateBench` (`test_bench_metrics.py`)

---

### §5.69 Pipeline Strip Luma IQR CV Gate ✅ [Priority: Medium — tonal spread inconsistency detection]

**Problem**: Strips from frames with very different scene content have different tonal spreads — a solid-colour action background has narrow IQR while a shaded indoor scene has wide IQR. The §5.45 luma range gate uses max-min of per-strip means; §5.49 luma MAD uses population dispersion of means. This gate uses IQR of within-strip pixel distributions (P75-P25 per strip), making it sensitive to within-strip content complexity rather than mean levels.

**Implementation** *(shipped S186)*:
- `_strip_luma_iqr_cv(img, n_strips=8)` in `canvas.py` — CV of per-strip luma IQR (P75-P25 of pixel intensities within each strip); 0.0 when mean IQR < 1.0 (flat guard) or degenerate; high CV = wide-IQR strips adjacent to flat strips
...
### §5.70 Pipeline Seam Column Variance CV Gate ✅ [Priority: Medium — horizontal step irregularity detection]

**Problem**: A well-blended seam has a horizontally uniform luma step (each column transitions at roughly the same level). A partial registration failure or diagonal alignment artifact creates a column-wise step profile with high variance (some columns have large steps, others are flat). The CV of per-seam column variance scores detects when some seams have such horizontal irregularity while others are smooth — a sign that multiple different failure modes are active simultaneously.

**Implementation** *(shipped S186)*:
- `_seam_column_variance_cv(img, n_strips=8, boundary_px=3)` in `canvas.py` — CV of per-seam variance of per-column absolute luma step (separate above/below row means per column); 0.0 when mean_var < 0.1 (flat guard), fewer than 2 seams, or degenerate; high CV = inconsistent horizontal regularity across seams
...
### §5.71 Bench Strip Luma IQR CV Comparative Gate ✅ [Priority: Low — bench luma-IQR-CV parity]

**Problem**: The pipeline's Stage 11.50 `LumaIqrCvGate` triggers SCANS when ASP's per-strip luma IQR CV is too high. The benchmark lacks a corresponding comparative gate.

**Implementation** *(shipped S186)*:
- `LumaIqrCvGate` block in `bench_anime_stitch.py run_dataset()` after `SeamGradientCvGate`; reuses `_strip_luma_iqr_cv` from `canvas.py`
- Dual condition: `asp_iqr > 0.40` AND (`sim_iqr < 0.05` OR `asp_iqr > 2.5 × sim_iqr`) → SCANS fallback
- Constants: `_LUMA_IQR_CV_ABS_FLOOR = 0.40`, `_LUMA_IQR_CV_RATIO = 2.5`; 4 schema entries in `config.py`
- `timings["render_gate_fallback"] += 1` when gate fires; 5 tests in `TestLumaIqrCvGateBench` (`test_bench_metrics.py`)

---

### §5.72 Bench Seam Column Variance CV Comparative Gate ✅ [Priority: Low — bench col-var-CV parity]

**Problem**: The pipeline's Stage 11.51 `SeamColVarCvGate` triggers SCANS when ASP's seam column variance CV is too high. The benchmark lacks a corresponding comparative gate.

**Implementation** *(shipped S186)*:
- `SeamColVarCvGate` block in `bench_anime_stitch.py run_dataset()` after `LumaIqrCvGate`; reuses `_seam_column_variance_cv` from `canvas.py`
- Dual condition: `asp_scvar > 0.40` AND (`sim_scvar < 0.05` OR `asp_scvar > 2.0 × sim_scvar`) → SCANS fallback
- Constants: `_SEAM_COL_VAR_CV_ABS_FLOOR = 0.40`, `_SEAM_COL_VAR_CV_RATIO = 2.0`; 4 schema entries in `config.py`
- `timings["render_gate_fallback"] += 1` when gate fires; 5 tests in `TestSeamColVarCvGateBench` (`test_bench_metrics.py`)

---

### §5.73 Pipeline Strip Luma Skewness CV Gate ✅ [Priority: Medium — tonal character inconsistency detection]

**Problem**: Anime strips can have fundamentally different tonal character — some dark with bright highlight tails (positive skewness), others bright with dark shadow tails (negative skewness). When the stitched canvas has strips with wildly varying skewness distributions, this signals tone-curve inconsistency or mismatched normalization that may not manifest as a mean-luma difference. CV of absolute per-strip skewness captures this orthogonally to §5.49 (MAD-CV, deviation magnitude) and §5.69 (IQR-CV, spread width).

**Implementation** *(shipped S187)*:
- `_strip_luma_skewness_cv(img, n_strips=8)` in `canvas.py` — CV of |skewness| per strip (3rd standardized moment of grayscale pixel distribution); 0.0 when mean_abs_skewness < 0.05 (flat/gradient images), std < 1.0 guard per strip, fewer than 2 strips, or degenerate input
- `_LUMA_SKEW_CV_GATE_ENABLED` (default ON) + `_LUMA_SKEW_CV_GATE_FLOOR=1.5` in `pipeline.py`; Stage 11.52; env: `ASP_GATE_LUMA_SKEW_CV`, `ASP_GATE_LUMA_SKEW_CV_FLOOR`
- Schema: `ASP_GATE_LUMA_SKEW_CV` (int 0/1), `ASP_GATE_LUMA_SKEW_CV_FLOOR` (float, 0–10); constant `LUMA_SKEW_CV_GATE_FLOOR=1.5` in `constants/animation.py`
- 5 tests in `TestStripLumaSkewnessCv` in `test_canvas.py`; 5 tests in `TestLumaSkewCvGatePipeline` in `test_pipeline.py`

### §5.74 Pipeline Seam Signed Step CV Gate ✅ [Priority: Medium — alternating normalization direction detection]

**Problem**: Gain normalization can produce alternating-direction brightness steps at seams (strip 0→1: brighter, strip 1→2: darker, strip 2→3: brighter). §5.58 (luma-step-CV) uses abs() on each step before computing CV, so uniform-magnitude alternating steps score near zero — they look "consistent" in magnitude even though the direction alternates. This gate uses `std(signed_steps) / mean(|signed_steps|)` which is high precisely when signs alternate.

**Implementation** *(shipped S187)*:
- `_seam_signed_step_cv(img, n_strips=8, boundary_px=3)` in `canvas.py` — `std(signed_steps) / mean(|signed_steps|)` where signed_step = mean_above − mean_below at each seam; 0.0 when mean_abs < 1.0 (flat images) or fewer than 2 seams
- `_SEAM_SIGNED_STEP_CV_GATE_ENABLED` (default ON) + `_SEAM_SIGNED_STEP_CV_GATE_FLOOR=1.2` in `pipeline.py`; Stage 11.53; env: `ASP_GATE_SEAM_SIGNED_STEP_CV`, `ASP_GATE_SEAM_SIGNED_STEP_CV_FLOOR`
- Schema: `ASP_GATE_SEAM_SIGNED_STEP_CV` (int 0/1), `ASP_GATE_SEAM_SIGNED_STEP_CV_FLOOR` (float, 0–10); constant `SEAM_SIGNED_STEP_CV_GATE_FLOOR=1.2` in `constants/animation.py`
- 5 tests in `TestSeamSignedStepCv` in `test_canvas.py`; 5 tests in `TestSeamSignedStepCvGatePipeline` in `test_pipeline.py`

### §5.75 Bench Strip Luma Skewness CV Comparative Gate ✅ [Priority: Low — bench skewness-CV parity]

**Problem**: §5.73 detects strip skewness inconsistency in the pipeline; benchmark needs a paired comparative gate to flag when ASP's skewness CV substantially exceeds simple_stitch's, indicating that ASP stitching introduced tonal inconsistency.

**Implementation** *(shipped S187)*:
- `_LUMA_SKEW_CV_ABS_FLOOR=0.50`, `_LUMA_SKEW_CV_RATIO=2.5` in `bench_anime_stitch.py`
- Comparative gate §5.75 in `run_dataset()`: fires when `asp_val > 0.50 AND (sim_val < 0.20 OR asp_val > 2.5 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_LUMA_SKEW_CV_ABS_FLOOR` (float), `ASP_BENCH_LUMA_SKEW_CV_RATIO` (float)
- 5 tests in `TestLumaSkewCvGateBench` in `test_bench_metrics.py`

### §5.76 Bench Seam Signed Step CV Comparative Gate ✅ [Priority: Low — bench signed-step-CV parity]

**Problem**: §5.74 detects alternating normalization artifacts in the pipeline; benchmark needs a paired comparative gate so ASP seam signed-step CV substantially worse than simple_stitch triggers SCANS fallback.

**Implementation** *(shipped S187)*:
- `_SEAM_SIGNED_STEP_CV_ABS_FLOOR=0.40`, `_SEAM_SIGNED_STEP_CV_RATIO=2.0` in `bench_anime_stitch.py`
- Comparative gate §5.76 in `run_dataset()`: fires when `asp_val > 0.40 AND (sim_val < 0.20 OR asp_val > 2.0 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_SEAM_SIGNED_STEP_CV_ABS_FLOOR` (float), `ASP_BENCH_SEAM_SIGNED_STEP_CV_RATIO` (float)
- 5 tests in `TestSeamSignedStepCvGateBench` in `test_bench_metrics.py`

---

### §5.77 Pipeline Strip Luma Kurtosis CV Gate ✅ [Priority: Medium — bimodal structure inconsistency detection]

**Problem**: Anime frames vary structurally — some frames consist of cel character against flat background (bimodal luma, high excess kurtosis), others are complex scene frames (near-Gaussian, low kurtosis). When the stitched canvas has strips with wildly varying kurtosis, it signals that the composition pulled from frames with different structural complexity — likely due to misaligned frame selection or failed fg/bg separation. This is orthogonal to §5.73 (skewness, 3rd moment asymmetry) and §5.69 (IQR-CV, spread width).

**Implementation** *(shipped S188)*:
- `_strip_luma_kurtosis_cv(img, n_strips=8)` in `canvas.py` — CV of |excess kurtosis| per strip (4th standardized moment − 3 of grayscale pixels); 0.0 when std < 1.0 per strip, mean_abs_kurtosis < 0.1, fewer than 2 strips, or degenerate input
- `_LUMA_KURTOSIS_CV_GATE_ENABLED` (default ON) + `_LUMA_KURTOSIS_CV_GATE_FLOOR=1.5` in `pipeline.py`; Stage 11.54; env: `ASP_GATE_LUMA_KURTOSIS_CV`, `ASP_GATE_LUMA_KURTOSIS_CV_FLOOR`
- Schema: `ASP_GATE_LUMA_KURTOSIS_CV` (bool), `ASP_GATE_LUMA_KURTOSIS_CV_FLOOR` (float, 0–10); constant `LUMA_KURTOSIS_CV_GATE_FLOOR=1.5` in `constants/animation.py`
- 5 tests in `TestStripLumaKurtosisCv` in `test_canvas.py`; 5 tests in `TestLumaKurtosisCvGatePipeline` in `test_pipeline.py`

### §5.78 Pipeline Seam Texture Ratio CV Gate ✅ [Priority: Medium — texture complexity mismatch detection]

**Problem**: When seams cross texture boundaries (e.g., detailed cel region above, flat sky below), the Laplacian variance ratio above/below the seam is large. In a well-stitched canvas, all seams should have similar texture ratios (either all matched or all different by similar degree). CV of log(above_var / below_var) across seams detects cases where some seams are well-matched in texture complexity but others cross hard texture boundaries — a sign of incorrect frame ordering or failed seam placement. Orthogonal to §5.66 (gradient CV, step magnitude) and §5.74 (signed step CV, direction).

**Implementation** *(shipped S188)*:
- `_seam_texture_ratio_cv(img, n_strips=8, band_px=5)` in `canvas.py` — CV of log(Laplacian-variance ratio) across seam boundaries (±band_px rows); 0.0 when mean_abs(log_ratio) < 0.05, fewer than 2 seams, or degenerate input; uses `cv2.Laplacian(band, cv2.CV_32F).var()` per band
- `_SEAM_TEXTURE_RATIO_CV_GATE_ENABLED` (default ON) + `_SEAM_TEXTURE_RATIO_CV_GATE_FLOOR=1.2` in `pipeline.py`; Stage 11.55; env: `ASP_GATE_SEAM_TEXTURE_RATIO_CV`, `ASP_GATE_SEAM_TEXTURE_RATIO_CV_FLOOR`
- Schema: `ASP_GATE_SEAM_TEXTURE_RATIO_CV` (bool), `ASP_GATE_SEAM_TEXTURE_RATIO_CV_FLOOR` (float, 0–10); constant `SEAM_TEXTURE_RATIO_CV_GATE_FLOOR=1.2` in `constants/animation.py`
- 5 tests in `TestSeamTextureRatioCv` in `test_canvas.py`; 5 tests in `TestSeamTextureRatioCvGatePipeline` in `test_pipeline.py`

### §5.79 Bench Strip Luma Kurtosis CV Comparative Gate ✅ [Priority: Low — bench kurtosis-CV parity]

**Problem**: §5.77 detects strip kurtosis inconsistency in the pipeline; benchmark needs a paired comparative gate to flag when ASP's kurtosis CV substantially exceeds simple_stitch's.

**Implementation** *(shipped S188)*:
- `_LUMA_KURTOSIS_CV_ABS_FLOOR=0.50`, `_LUMA_KURTOSIS_CV_RATIO=2.5` in `bench_anime_stitch.py`
- Comparative gate §5.79 in `run_dataset()`: fires when `asp_val > 0.50 AND (sim_val < 0.20 OR asp_val > 2.5 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_LUMA_KURTOSIS_CV_ABS_FLOOR` (float), `ASP_BENCH_LUMA_KURTOSIS_CV_RATIO` (float)
- 5 tests in `TestLumaKurtosisCvGateBench` in `test_bench_metrics.py`

### §5.80 Bench Seam Texture Ratio CV Comparative Gate ✅ [Priority: Low — bench texture-ratio-CV parity]

**Problem**: §5.78 detects texture mismatch at seams in the pipeline; benchmark needs a paired comparative gate.

**Implementation** *(shipped S188)*:
- `_SEAM_TEXTURE_RATIO_CV_ABS_FLOOR=0.40`, `_SEAM_TEXTURE_RATIO_CV_RATIO=2.0` in `bench_anime_stitch.py`
- Comparative gate §5.80 in `run_dataset()`: fires when `asp_val > 0.40 AND (sim_val < 0.20 OR asp_val > 2.0 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_SEAM_TEXTURE_RATIO_CV_ABS_FLOOR` (float), `ASP_BENCH_SEAM_TEXTURE_RATIO_CV_RATIO` (float)
- 5 tests in `TestSeamTextureRatioCvGateBench` in `test_bench_metrics.py`

---

### §5.81 Pipeline Strip Edge Density CV Gate ✅ [Priority: Medium — detail-level inconsistency detection]

**Problem**: Anime strips can vary wildly in edge content — a cel character strip has many edges, while a flat sky strip has almost none. When the stitched canvas has strips with wildly varying edge density, it signals that frames from different scene complexity levels were incorrectly interleaved or that bg/fg separation failed and introduced flat zones between detailed zones. Orthogonal to §5.50 (sharpness CV = Laplacian std/mean, measuring response amplitude) and §5.46 (seam edge density, a single seam-region value not a strip-wise CV).

**Implementation** *(shipped S189)*:
- `_strip_edge_density_cv(img, n_strips=8)` in `canvas.py` — CV of Canny(50,150) edge pixel fraction per strip; 0.0 when mean_density < 0.005 or fewer than 2 strips; uint8 conversion before Canny
- `_EDGE_DENSITY_CV_GATE_ENABLED` (default ON) + `_EDGE_DENSITY_CV_GATE_FLOOR=1.2` in `pipeline.py`; Stage 11.56; env: `ASP_GATE_EDGE_DENSITY_CV`, `ASP_GATE_EDGE_DENSITY_CV_FLOOR`
- Schema: `ASP_GATE_EDGE_DENSITY_CV` (bool), `ASP_GATE_EDGE_DENSITY_CV_FLOOR` (float, 0–10); constant `EDGE_DENSITY_CV_GATE_FLOOR=1.2` in `constants/animation.py`
- 5 tests in `TestStripEdgeDensityCv` in `test_canvas.py`; 5 tests in `TestEdgeDensityCvGatePipeline` in `test_pipeline.py`

### §5.82 Pipeline Seam Local Contrast CV Gate ✅ [Priority: Medium — seam placement complexity inconsistency]

**Problem**: Seams placed in high-contrast regions are harder to blend than seams in flat regions. When the canvas has some seams in rich-texture zones (high local pixel std) and others in flat zones (low local pixel std), the CV of per-seam local contrast is high — indicating inconsistent seam placement that may yield visible artifacts at some seams while others look clean. Orthogonal to §5.78 (texture ratio = above/below comparison of Laplacian var) and §5.66 (gradient CV = step steepness magnitude, not absolute contrast level).

**Implementation** *(shipped S189)*:
- `_seam_local_contrast_cv(img, n_strips=8, band_px=5)` in `canvas.py` — CV of pixel std in ±band_px seam bands; 0.0 when mean_contrast < 1.0 or fewer than 2 seams
- `_SEAM_LOCAL_CONTRAST_CV_GATE_ENABLED` (default ON) + `_SEAM_LOCAL_CONTRAST_CV_GATE_FLOOR=1.0` in `pipeline.py`; Stage 11.57; env: `ASP_GATE_SEAM_LOCAL_CONTRAST_CV`, `ASP_GATE_SEAM_LOCAL_CONTRAST_CV_FLOOR`
- Schema: `ASP_GATE_SEAM_LOCAL_CONTRAST_CV` (bool), `ASP_GATE_SEAM_LOCAL_CONTRAST_CV_FLOOR` (float, 0–10); constant `SEAM_LOCAL_CONTRAST_CV_GATE_FLOOR=1.0` in `constants/animation.py`
- 5 tests in `TestSeamLocalContrastCv` in `test_canvas.py`; 5 tests in `TestSeamLocalContrastCvGatePipeline` in `test_pipeline.py`

### §5.83 Bench Strip Edge Density CV Comparative Gate ✅ [Priority: Low — bench edge-density-CV parity]

**Problem**: §5.81 detects strip edge density inconsistency in the pipeline; benchmark needs a paired comparative gate to flag when ASP's edge density CV substantially exceeds simple_stitch's.

**Implementation** *(shipped S189)*:
- `_EDGE_DENSITY_CV_ABS_FLOOR=0.40`, `_EDGE_DENSITY_CV_RATIO=2.5` in `bench_anime_stitch.py`
- Comparative gate §5.83 in `run_dataset()`: fires when `asp_val > 0.40 AND (sim_val < 0.15 OR asp_val > 2.5 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_EDGE_DENSITY_CV_ABS_FLOOR` (float), `ASP_BENCH_EDGE_DENSITY_CV_RATIO` (float)
- 5 tests in `TestEdgeDensityCvGateBench` in `test_bench_metrics.py`

### §5.84 Bench Seam Local Contrast CV Comparative Gate ✅ [Priority: Low — bench local-contrast-CV parity]

**Problem**: §5.82 detects seam local contrast inconsistency in the pipeline; benchmark needs a paired comparative gate.

**Implementation** *(shipped S189)*:
- `_SEAM_LOCAL_CONTRAST_CV_ABS_FLOOR=0.30`, `_SEAM_LOCAL_CONTRAST_CV_RATIO=2.0` in `bench_anime_stitch.py`
- Comparative gate §5.84 in `run_dataset()`: fires when `asp_val > 0.30 AND (sim_val < 0.15 OR asp_val > 2.0 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_SEAM_LOCAL_CONTRAST_CV_ABS_FLOOR` (float), `ASP_BENCH_SEAM_LOCAL_CONTRAST_CV_RATIO` (float)
- 5 tests in `TestSeamLocalContrastCvGateBench` in `test_bench_metrics.py`

### §5.85 Pipeline Strip Luma P90–P10 CV Gate ✅ [Priority: Medium — outlier-robust tonal range inconsistency]

**Problem**: The per-strip P90−P10 luma spread is a robust tonal range that excludes the top and bottom 10% of pixels (noise and hard highlights/shadows). When CV of this spread is high, strips have wildly different outlier-robust tonal extents — indicating tone-curve inconsistency not captured by the full range (§5.45 = max−min, sensitive to outliers), IQR (§5.69 = P75−P25, narrower), or MAD (§5.49, median-centred deviation).

**Implementation** *(shipped S190)*:
- `_strip_luma_p90p10_cv(img, n_strips=8)` in `canvas.py` — CV of (P90−P10) per strip; 0.0 when mean_spread < 1.0 or fewer than 2 strips
- `_LUMA_P90P10_CV_GATE_ENABLED` (default ON) + `_LUMA_P90P10_CV_GATE_FLOOR=0.8` in `pipeline.py`; Stage 11.58; env: `ASP_GATE_LUMA_P90P10_CV`, `ASP_GATE_LUMA_P90P10_CV_FLOOR`
- Schema: `ASP_GATE_LUMA_P90P10_CV` (bool), `ASP_GATE_LUMA_P90P10_CV_FLOOR` (float, 0–10); constant `LUMA_P90P10_CV_GATE_FLOOR=0.8` in `constants/animation.py`
- 5 tests in `TestStripLumaP90P10Cv` in `test_canvas.py`; 5 tests in `TestLumaP90P10CvGatePipeline` in `test_pipeline.py`

### §5.86 Pipeline Seam Hue Shift CV Gate ✅ [Priority: Medium — cross-seam white-balance hue mismatch]

**Problem**: White-balance normalization applied inconsistently across seams can produce shifts on the color wheel (hue angle) that differ in magnitude from seam to seam. The existing §5.62 (seam chroma step CV) uses YCrCb Cb+Cr magnitude; this gate uses HSV hue angle (degrees [0,180]) which is orthogonal: a large chroma step in YCrCb can be saturation-driven without a hue shift, and vice versa.

**Implementation** *(shipped S190)*:
- `_seam_hue_shift_cv(img, n_strips=8, boundary_px=3)` in `canvas.py` — mean hue per seam band (OpenCV HSV hue [0,180)), wrapped difference to [0,90]; CV of absolute hue shifts; 0.0 for grayscale, mean_shift < 1.0, or fewer than 2 seams
- `_SEAM_HUE_SHIFT_CV_GATE_ENABLED` (default ON) + `_SEAM_HUE_SHIFT_CV_GATE_FLOOR=1.5` in `pipeline.py`; Stage 11.59; env: `ASP_GATE_SEAM_HUE_SHIFT_CV`, `ASP_GATE_SEAM_HUE_SHIFT_CV_FLOOR`
- Schema: `ASP_GATE_SEAM_HUE_SHIFT_CV` (bool), `ASP_GATE_SEAM_HUE_SHIFT_CV_FLOOR` (float, 0–10); constant `SEAM_HUE_SHIFT_CV_GATE_FLOOR=1.5` in `constants/animation.py`
- 5 tests in `TestSeamHueShiftCv` in `test_canvas.py`; 5 tests in `TestSeamHueShiftCvGatePipeline` in `test_pipeline.py`

### §5.87 Bench Strip Luma P90–P10 CV Comparative Gate ✅ [Priority: Low — bench P90–P10-CV parity]

**Problem**: §5.85 detects strip P90–P10 inconsistency in the pipeline; benchmark needs a paired comparative gate.

**Implementation** *(shipped S190)*:
- `_LUMA_P90P10_CV_ABS_FLOOR=0.30`, `_LUMA_P90P10_CV_RATIO=2.5` in `bench_anime_stitch.py`
- Comparative gate §5.87 in `run_dataset()`: fires when `asp_val > 0.30 AND (sim_val < 0.10 OR asp_val > 2.5 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_LUMA_P90P10_CV_ABS_FLOOR` (float), `ASP_BENCH_LUMA_P90P10_CV_RATIO` (float)
- 5 tests in `TestLumaP90P10CvGateBench` in `test_bench_metrics.py`

### §5.88 Bench Seam Hue Shift CV Comparative Gate ✅ [Priority: Low — bench hue-shift-CV parity]

**Problem**: §5.86 detects seam hue shift inconsistency in the pipeline; benchmark needs a paired comparative gate.

**Implementation** *(shipped S190)*:
- `_SEAM_HUE_SHIFT_CV_ABS_FLOOR=0.40`, `_SEAM_HUE_SHIFT_CV_RATIO=2.0` in `bench_anime_stitch.py`
- Comparative gate §5.88 in `run_dataset()`: fires when `asp_val > 0.40 AND (sim_val < 0.15 OR asp_val > 2.0 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_SEAM_HUE_SHIFT_CV_ABS_FLOOR` (float), `ASP_BENCH_SEAM_HUE_SHIFT_CV_RATIO` (float)
- 5 tests in `TestSeamHueShiftCvGateBench` in `test_bench_metrics.py`

### §5.89 Pipeline Strip Dark Pixel Fraction CV Gate ✅ [Priority: Medium — tonal polarity inconsistency detection]

**Problem**: Anime frames have dark outlines and backgrounds; some strips may be predominantly dark (dark scene, silhouette zone) while others are bright (sky, interior with lighting). When CV of the per-strip dark pixel fraction (luma < 64) is high, the canvas has alternating mostly-dark and mostly-bright strips — indicating incorrect frame interleaving or failed tonal normalization. Orthogonal to §5.85 (P90−P10 spread, outlier-robust range not threshold), §5.73 (skewness, asymmetry not fraction), §5.45 (full range extent not polarity).

**Implementation**:
- `_strip_dark_pixel_fraction_cv(img, n_strips=8, threshold=64)` in `canvas.py` — CV of dark pixel fraction per strip; 0.0 when mean_fraction < 0.005 or > 0.995 (uniformly all-bright or all-dark)
- `_DARK_PIXEL_FRAC_CV_GATE_ENABLED` (default ON) + `_DARK_PIXEL_FRAC_CV_GATE_FLOOR=1.5` in `pipeline.py`; Stage 11.60; env: `ASP_GATE_DARK_PIXEL_FRAC_CV`, `ASP_GATE_DARK_PIXEL_FRAC_CV_FLOOR`
- Schema: `ASP_GATE_DARK_PIXEL_FRAC_CV` (bool), `ASP_GATE_DARK_PIXEL_FRAC_CV_FLOOR` (float, 0–10); constant `DARK_PIXEL_FRAC_CV_GATE_FLOOR=1.5` in `constants/animation.py`
- 5 tests in `TestStripDarkPixelFractionCv` in `test_canvas.py`; 5 tests in `TestDarkPixelFracCvGatePipeline` in `test_pipeline.py`

### §5.90 Pipeline Seam Saturation Shift CV Gate ✅ [Priority: Medium — cross-seam colorfulness mismatch]

**Problem**: When seams cross saturation boundaries (vivid cel region above, desaturated background below), the absolute mean saturation shift is large. CV of these per-seam absolute saturation shifts detects inconsistent colorfulness matching across seams — some seams in well-matched saturation zones, others crossing vivid/pastel boundaries. Orthogonal to §5.86 (hue angle shift, different HSV channel), §5.62 (YCrCb Cb+Cr magnitude, not channel-separated), and §5.38 (within-strip saturation spread, not cross-seam).

**Implementation**:
- `_seam_saturation_shift_cv(img, n_strips=8, boundary_px=3)` in `canvas.py` — CV of |mean_sat_above − mean_sat_below| per seam (HSV S channel, [0,255]); 0.0 for grayscale or mean_shift < 1.0
- `_SEAM_SAT_SHIFT_CV_GATE_ENABLED` (default ON) + `_SEAM_SAT_SHIFT_CV_GATE_FLOOR=1.5` in `pipeline.py`; Stage 11.61; env: `ASP_GATE_SEAM_SAT_SHIFT_CV`, `ASP_GATE_SEAM_SAT_SHIFT_CV_FLOOR`
- Schema: `ASP_GATE_SEAM_SAT_SHIFT_CV` (bool), `ASP_GATE_SEAM_SAT_SHIFT_CV_FLOOR` (float, 0–10); constant `SEAM_SAT_SHIFT_CV_GATE_FLOOR=1.5` in `constants/animation.py`
- 5 tests in `TestSeamSaturationShiftCv` in `test_canvas.py`; 5 tests in `TestSeamSatShiftCvGatePipeline` in `test_pipeline.py`

### §5.91 Bench Strip Dark Pixel Fraction CV Comparative Gate ✅ [Priority: Low — bench dark-fraction-CV parity]

**Problem**: §5.89 detects strip dark pixel fraction inconsistency in the pipeline; benchmark needs a paired comparative gate.

**Implementation**:
- `_DARK_PIXEL_FRAC_CV_ABS_FLOOR=0.40`, `_DARK_PIXEL_FRAC_CV_RATIO=2.5` in `bench_anime_stitch.py`
- Comparative gate §5.91 in `run_dataset()`: fires when `asp_val > 0.40 AND (sim_val < 0.15 OR asp_val > 2.5 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_DARK_PIXEL_FRAC_CV_ABS_FLOOR` (float), `ASP_BENCH_DARK_PIXEL_FRAC_CV_RATIO` (float)
- 5 tests in `TestDarkPixelFracCvGateBench` in `test_bench_metrics.py`

### §5.92 Bench Seam Saturation Shift CV Comparative Gate ✅ [Priority: Low — bench sat-shift-CV parity]

**Problem**: §5.90 detects seam saturation mismatch in the pipeline; benchmark needs a paired comparative gate.

**Implementation**:
- `_SEAM_SAT_SHIFT_CV_ABS_FLOOR=0.30`, `_SEAM_SAT_SHIFT_CV_RATIO=2.0` in `bench_anime_stitch.py`
- Comparative gate §5.92 in `run_dataset()`: fires when `asp_val > 0.30 AND (sim_val < 0.10 OR asp_val > 2.0 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_SEAM_SAT_SHIFT_CV_ABS_FLOOR` (float), `ASP_BENCH_SEAM_SAT_SHIFT_CV_RATIO` (float)
- 5 tests in `TestSeamSatShiftCvGateBench` in `test_bench_metrics.py`

---

### §5.93 Pipeline Strip Sobel Energy CV Gate ✅ [Priority: Medium — gradient activity inconsistency detection]

**Problem**: Different strips in the stitched canvas may have very different levels of directional gradient activity — strips with dense line art have high Sobel energy while flat background strips have near-zero energy. High CV of mean Sobel energy across strips indicates mismatched detail levels that may reveal incorrect frame selection or bg/fg separation failures. Orthogonal to §5.50 (sharpness CV = Laplacian std/mean, different operator and normalisation), §5.81 (Canny edge density = binary threshold not continuous energy), §5.66 (seam gradient CV = step magnitude at boundaries, not full-strip average).

**Implementation**:
- `_strip_sobel_energy_cv(img, n_strips=8)` in `canvas.py` — CV of mean sqrt(Gx²+Gy²) per strip (Sobel ksize=3); 0.0 when mean_energy < 0.5
- `_SOBEL_ENERGY_CV_GATE_ENABLED` (default ON) + `_SOBEL_ENERGY_CV_GATE_FLOOR=1.2` in `pipeline.py`; Stage 11.62; env: `ASP_GATE_SOBEL_ENERGY_CV`, `ASP_GATE_SOBEL_ENERGY_CV_FLOOR`
- Schema: `ASP_GATE_SOBEL_ENERGY_CV` (bool), `ASP_GATE_SOBEL_ENERGY_CV_FLOOR` (float, 0–10); constant `SOBEL_ENERGY_CV_GATE_FLOOR=1.2` in `constants/animation.py`
- 5 tests in `TestStripSobelEnergyCv` in `test_canvas.py`; 5 tests in `TestSobelEnergyCvGatePipeline` in `test_pipeline.py`

### §5.94 Pipeline Seam Value Shift CV Gate ✅ [Priority: Medium — max-channel brightness mismatch at seams]

**Problem**: The HSV Value channel (V = max(R,G,B)) captures brightness as the dominant channel — distinct from luma (weighted average) and from saturation/hue. A seam between a vivid yellow region (high Value despite no luma mismatch due to green+red balance) and a dark region will show a large Value shift. CV of per-seam |V_above − V_below| detects inconsistent max-channel brightness matching across seams. Orthogonal to §5.86 (hue), §5.90 (saturation), and §5.58/§5.60 (luma = 0.114B+0.587G+0.299R, weighted average not max).

**Implementation**:
- `_seam_value_shift_cv(img, n_strips=8, boundary_px=3)` in `canvas.py` — CV of |mean_V_above − mean_V_below| per seam (HSV V [0,255]); 0.0 for grayscale or mean_shift < 1.0
- `_SEAM_VALUE_SHIFT_CV_GATE_ENABLED` (default ON) + `_SEAM_VALUE_SHIFT_CV_GATE_FLOOR=1.2` in `pipeline.py`; Stage 11.63; env: `ASP_GATE_SEAM_VALUE_SHIFT_CV`, `ASP_GATE_SEAM_VALUE_SHIFT_CV_FLOOR`
- Schema: `ASP_GATE_SEAM_VALUE_SHIFT_CV` (bool), `ASP_GATE_SEAM_VALUE_SHIFT_CV_FLOOR` (float, 0–10); constant `SEAM_VALUE_SHIFT_CV_GATE_FLOOR=1.2` in `constants/animation.py`
- 5 tests in `TestSeamValueShiftCv` in `test_canvas.py`; 5 tests in `TestSeamValueShiftCvGatePipeline` in `test_pipeline.py`

### §5.95 Bench Strip Sobel Energy CV Comparative Gate ✅ [Priority: Low — bench Sobel-energy-CV parity]

**Problem**: §5.93 detects strip Sobel energy inconsistency in the pipeline; benchmark needs a paired comparative gate.

**Implementation**:
- `_SOBEL_ENERGY_CV_ABS_FLOOR=0.35`, `_SOBEL_ENERGY_CV_RATIO=2.5` in `bench_anime_stitch.py`
- Comparative gate §5.95 in `run_dataset()`: fires when `asp_val > 0.35 AND (sim_val < 0.12 OR asp_val > 2.5 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_SOBEL_ENERGY_CV_ABS_FLOOR` (float), `ASP_BENCH_SOBEL_ENERGY_CV_RATIO` (float)
- 5 tests in `TestSobelEnergyCvGateBench` in `test_bench_metrics.py`

### §5.96 Bench Seam Value Shift CV Comparative Gate ✅ [Priority: Low — bench value-shift-CV parity]

**Problem**: §5.94 detects seam Value mismatch in the pipeline; benchmark needs a paired comparative gate.

**Implementation**:
- `_SEAM_VALUE_SHIFT_CV_ABS_FLOOR=0.30`, `_SEAM_VALUE_SHIFT_CV_RATIO=2.0` in `bench_anime_stitch.py`
- Comparative gate §5.96 in `run_dataset()`: fires when `asp_val > 0.30 AND (sim_val < 0.10 OR asp_val > 2.0 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_SEAM_VALUE_SHIFT_CV_ABS_FLOOR` (float), `ASP_BENCH_SEAM_VALUE_SHIFT_CV_RATIO` (float)
- 5 tests in `TestSeamValueShiftCvGateBench` in `test_bench_metrics.py`

### §5.97 Pipeline Strip Median Luma CV Gate ✅ [Priority: Medium — strip brightness location inconsistency]

**Problem**: The median (P50) of each strip captures its typical brightness — robust to outlier pixels from noise, compression artifacts, or isolated highlights. When CV of per-strip medians is high, strips have different typical brightness levels (some typically bright, others typically dark), indicating tonal normalization failed to equalize median brightness across strips. Orthogonal to: §5.45 (full range = max−min, captures extent not center), §5.85 (P90−P10 spread, robust range not location), §5.69 (IQR = P75−P25, spread not center), §5.49 (MAD = deviation from median, not the median value), §5.89 (dark pixel fraction = threshold count not median).

**Implementation**:
- `_strip_median_luma_cv(img, n_strips=8)` in `canvas.py` — CV of np.median() per strip; 0.0 when mean_median < 1.0
- `_MEDIAN_LUMA_CV_GATE_ENABLED` (default ON) + `_MEDIAN_LUMA_CV_GATE_FLOOR=0.5` in `pipeline.py`; Stage 11.64; env: `ASP_GATE_MEDIAN_LUMA_CV`, `ASP_GATE_MEDIAN_LUMA_CV_FLOOR`
- Schema: `ASP_GATE_MEDIAN_LUMA_CV` (bool), `ASP_GATE_MEDIAN_LUMA_CV_FLOOR` (float, 0–10); constant `MEDIAN_LUMA_CV_GATE_FLOOR=0.5` in `constants/animation.py`
- 5 tests in `TestStripMedianLumaCv` in `test_canvas.py`; 5 tests in `TestMedianLumaCvGatePipeline` in `test_pipeline.py`

### §5.98 Pipeline Seam Entropy Shift CV Gate ✅ [Priority: Medium — cross-seam information content mismatch]

**Problem**: Seams can cross information-content boundaries — complex detailed content on one side and flat uniform content on the other produces a large Shannon entropy difference between above and below the seam. When some seams have large entropy shifts and others have small ones, CV of these absolute entropy shifts is high — indicating inconsistent content complexity matching. Orthogonal to §5.61 (strip entropy CV = variation of entropy across strips, not comparison above/below each seam), §5.82 (seam local contrast = pixel std in seam band, not histogram entropy), §5.78 (Laplacian variance ratio, frequency response not histogram).

**Implementation**:
- `_seam_entropy_shift_cv(img, n_strips=8, boundary_px=3)` in `canvas.py` — Shannon entropy (base-2) of 256-bin histogram per seam band; CV of |H_above − H_below|; 0.0 when mean_shift < 0.05
- `_SEAM_ENTROPY_SHIFT_CV_GATE_ENABLED` (default ON) + `_SEAM_ENTROPY_SHIFT_CV_GATE_FLOOR=1.5` in `pipeline.py`; Stage 11.65; env: `ASP_GATE_SEAM_ENTROPY_SHIFT_CV`, `ASP_GATE_SEAM_ENTROPY_SHIFT_CV_FLOOR`
- Schema: `ASP_GATE_SEAM_ENTROPY_SHIFT_CV` (bool), `ASP_GATE_SEAM_ENTROPY_SHIFT_CV_FLOOR` (float, 0–10); constant `SEAM_ENTROPY_SHIFT_CV_GATE_FLOOR=1.5` in `constants/animation.py`
- 5 tests in `TestSeamEntropyShiftCv` in `test_canvas.py`; 5 tests in `TestSeamEntropyShiftCvGatePipeline` in `test_pipeline.py`

### §5.99 Bench Strip Median Luma CV Comparative Gate ✅ [Priority: Low — bench median-luma-CV parity]

**Problem**: §5.97 detects strip median brightness inconsistency in the pipeline; benchmark needs a paired comparative gate.

**Implementation**:
- `_MEDIAN_LUMA_CV_ABS_FLOOR=0.20`, `_MEDIAN_LUMA_CV_RATIO=3.0` in `bench_anime_stitch.py`
- Comparative gate §5.99 in `run_dataset()`: fires when `asp_val > 0.20 AND (sim_val < 0.08 OR asp_val > 3.0 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_MEDIAN_LUMA_CV_ABS_FLOOR` (float), `ASP_BENCH_MEDIAN_LUMA_CV_RATIO` (float)
- 5 tests in `TestMedianLumaCvGateBench` in `test_bench_metrics.py`

### §5.100 Bench Seam Entropy Shift CV Comparative Gate ✅ [Priority: Low — bench entropy-shift-CV parity]

**Problem**: §5.98 detects seam information-content mismatch in the pipeline; benchmark needs a paired comparative gate.

**Implementation**:
- `_SEAM_ENTROPY_SHIFT_CV_ABS_FLOOR=0.30`, `_SEAM_ENTROPY_SHIFT_CV_RATIO=2.0` in `bench_anime_stitch.py`
- Comparative gate §5.100 in `run_dataset()`: fires when `asp_val > 0.30 AND (sim_val < 0.10 OR asp_val > 2.0 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_SEAM_ENTROPY_SHIFT_CV_ABS_FLOOR` (float), `ASP_BENCH_SEAM_ENTROPY_SHIFT_CV_RATIO` (float)
- 5 tests in `TestSeamEntropyShiftCvGateBench` in `test_bench_metrics.py`

---

### §5.101 Pipeline Strip Red Channel CV Gate ✅ [Priority: Medium — red-axis per-strip color inconsistency]

**Problem**: Color normalization can fail along individual color axes. The Red channel (BGR channel 2) is particularly important for anime where red is used for blood, fire, character accents, and warm lighting. When some strips have high mean Red and others have low mean Red, this indicates R-axis normalization failure invisible to luma (weighted combination where B and G can compensate) and hue (which only captures angle not axis magnitude). Orthogonal to §5.94 (Value = max channel, not R specifically), §5.86 (hue angle), §5.90 (saturation purity), §5.97 (median luma, grayscale).

**Implementation**:
- `_strip_red_channel_cv(img, n_strips=8)` in `canvas.py` — CV of mean Red (BGR channel 2) per strip; 0.0 for grayscale or mean_red < 1.0
- `_RED_CHANNEL_CV_GATE_ENABLED` (default ON) + `_RED_CHANNEL_CV_GATE_FLOOR=0.6` in `pipeline.py`; Stage 11.66; env: `ASP_GATE_RED_CHANNEL_CV`, `ASP_GATE_RED_CHANNEL_CV_FLOOR`
- Schema: `ASP_GATE_RED_CHANNEL_CV` (bool), `ASP_GATE_RED_CHANNEL_CV_FLOOR` (float, 0–10); constant `RED_CHANNEL_CV_GATE_FLOOR=0.6` in `constants/animation.py`
- 5 tests in `TestStripRedChannelCv` in `test_canvas.py`; 5 tests in `TestRedChannelCvGatePipeline` in `test_pipeline.py`

### §5.102 Pipeline Seam Blue Shift CV Gate ✅ [Priority: Medium — blue-axis seam normalization artifact detection]

**Problem**: Blue-channel normalization artifacts are common at seams when anime frames alternate between outdoor sky/water scenes (high Blue) and warm indoor scenes (low Blue). When the absolute Blue channel shift at seams varies widely (high CV), some seams are well-matched in the blue axis while others have large shifts — indicating inconsistent blue-channel correction. Orthogonal to luma step (§5.58/§5.60, weighted average), Value shift (§5.94, max channel), hue (§5.86, angle), saturation (§5.90, purity), seam entropy shift (§5.98).

**Implementation**:
- `_seam_blue_shift_cv(img, n_strips=8, boundary_px=3)` in `canvas.py` — CV of |mean_B_above − mean_B_below| per seam (BGR channel 0); 0.0 for grayscale or mean_shift < 1.0
- `_SEAM_BLUE_SHIFT_CV_GATE_ENABLED` (default ON) + `_SEAM_BLUE_SHIFT_CV_GATE_FLOOR=1.2` in `pipeline.py`; Stage 11.67; env: `ASP_GATE_SEAM_BLUE_SHIFT_CV`, `ASP_GATE_SEAM_BLUE_SHIFT_CV_FLOOR`
- Schema: `ASP_GATE_SEAM_BLUE_SHIFT_CV` (bool), `ASP_GATE_SEAM_BLUE_SHIFT_CV_FLOOR` (float, 0–10); constant `SEAM_BLUE_SHIFT_CV_GATE_FLOOR=1.2` in `constants/animation.py`
- 5 tests in `TestSeamBlueShiftCv` in `test_canvas.py`; 5 tests in `TestSeamBlueShiftCvGatePipeline` in `test_pipeline.py`

### §5.103 Bench Strip Red Channel CV Comparative Gate ✅ [Priority: Low — bench red-channel-CV parity]

**Problem**: §5.101 detects strip red-channel inconsistency in the pipeline; benchmark needs a paired comparative gate.

**Implementation**:
- `_RED_CHANNEL_CV_ABS_FLOOR=0.20`, `_RED_CHANNEL_CV_RATIO=3.0` in `bench_anime_stitch.py`
- Comparative gate §5.103 in `run_dataset()`: fires when `asp_val > 0.20 AND (sim_val < 0.07 OR asp_val > 3.0 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_RED_CHANNEL_CV_ABS_FLOOR` (float), `ASP_BENCH_RED_CHANNEL_CV_RATIO` (float)
- 5 tests in `TestRedChannelCvGateBench` in `test_bench_metrics.py`

### §5.104 Bench Seam Blue Shift CV Comparative Gate ✅ [Priority: Low — bench blue-shift-CV parity]

**Problem**: §5.102 detects blue-axis seam normalization artifacts in the pipeline; benchmark needs a paired comparative gate.

**Implementation**:
- `_SEAM_BLUE_SHIFT_CV_ABS_FLOOR=0.30`, `_SEAM_BLUE_SHIFT_CV_RATIO=2.0` in `bench_anime_stitch.py`
- Comparative gate §5.104 in `run_dataset()`: fires when `asp_val > 0.30 AND (sim_val < 0.10 OR asp_val > 2.0 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_SEAM_BLUE_SHIFT_CV_ABS_FLOOR` (float), `ASP_BENCH_SEAM_BLUE_SHIFT_CV_RATIO` (float)
- 5 tests in `TestSeamBlueShiftCvGateBench` in `test_bench_metrics.py`

---

### §5.105 Pipeline Strip Green Channel CV Gate ✅ [Priority: Medium — green-axis per-strip color inconsistency]

**Problem**: Green channel is the dominant perceptual luminance contributor (≈0.587G), so G-axis normalization failures are common yet invisible to: §5.97 (median luma — grayscale, blends all channels), §5.101 (Red channel — different channel), §5.86 (hue angle — not raw axis magnitude), §5.90 (saturation — purity not per-channel value). High Green CV with low Red/Blue CV pinpoints G-axis-only normalization failure.

**Implementation**:
- `_strip_green_channel_cv(img, n_strips=8)` in `canvas.py` — CV of mean Green (BGR channel 1) per strip; 0.0 for grayscale or mean_green < 1.0
- `_GREEN_CHANNEL_CV_GATE_ENABLED` (default ON) + `_GREEN_CHANNEL_CV_GATE_FLOOR=0.5` in `pipeline.py`; Stage 11.68; env: `ASP_GATE_GREEN_CHANNEL_CV`, `ASP_GATE_GREEN_CHANNEL_CV_FLOOR`
- Schema: `ASP_GATE_GREEN_CHANNEL_CV` (bool), `ASP_GATE_GREEN_CHANNEL_CV_FLOOR` (float, 0–10); constant `GREEN_CHANNEL_CV_GATE_FLOOR=0.5` in `constants/animation.py`
- 5 tests in `TestStripGreenChannelCv` in `test_canvas.py`; 5 tests in `TestGreenChannelCvGatePipeline` in `test_pipeline.py`

### §5.106 Pipeline Seam Red Shift CV Gate ✅ [Priority: Medium — red-axis seam normalization artifact detection]

**Problem**: Red-axis normalization artifacts are common at seams in anime when frames alternate between warm-lit scenes (high Red) and cool-lit or night scenes (low Red). When the absolute Red shift at seams varies widely, some seams are well-matched while others have large R-axis steps — indicating inconsistent red-channel correction. Orthogonal to: §5.58/§5.60 (luma step — grayscale weighted average), §5.102 (seam blue shift — different channel), §5.86 (hue), §5.90 (saturation), §5.94 (Value = max of all channels).

**Implementation**:
- `_seam_red_shift_cv(img, n_strips=8, boundary_px=3)` in `canvas.py` — CV of |mean_R_above − mean_R_below| per seam (BGR channel 2); 0.0 for grayscale or mean_shift < 1.0
- `_SEAM_RED_SHIFT_CV_GATE_ENABLED` (default ON) + `_SEAM_RED_SHIFT_CV_GATE_FLOOR=1.2` in `pipeline.py`; Stage 11.69; env: `ASP_GATE_SEAM_RED_SHIFT_CV`, `ASP_GATE_SEAM_RED_SHIFT_CV_FLOOR`
- Schema: `ASP_GATE_SEAM_RED_SHIFT_CV` (bool), `ASP_GATE_SEAM_RED_SHIFT_CV_FLOOR` (float, 0–10); constant `SEAM_RED_SHIFT_CV_GATE_FLOOR=1.2` in `constants/animation.py`
- 5 tests in `TestSeamRedShiftCv` in `test_canvas.py`; 5 tests in `TestSeamRedShiftCvGatePipeline` in `test_pipeline.py`

### §5.107 Bench Strip Green Channel CV Comparative Gate ✅ [Priority: Low — bench green-channel-CV parity]

**Problem**: §5.105 detects strip green-channel inconsistency in the pipeline; benchmark needs a paired comparative gate.

**Implementation**:
- `_GREEN_CHANNEL_CV_ABS_FLOOR=0.20`, `_GREEN_CHANNEL_CV_RATIO=3.0` in `bench_anime_stitch.py`
- Comparative gate §5.107 in `run_dataset()`: fires when `asp_val > 0.20 AND (sim_val < 0.07 OR asp_val > 3.0 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_GREEN_CHANNEL_CV_ABS_FLOOR` (float), `ASP_BENCH_GREEN_CHANNEL_CV_RATIO` (float)
- 5 tests in `TestGreenChannelCvGateBench` in `test_bench_metrics.py`

### §5.108 Bench Seam Red Shift CV Comparative Gate ✅ [Priority: Low — bench red-shift-CV parity]

**Problem**: §5.106 detects red-axis seam normalization artifacts in the pipeline; benchmark needs a paired comparative gate.

**Implementation**:
- `_SEAM_RED_SHIFT_CV_ABS_FLOOR=0.30`, `_SEAM_RED_SHIFT_CV_RATIO=2.0` in `bench_anime_stitch.py`
- Comparative gate §5.108 in `run_dataset()`: fires when `asp_val > 0.30 AND (sim_val < 0.10 OR asp_val > 2.0 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_SEAM_RED_SHIFT_CV_ABS_FLOOR` (float), `ASP_BENCH_SEAM_RED_SHIFT_CV_RATIO` (float)
- 5 tests in `TestSeamRedShiftCvGateBench` in `test_bench_metrics.py`

---

### §5.109 Pipeline Strip Blue Channel CV Gate ✅ [Priority: Medium — blue-axis per-strip color inconsistency]

**Problem**: Blue channel is the dominant perceptual differentiator between warm and cool scenes in anime (sky, water, night scenes vs. indoor warm lighting). Completing the R/G/B per-strip channel CV trilogy: §5.101 (Red), §5.105 (Green), §5.109 (Blue). Orthogonal to all three: §5.97 (median luma — grayscale), §5.86 (hue angle — not raw axis magnitude), §5.90 (saturation — purity), §5.102 (seam blue shift — seam-boundary metric, not strip-mean). Per-strip Blue CV detects frames where B-axis correction was applied at different strength across strips.

**Implementation**:
- `_strip_blue_channel_cv(img, n_strips=8)` in `canvas.py` — CV of mean Blue (BGR channel 0) per strip; 0.0 for grayscale or mean_blue < 1.0
- `_BLUE_CHANNEL_CV_GATE_ENABLED` (default ON) + `_BLUE_CHANNEL_CV_GATE_FLOOR=0.6` in `pipeline.py`; Stage 11.70; env: `ASP_GATE_BLUE_CHANNEL_CV`, `ASP_GATE_BLUE_CHANNEL_CV_FLOOR`
- Schema: `ASP_GATE_BLUE_CHANNEL_CV` (bool), `ASP_GATE_BLUE_CHANNEL_CV_FLOOR` (float, 0–10); constant `BLUE_CHANNEL_CV_GATE_FLOOR=0.6` in `constants/animation.py`
- 5 tests in `TestStripBlueChannelCv` in `test_canvas.py`; 5 tests in `TestBlueChannelCvGatePipeline` in `test_pipeline.py`

### §5.110 Pipeline Seam Green Shift CV Gate ✅ [Priority: Medium — green-axis seam normalization artifact detection]

**Problem**: Completing the R/G/B per-seam channel shift CV trilogy: §5.102 (Blue), §5.106 (Red), §5.110 (Green). Green seam shift CV differs from luma (§5.58/§5.60, weighted channel blend) because G-axis artifacts visible at seams with high G-vs-G mismatch may be masked when B/R compensate in the luma formula. Orthogonal to Value shift (§5.94, max channel), hue (§5.86, angle), saturation (§5.90, purity), and seam blue (§5.102) / red shift (§5.106, different channels).

**Implementation**:
- `_seam_green_shift_cv(img, n_strips=8, boundary_px=3)` in `canvas.py` — CV of |mean_G_above − mean_G_below| per seam (BGR channel 1); 0.0 for grayscale or mean_shift < 1.0
- `_SEAM_GREEN_SHIFT_CV_GATE_ENABLED` (default ON) + `_SEAM_GREEN_SHIFT_CV_GATE_FLOOR=1.2` in `pipeline.py`; Stage 11.71; env: `ASP_GATE_SEAM_GREEN_SHIFT_CV`, `ASP_GATE_SEAM_GREEN_SHIFT_CV_FLOOR`
- Schema: `ASP_GATE_SEAM_GREEN_SHIFT_CV` (bool), `ASP_GATE_SEAM_GREEN_SHIFT_CV_FLOOR` (float, 0–10); constant `SEAM_GREEN_SHIFT_CV_GATE_FLOOR=1.2` in `constants/animation.py`
- 5 tests in `TestSeamGreenShiftCv` in `test_canvas.py`; 5 tests in `TestSeamGreenShiftCvGatePipeline` in `test_pipeline.py`

### §5.111 Bench Strip Blue Channel CV Comparative Gate ✅ [Priority: Low — bench blue-channel-CV parity]

**Problem**: §5.109 detects strip blue-channel inconsistency in the pipeline; benchmark needs a paired comparative gate.

**Implementation**:
- `_BLUE_CHANNEL_CV_ABS_FLOOR=0.20`, `_BLUE_CHANNEL_CV_RATIO=3.0` in `bench_anime_stitch.py`
- Comparative gate §5.111 in `run_dataset()`: fires when `asp_val > 0.20 AND (sim_val < 0.07 OR asp_val > 3.0 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_BLUE_CHANNEL_CV_ABS_FLOOR` (float), `ASP_BENCH_BLUE_CHANNEL_CV_RATIO` (float)
- 5 tests in `TestBlueChannelCvGateBench` in `test_bench_metrics.py`

### §5.112 Bench Seam Green Shift CV Comparative Gate ✅ [Priority: Low — bench green-shift-CV parity]

**Problem**: §5.110 detects green-axis seam normalization artifacts in the pipeline; benchmark needs a paired comparative gate.

**Implementation**:
- `_SEAM_GREEN_SHIFT_CV_ABS_FLOOR=0.30`, `_SEAM_GREEN_SHIFT_CV_RATIO=2.0` in `bench_anime_stitch.py`
- Comparative gate §5.112 in `run_dataset()`: fires when `asp_val > 0.30 AND (sim_val < 0.10 OR asp_val > 2.0 × max(sim_val, 0.01))`
- Schema: `ASP_BENCH_SEAM_GREEN_SHIFT_CV_ABS_FLOOR` (float), `ASP_BENCH_SEAM_GREEN_SHIFT_CV_RATIO` (float)
- 5 tests in `TestSeamGreenShiftCvGateBench` in `test_bench_metrics.py`
