...
from backend.src.constants.anim import (
...
def trapped_ball_segment(
    ...
    for iteration in range(n_iter):
        radius = min_radius + iteration * max(1, (max_radius - min_radius) // max(n_iter - 1, 1))
        ...
        indices = rng.choice(len(unlabeled_ys), size=min(n_seeds, len(unlabeled_ys)), replace=False)
        ...
        for sy, sx in zip(seed_ys, seed_xs):
            ...
            area, _, _, _ = cv2.floodFill(
                lab.copy(), flood_mask, (int(sx), int(sy)), 0,
                loDiff=lo, upDiff=hi, flags=flood_flags
            )
            if area < 1:
                continue
    ...
    if still_unlabeled.any() and (~still_unlabeled).any():
        ...
        for _ in range(max(1, H // 32)):
            dilated = cv2.dilate(labeled_img.astype(np.float32), kernel).astype(np.int32)
...
def build_mdm(
    ...
    fa_mat = np.stack([feats_a[k] for k in keys_a], axis=0).astype(np.float32)  # (Na, C)
    fb_mat = np.stack([feats_b[k] for k in keys_b], axis=0).astype(np.float32)  # (Nb, C)
...
if _TORCH_OK:
    class ConvGRUCell(nn.Module):
        ...
        def __init__(self, input_dim: int = 2, hidden_dim: int = 32, kernel_size: int = 3):
            super().__init__()
            pad = kernel_size // 2
            self.reset_gate = nn.Conv2d(input_dim + hidden_dim, hidden_dim, kernel_size, padding=pad)
            self.update_gate = nn.Conv2d(input_dim + hidden_dim, hidden_dim, kernel_size, padding=pad)
            self.out_gate = nn.Conv2d(input_dim + hidden_dim, hidden_dim, kernel_size, padding=pad)
...
def _get_gru(weights_path: Optional[str] = None) -> Tuple[Optional[object], Optional[str]]:
...
def compute_animeinterp_flow(
    ...
    cb_arr = np.array([cents_b[k] for k in keys_b], dtype=np.float32)  # (Nb, 2) [cy, cx]
    ...
    valid = (flat_labels < max_lbl_a) & (row_lookup[flat_labels.clip(0, max_lbl_a - 1)] >= 0)
    ...
    if n_gru_iters > 0:
        ...
        if gru is not None:
            try:
                t_flow = torch.from_numpy(flow).permute(2, 0, 1).unsqueeze(0).to(gru_device)
