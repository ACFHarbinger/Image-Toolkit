...
def _flow_refine(
    ...
    for i in range(1, N):
        ...
        ty_p = float(M_prev[1, 2]);  tx_p = float(M_prev[0, 2])
        ty_c = float(M_cur[1, 2]);   tx_c = float(M_cur[0, 2])
        ...
        r0_p = int(round(ov_top  - ty_p));  r1_p = r0_p + ov_h
        c0_p = int(round(ov_left - tx_p));  c1_p = c0_p + ov_w
        r0_c = int(round(ov_top  - ty_c));  r1_c = r0_c + ov_h
        c0_c = int(round(ov_left - tx_c));  c1_c = c0_c + ov_w

        # Clamp to valid frame bounds
        r0_p = max(0, r0_p); r1_p = min(H_p, r1_p)
        c0_p = max(0, c0_p); c1_p = min(W_p, c1_p)
        r0_c = max(0, r0_c); r1_c = min(H_c, r1_c)
        c0_c = max(0, c0_c); c1_c = min(W_c, c1_c)
        ...
        if bg_p is not None:
            ...
            if scale < 1.0:
                bm_c = cv2.resize(
                    bm_c, (crop_p.shape[1], crop_p.shape[0]),
