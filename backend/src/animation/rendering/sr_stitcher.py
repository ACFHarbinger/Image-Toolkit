"""
backend/src/animation/sr_stitcher.py
================================
...
def _get_inpaint_pipeline(device: str = "cpu", model_id: Optional[str] = None):
    ...
    models_to_try = (
        [model_id] + _ANIME_INPAINT_MODELS if model_id else _ANIME_INPAINT_MODELS
    )
...
def _inpaint_region(
    ...
    for y0 in range(0, H, step):
        ...
        for x0 in range(0, W, step):
            ...
            pad_h = tile_size - th
            pad_w = tile_size - tw
            tile_img_pad = cv2.copyMakeBorder(
                tile_img, 0, pad_h, 0, pad_w, cv2.BORDER_REFLECT
            )
            tile_mask_pad = cv2.copyMakeBorder(
...
def seam_diffusion_fusion(
    ...
    return _inpaint_region(
        canvas, mask, device=device, num_steps=num_steps, model_id=model_id
    )
...
def border_diffusion_fill(
    ...
    print(f"[SRStitcher] Border diffusion fill: coverage gap={coverage:.1%}.")
    return _inpaint_region(
        canvas, mask, device=device, num_steps=num_steps, model_id=model_id
    )
