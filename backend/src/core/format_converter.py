...
class ImageFormatConverter:
    ...
    def _convert_img_core(
        image_path: str, output_path: str, format: str, delete: bool
        ...
        try:
            with Image.open(image_path) as img:
                ...
                print(
                    f"Converted '{os.path.basename(image_path)}' to '{os.path.basename(output_path)}'."
                )
                return img
    ...
    def convert_batch(
        ...
        input_formats = [
            f.lower()
            for f in inputs_formats
            if not (
                f.lower() == output_fmt
                or (is_jpeg_or_jpg(f.lower()) and is_jpeg_or_jpg(output_fmt))
            )
        ]
            ...
            for input_file in glob.glob(os.path.join(input_dir, f"*.{input_format}")):
                ...
                if not os.path.isfile(output_path):
                    img = self._convert_img_core(
                        input_file, output_path, output_format, delete
                    )
                    if img is not None:
