...
class CryptographyJarNotBuiltError(RuntimeError):
    """Raised when cryptography/build/libs/*.jar (generated by `just build-jar`)
...
class VaultManager:
    ...
    def __init__(self, bc_provider_path: Optional[str] = None, is_guest: bool = False):
        ...
        if not jpype.isJVMStarted(): # pyrefly: ignore [missing-attribute]
            # Ensure udef.JAR_FILE exists before starting JVM. Deliberately not a
            ...
            # simply hasn't built the Kotlin cryptography module yet.
            if not os.path.exists(udef.JAR_FILE):
                raise CryptographyJarNotBuiltError(
                    f"Cryptography JAR not found at: {udef.JAR_FILE}\n"
                    "It hasn't been built yet (this is a generated, gitignored "
                    "artifact, not tracked account data). Run `just build-jar` "
