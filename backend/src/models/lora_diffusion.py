...
class LoRATuner:
    ...
    def __init__(self, model_id="stablediffusionapi/anything-v5", output_dir="output_lora"):
        ...
        if self.is_sdxl:
            print("Mode: SDXL (Animagine/Pony/etc) detected. Enabling dual-encoder training.")
        else:
        ...
        self.text_encoder_one = CLIPTextModel.from_pretrained(model_id, subfolder="text_encoder")
        
        if self.is_sdxl:
            # SDXL needs a second set
            self.tokenizer_two = CLIPTokenizer.from_pretrained(model_id, subfolder="tokenizer_2")
            self.text_encoder_two = CLIPTextModelWithProjection.from_pretrained(model_id, subfolder="text_encoder_2")
    ...
    def encode_prompt_sdxl(self, prompts):
        ...
        text_inputs1 = self.tokenizer_one(
            prompts, padding="max_length", max_length=self.tokenizer_one.model_max_length, truncation=True, return_tensors="pt"
        )
        with torch.no_grad():
            prompt_embeds1 = self.text_encoder_one(text_inputs1.input_ids.to(self.accelerator.device))[0]

        # 2. Encode with Tokenizer 2
        text_inputs2 = self.tokenizer_two(
            prompts, padding="max_length", max_length=self.tokenizer_two.model_max_length, truncation=True, return_tensors="pt"
        )
        with torch.no_grad():
            output2 = self.text_encoder_two(text_inputs2.input_ids.to(self.accelerator.device), output_hidden_states=True)
            prompt_embeds2 = output2.hidden_states[-2] # Penultimate layer
            pooled_prompt_embeds = output2.text_embeds  # Pooled outputs for "add_text_embeds"
    ...
    def train(self, data_dir, instance_prompt, epochs=1, learning_rate=1e-4, batch_size=1):
        ...
        dataloader = self.create_dataloader(data_dir, instance_prompt, resolution=resolution, batch_size=batch_size)
        
        optimizer = torch.optim.AdamW(
            self.unet.parameters(), lr=learning_rate, betas=(0.9, 0.999), weight_decay=1e-2, eps=1e-08
        )
        self.unet, optimizer, dataloader = self.accelerator.prepare(self.unet, optimizer, dataloader)
        ...
        for epoch in range(epochs):
            ...
            for step, batch in enumerate(dataloader):
                ...
                with torch.no_grad():
                    latents = self.vae.encode(batch["pixel_values"].to(dtype=self.text_encoder_one.dtype)).latent_dist.sample()
                ...
                if self.is_sdxl:
                    # SDXL Encoding (Dual)
                    prompt_embeds, pooled_embeds = self.encode_prompt_sdxl(batch["prompt"])
                    ...
                    time_ids = torch.tensor([[resolution, resolution, 0, 0, resolution, resolution]], device=self.accelerator.device, dtype=prompt_embeds.dtype)
                    ...
                    input_ids = self.tokenizer_one(
                        batch["prompt"], padding="max_length", max_length=self.tokenizer_one.model_max_length, 
                ...
                if added_cond_kwargs:
                    model_pred = self.unet(noisy_latents, timesteps, encoder_hidden_states, added_cond_kwargs=added_cond_kwargs).sample
                else:
    ...
    def generate_anime_image(
        ...
        if LoRATuner.is_cancelled: return
        ...
        use_negative = negative_prompt if negative_prompt else "lowres, bad anatomy, bad hands, text, error..."
