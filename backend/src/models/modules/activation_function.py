...
class ActivationFunction(nn.Module):
    def __init__(
        self,
        af_name: str = "relu",
        fparam: Optional[float] = 0.25,
        tval: Optional[float] = None,
        rval: Optional[float] = None,
        n_params: Optional[int] = 1,
        urange: Optional[Tuple[float, float]] = [1 / 8, 1 / 3],
        ...
        self.activation = {
            ...
            "prelu": nn.PReLU(
                num_parameters=n_params, init=fparam
        ...
        if tval and rval and not isinstance(self.activation, nn.Threshold):
            self.activation = lambda x: torch.where(
                self.activation(x) > tval, rval, self.activation(x)
            )
