    ...
    def create_dataloader(
        ...
        class SimpleImageDataset(Dataset):
            def __init__(self, root_dir, tokenizer, size=512, pruned_tags=None):
                ...
                self.pruned_tags = (
                    [t.strip().lower() for t in pruned_tags] if pruned_tags else []
                )
            ...
            def __getitem__(self, idx):
                ...
                final_prompt = (
                    f"{self.instance_prompt}, {', '.join(filtered_tags)}"
                    if filtered_tags
        ...
        dataset = SimpleImageDataset(
            data_dir, main_tokenizer, size=resolution, pruned_tags=pruned_tags
        )
        if len(dataset) == 0:
    ...
    def generate_anime_image(
        ...
        for i, image in enumerate(results):
            if LoRATuner.is_cancelled:
                return
            file_path = (
                f"{base_name}_{i + 1}{ext}" if batch_size > 1 else output_filename
            )
