        ...
        if command_name:
            ...
            if subparsers_action and command_name in subparsers_action.choices:
                ...
                if len(args) > 1 and not args[1].startswith("-"):
                    ...
                    if inner_subparsers_action and sub_command_name in inner_subparsers_action.choices:
                        actions_to_check.extend(inner_subparsers_action.choices[sub_command_name]._actions)
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        for kv in values:
            if "=" in kv:
                ...
                raise argparse.ArgumentError(self, f"Could not parse '{kv}' as key=value")
...
def add_core_args(parser):
    ...
    conv.add_argument("-i", "--input", nargs="+", required=True, help="Input file(s) or directory")
    conv.add_argument("-o", "--output", help="Output directory or file")
    conv.add_argument("-f", "--format", default="png", choices=SUPPORTED_IMG_FORMATS, help="Target format")
    conv.add_argument("-q", "--quality", type=int, default=95, help="JPEG/WebP quality (0-100)")
    ...
    merge.add_argument("-d", "--direction", default="horizontal", choices=["horizontal", "vertical", "grid"], help="Merge direction")
    merge.add_argument("-s", "--spacing", type=int, default=0, help="Spacing between images")
    merge.add_argument("--grid_size", nargs=2, type=int, help="Grid dimensions (rows cols)")
...
def add_web_args(parser):
    ...
    crawl.add_argument("-l", "--limit", type=int, default=10, help="Max images to download")
...
def add_database_args(parser):
    ...
    search.add_argument("-n", "--limit", dest="limit", type=int, default=50, help="Max results")
...
def add_model_args(parser):
    ...
    gen = model_subparsers.add_parser("generate", help="Generate images using ML models")
...
def add_gui_args(parser):
    parser.add_argument("--app_style", action=LowercaseAction, default="fusion", choices=APP_STYLES, help="GUI Style")
    parser.add_argument("--no_dropdown", action="store_true", help="Disable dropdown menu")
    return parser

def get_main_parser():
    parser = ConfigsParser(description="Image-Toolkit CLI", formatter_class=argparse.RawTextHelpFormatter)
...
def validate_core_args(opts):
    if opts.get("core_command") == "merge":
        if opts.get("direction") == "grid" and not opts.get("grid_size"):
            raise argparse.ArgumentError(None, "Merge mode 'grid' requires --grid_size R C")
    return opts
