...
class GoogleDriveSync:
    ...
    def _get_drive_service(self):
        ...
        if self.sa_data:
            ...
            self.logger(
                "❌ Authentication Error: Missing both Service Account data and Client Secrets data."
            )
    ...
    def _auth_service_account(self):
        ...
        try:
            ...
            if "client_email" not in self.sa_data or "private_key" not in self.sa_data:
                self.logger(
                    "❌ Authentication Error: Service account data is malformed."
                )
                raise RuntimeError(SYNC_ERROR)
            ...
            self.logger(
                f"❌ An unexpected authentication error occurred (Service Account): {e}"
            )
            raise RuntimeError(SYNC_ERROR)
    ...
    def _auth_personal_account(self):
        ...
        if not self.token_file:
            self.logger(
                "❌ Authentication Error: Token file path is required for Personal Account."
            )
            raise RuntimeError(SYNC_ERROR)
            ...
            if not creds or not creds.valid:
                if creds and creds.expired and creds.refresh_token:
                    ...
                    if "installed" not in self.cs_data and "web" not in self.cs_data:
                        self.logger(
                            "❌ Authentication Error: Client secrets data invalid (missing 'installed' or 'web' key)."
                        )
                        raise RuntimeError(SYNC_ERROR)

                    # Initialize flow from dictionary
                    flow = InstalledAppFlow.from_client_config(self.cs_data, SCOPES)
    ...
    def _find_or_create_destination_folder(self) -> Optional[str]:
        ...
        for folder_name in path_components:
            ...
            try:
                ...
                if files:
                    ...
                    if self.dry_run:
                        self.logger(
                            f"   [DRY RUN] Would have created folder '{folder_name}'"
                        )
                        ...
                        self.logger(
                            f"   Created folder: {folder_name} (ID: {current_parent_id})"
                        )
                ...
                self.logger(
                    f"❌ Unexpected error in folder creation for '{folder_name}': {e}"
                )
                return None
        ...
        if (
            ...
            and not current_parent_id.startswith("DRY_RUN_ID")
        ):
    ...
    def _get_remote_files_map(self) -> Dict[str, Dict[str, Any]]:
        ...
        while folder_queue:
            ...
            while True:
                ...
                for item in items:
                    item_name = item["name"]
                    full_path = os.path.join(current_rel_path, item_name).replace(
                        os.sep, "/"
                    )
                    item_id = item["id"]
                    ...
                    try:
                        if modified_time_iso and "." in modified_time_iso:
                            dt_object = datetime.datetime.strptime(
                                modified_time_iso, "%Y-%m-%dT%H:%M:%S.%fZ"
                            )
                        elif modified_time_iso:
                            dt_object = datetime.datetime.strptime(
                                modified_time_iso, "%Y-%m-%dT%H:%M:%SZ"
                            )
        ...
        if not remote_items:
            ...
            for path in sorted(remote_items.keys()):
                ...
                self.logger(
                    f"   {item_type} {path} (Modified: {datetime.datetime.fromtimestamp(item['mtime']).strftime('%Y-%m-%d %H:%M:%S')})"
                )
        self.logger("--------------------------------------------------")
    ...
    def _download_file(self, file_id: str, local_destination_path: str) -> bool:
        ...
        if self.dry_run:
            self.logger(
                f"   [DRY RUN] Download: {os.path.basename(local_destination_path)}"
            )
            return True
        ...
        try:
            ...
            try:
                self.drive_service.files().get(
                    fileId=file_id, fields="trashed"
                ).execute()
                # If GET succeeds, deletion failed or file was just trashed (Drive default behavior)
                self.logger(
                    f"   ⚠️ WARNING: Deletion of {rel_path} failed verification (File ID {file_id} still exists)."
                )
                return False
            except HttpError as e:
    ...
    def execute_sync(self) -> tuple[bool, str]:
        ...
        try:
            ...
            if not self.check_stop_status(dest_folder_id):
                return (False, "Synchronization manually interrupted.")
            ...
            if (
                self.share_email
                and dest_folder_id
                and not dest_folder_id.startswith("DRY_RUN_ID")
            ):
                if self.sa_data:
                    self.logger(
                        f"ℹ️  [Note] Sharing functionality with {self.share_email} is pending implementation."
                    )
                else:
                    self.logger(
                        "Skipping Share Action: Sharing is only relevant for Service Account flow."
                    )
            ...
            if not local_path_exists or not local_path_is_dir:
                return (
                    False,
                    f"Local source path '{self.local_path}' does not exist or is not a directory.",
                )
            ...
            for rel_path, local_data in local_items.items():
                ...
                if remote_data:
                    ...
                    self.logger(
                        f"   SKIPPING: {rel_path} (File exists locally and remotely)"
                    )
                    items_skipped += 1
                    items_skipped_remote.pop(rel_path)
                else:
            ...
            sorted_remote_orphans = sorted(
                items_skipped_remote.items(), key=lambda item: item[0], reverse=True
            )
            ...
            for rel_path, remote_data in sorted_remote_orphans:
                ...
                if self.action_remote == "download":
                    ...
                    if self._download_file(
                        remote_data["id"], os.path.join(self.local_path, rel_path)
                    ):
                        items_downloaded += 1
                    ...
                    self.logger(
                        f"   IGNORING REMOTE: {rel_path} (Action set to 'Do Nothing')"
                    )
                    items_ignored += 1
            ...
            if total_actions == 0 and items_ignored == 0:
                self.logger(
                    f"✅ Sync successful! No changes required. ({items_skipped} items matched)"
                )
                ...
                self.logger(
                    f"   Uploads: {items_uploaded}, Downloads: {items_downloaded}"
                )
                self.logger(
                    f"   Local Deletes: {items_deleted_local}, Remote Deletes: {items_deleted_remote}"
                )
                self.logger(
                    f"   Items Ignored: {items_ignored}, Items Skipped (Matched): {items_skipped}"
                )
