...
class OneDriveSync:
        ...
        while queue:
            ...
            while url:
                ...
                if response.status_code != 200:
                    self.logger(
                        f"❌ Error listing children for {current_rel}: {response.text}"
                    )
                    break
                ...
                for item in items:
                    ...
                    rel_path = (
                        os.path.join(current_rel, name).replace(os.sep, "/")
                        if current_rel
                        else name
                    )

                    remote_items[rel_path] = {"id": item_id, "is_folder": is_folder}

                    if is_folder:
                        queue.append((item_id, rel_path))
                        # Cache ID for potential uploads later
                        full_path_key = (
                            f"{self.remote_path}/{rel_path}"
                            if self.remote_path
                            else rel_path
                        )
    ...
    def _create_remote_folder(self, rel_path: str):
        ...
        full_parent_path = (
            f"{self.remote_path}/{parent_rel}" if parent_rel else self.remote_path
        )
        if not full_parent_path:
            full_parent_path = ""  # root
        ...
        if response.status_code == 201:
            ...
            full_path = (
                f"{self.remote_path}/{rel_path}" if self.remote_path else rel_path
            )
    ...
    def _download_file(self, item_id: str, local_dest: str):
        ...
        if self.dry_run:
            self.logger(
                f"   [DRY RUN] DOWNLOAD: {item_id} -> {os.path.basename(local_dest)}"
            )
            return True
            ...
            for rel_path, remote_data in items_skipped_remote.items():
                ...
                if remote_data["is_folder"]:
                    if self.action_remote == "delete_remote":
                        ...
                        if self._delete_remote(remote_data["id"], rel_path):
                ...
                if self.action_remote == "download":
                    ...
                    if self._download_file(remote_data["id"], local_dest):
                        items_downloaded += 1
