...
class WebRequestsLogic(QObject):
        ...
        for action in self.action_list:
            ...
            try:
                if action_type == "Print Response URL":
                    ...
                    self.on_status.emit(f"  > Action: Status Code: {response.status_code}")
                    ...
                    if not param:
                        self.on_error.emit("  > Action: Save failed. No file path provided in parameter.")
                        continue
                    ...
                    try:
                        ...
                        self.on_status.emit(f"  > Action: Response content saved to {filepath}")
                    except Exception as e:
                        self.on_error.emit(f"  > Action: Failed to save file to {filepath}: {e}")
            
            except Exception as e:
                self.on_error.emit(f"  > Action: Failed to execute '{action_type}': {e}")
    ...
    def run(self):
        ...
        for i, req in enumerate(self.request_list):
            ...
            self.on_status.emit(f"--- Request {i+1}/{len(self.request_list)}: [{req_type}] ---")
            ...
            try:
                ...
                if req_type == "GET":
                    ...
                    response = requests.get(url_to_request, timeout=10, allow_redirects=True)
                
                elif req_type == "POST":
                    post_data = self._parse_post_data(param)
                    self.on_status.emit(f"Executing POST: {url_to_request} with data: {post_data}")
                    response = requests.post(url_to_request, data=post_data, timeout=10, allow_redirects=True)
                
                if response is not None:
                    self.on_status.emit(f"Request complete. Status: {response.status_code}")
                ...
                self.on_error.emit(f"Request failed: HTTP {he.response.status_code} {he.response.reason}")
