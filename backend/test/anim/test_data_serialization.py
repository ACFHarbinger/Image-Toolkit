"""
Tests for backend.src.anim.data_serialization — Issue 10B1+10B2.
"""
...
from backend.src.anim.data_serialization import (
...
class TestCOCOAnnotationBuilder:
    ...
    def test_add_seam_exclusion_with_bbox(self):
        ...
        ann_id = b.add_seam_exclusion(img_id, bbox=[100, 50, 80, 60], text_prompt="right arm")
        d = b.to_dict()
        ann = d["annotations"][0]
    ...
    def test_frame_selection_override_annotation(self):
        ...
        ann_id = b.add_frame_selection_override(img_id, accepted=False, reason="duplicate pose")
        d = b.to_dict()
        ann = d["annotations"][0]
...
class TestCreateSessionSerializers:
    def test_returns_three_values(self, tmp_path):
        builder, exporter, session_dir = create_session_serializers(str(tmp_path / "session"))
