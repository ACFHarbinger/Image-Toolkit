...
class TestSobelEnergyCvGateBench:
    def test_module_flags_exist_and_are_floats(self):
        assert isinstance(_SOBEL_ENERGY_CV_ABS_FLOOR, float)
        assert isinstance(_SOBEL_ENERGY_CV_RATIO, float)

    def test_schema_keys_present(self):
        assert "ASP_GATE_SOBEL_ENERGY_CV" in _asp_config._CONFIG_SCHEMA
        assert "ASP_GATE_SOBEL_ENERGY_CV_FLOOR" in _asp_config._CONFIG_SCHEMA

    def test_gate_fires_when_asp_high_sim_low(self):
        asp_val = _SOBEL_ENERGY_CV_ABS_FLOOR + 0.10
        sim_val = 0.01
        fires = asp_val > _SOBEL_ENERGY_CV_ABS_FLOOR and (
            sim_val < 0.12 or asp_val > _SOBEL_ENERGY_CV_RATIO * max(sim_val, 0.01)
        )
        assert fires
...
class TestSeamValueShiftCvGateBench:
    def test_module_flags_exist_and_are_floats(self):
        assert isinstance(_SEAM_VALUE_SHIFT_CV_ABS_FLOOR, float)
        assert isinstance(_SEAM_VALUE_SHIFT_CV_RATIO, float)

    def test_schema_keys_present(self):
        assert "ASP_GATE_SEAM_VALUE_SHIFT_CV" in _asp_config._CONFIG_SCHEMA
        assert "ASP_GATE_SEAM_VALUE_SHIFT_CV_FLOOR" in _asp_config._CONFIG_SCHEMA

    def test_gate_fires_when_asp_high_sim_low(self):
        asp_val = _SEAM_VALUE_SHIFT_CV_ABS_FLOOR + 0.10
        sim_val = 0.01
        fires = asp_val > _SEAM_VALUE_SHIFT_CV_ABS_FLOOR and (
            sim_val < 0.10 or asp_val > _SEAM_VALUE_SHIFT_CV_RATIO * max(sim_val, 0.01)
        )
        assert fires
...
class TestMedianLumaCvGateBench:
    def test_module_flags_exist_and_are_floats(self):
        assert isinstance(_MEDIAN_LUMA_CV_ABS_FLOOR, float)
        assert isinstance(_MEDIAN_LUMA_CV_RATIO, float)

    def test_schema_keys_present(self):
        assert "ASP_GATE_MEDIAN_LUMA_CV" in _asp_config._CONFIG_SCHEMA
        assert "ASP_GATE_MEDIAN_LUMA_CV_FLOOR" in _asp_config._CONFIG_SCHEMA

    def test_gate_fires_when_asp_high_sim_low(self):
        asp_val = _MEDIAN_LUMA_CV_ABS_FLOOR + 0.10
        sim_val = 0.01
        fires = asp_val > _MEDIAN_LUMA_CV_ABS_FLOOR and (
            sim_val < 0.08 or asp_val > _MEDIAN_LUMA_CV_RATIO * max(sim_val, 0.01)
        )
        assert fires
    ...
    def test_gate_does_not_fire_below_abs_floor(self):
        asp_val = _MEDIAN_LUMA_CV_ABS_FLOOR - 0.05
        sim_val = 0.0
        fires = asp_val > _MEDIAN_LUMA_CV_ABS_FLOOR and (
            sim_val < 0.08 or asp_val > _MEDIAN_LUMA_CV_RATIO * max(sim_val, 0.01)
        )
...
class TestSeamEntropyShiftCvGateBench:
    def test_module_flags_exist_and_are_floats(self):
        assert isinstance(_SEAM_ENTROPY_SHIFT_CV_ABS_FLOOR, float)
        assert isinstance(_SEAM_ENTROPY_SHIFT_CV_RATIO, float)

    def test_schema_keys_present(self):
        assert "ASP_GATE_SEAM_ENTROPY_SHIFT_CV" in _asp_config._CONFIG_SCHEMA
        assert "ASP_GATE_SEAM_ENTROPY_SHIFT_CV_FLOOR" in _asp_config._CONFIG_SCHEMA

    def test_gate_fires_when_asp_high_sim_low(self):
        asp_val = _SEAM_ENTROPY_SHIFT_CV_ABS_FLOOR + 0.10
        sim_val = 0.01
        fires = asp_val > _SEAM_ENTROPY_SHIFT_CV_ABS_FLOOR and (
            sim_val < 0.10 or asp_val > _SEAM_ENTROPY_SHIFT_CV_RATIO * max(sim_val, 0.01)
        )
        assert fires
    ...
    def test_gate_does_not_fire_below_abs_floor(self):
        asp_val = _SEAM_ENTROPY_SHIFT_CV_ABS_FLOOR - 0.05
        sim_val = 0.0
        fires = asp_val > _SEAM_ENTROPY_SHIFT_CV_ABS_FLOOR and (
            sim_val < 0.10 or asp_val > _SEAM_ENTROPY_SHIFT_CV_RATIO * max(sim_val, 0.01)
        )
...
class TestRedChannelCvGateBench:
    def test_module_flags_exist_and_are_floats(self):
        assert isinstance(_RED_CHANNEL_CV_ABS_FLOOR, float)
        assert isinstance(_RED_CHANNEL_CV_RATIO, float)

    def test_schema_keys_present(self):
        assert "ASP_GATE_RED_CHANNEL_CV" in _asp_config._CONFIG_SCHEMA
        assert "ASP_GATE_RED_CHANNEL_CV_FLOOR" in _asp_config._CONFIG_SCHEMA

    def test_gate_fires_when_asp_high_sim_low(self):
        asp_val = _RED_CHANNEL_CV_ABS_FLOOR + 0.10
        sim_val = 0.01
        fires = asp_val > _RED_CHANNEL_CV_ABS_FLOOR and (
            sim_val < 0.07 or asp_val > _RED_CHANNEL_CV_RATIO * max(sim_val, 0.01)
        )
        assert fires
    ...
    def test_gate_does_not_fire_below_abs_floor(self):
        asp_val = _RED_CHANNEL_CV_ABS_FLOOR - 0.05
        sim_val = 0.0
        fires = asp_val > _RED_CHANNEL_CV_ABS_FLOOR and (
            sim_val < 0.07 or asp_val > _RED_CHANNEL_CV_RATIO * max(sim_val, 0.01)
        )
...
class TestSeamBlueShiftCvGateBench:
    def test_module_flags_exist_and_are_floats(self):
        assert isinstance(_SEAM_BLUE_SHIFT_CV_ABS_FLOOR, float)
        assert isinstance(_SEAM_BLUE_SHIFT_CV_RATIO, float)

    def test_schema_keys_present(self):
        assert "ASP_GATE_SEAM_BLUE_SHIFT_CV" in _asp_config._CONFIG_SCHEMA
        assert "ASP_GATE_SEAM_BLUE_SHIFT_CV_FLOOR" in _asp_config._CONFIG_SCHEMA

    def test_gate_fires_when_asp_high_sim_low(self):
        asp_val = _SEAM_BLUE_SHIFT_CV_ABS_FLOOR + 0.10
        sim_val = 0.01
        fires = asp_val > _SEAM_BLUE_SHIFT_CV_ABS_FLOOR and (
            sim_val < 0.10 or asp_val > _SEAM_BLUE_SHIFT_CV_RATIO * max(sim_val, 0.01)
        )
        assert fires
    ...
    def test_gate_does_not_fire_below_abs_floor(self):
        asp_val = _SEAM_BLUE_SHIFT_CV_ABS_FLOOR - 0.05
        sim_val = 0.0
        fires = asp_val > _SEAM_BLUE_SHIFT_CV_ABS_FLOOR and (
            sim_val < 0.10 or asp_val > _SEAM_BLUE_SHIFT_CV_RATIO * max(sim_val, 0.01)
        )
...
class TestGreenChannelCvGateBench:
    def test_constants_are_floats(self):
        assert isinstance(_GREEN_CHANNEL_CV_ABS_FLOOR, float)
        assert isinstance(_GREEN_CHANNEL_CV_RATIO, float)

    def test_gate_fires_when_asp_high_and_sim_low(self):
        asp_val = _GREEN_CHANNEL_CV_ABS_FLOOR + 0.10
        sim_val = 0.0
        fires = asp_val > _GREEN_CHANNEL_CV_ABS_FLOOR and (
            sim_val < _GREEN_CHANNEL_CV_ABS_FLOOR / 3.0
            or asp_val > _GREEN_CHANNEL_CV_RATIO * max(sim_val, 0.01)
        )
        assert fires
    ...
    def test_gate_does_not_fire_below_abs_floor(self):
        asp_val = _GREEN_CHANNEL_CV_ABS_FLOOR - 0.05
        sim_val = 0.0
        fires = asp_val > _GREEN_CHANNEL_CV_ABS_FLOOR and (
            sim_val < _GREEN_CHANNEL_CV_ABS_FLOOR / 3.0
            or asp_val > _GREEN_CHANNEL_CV_RATIO * max(sim_val, 0.01)
...
class TestSeamRedShiftCvGateBench:
    def test_constants_are_floats(self):
        assert isinstance(_SEAM_RED_SHIFT_CV_ABS_FLOOR, float)
        assert isinstance(_SEAM_RED_SHIFT_CV_RATIO, float)

    def test_gate_fires_when_asp_high_and_sim_low(self):
        asp_val = _SEAM_RED_SHIFT_CV_ABS_FLOOR + 0.10
        sim_val = 0.0
    ...
    def test_gate_does_not_fire_below_abs_floor(self):
        asp_val = _SEAM_RED_SHIFT_CV_ABS_FLOOR - 0.05
        sim_val = 0.0
        fires = asp_val > _SEAM_RED_SHIFT_CV_ABS_FLOOR and (
            sim_val < 0.10 or asp_val > _SEAM_RED_SHIFT_CV_RATIO * max(sim_val, 0.01)
        )
