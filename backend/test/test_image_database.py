...
class TestPgvectorImageDatabase:
    ...
    def mock_db(self):
        with patch("src.core.image_database.psycopg2.connect") as mock_connect:
            ...
            db = PgvectorImageDatabase(db_name="test_db", db_user="user", db_password="pw", db_host="host", db_port="5432")
    ...
    def test_add_image_with_tags(self, mock_db):
        ...
        insert_image_tags_calls = [c for c in calls if "INSERT INTO image_tags" in c[0][0]]
    ...
    def test_reset_database(self, mock_db):
        ...
        # Should drop tables
        drops = [c for c in mock_cursor.execute.call_args_list if "DROP TABLE" in c[0][0]]
        assert len(drops) >= 5
        
        # Should recreate tables (CREATE TABLE)
        creates = [c for c in mock_cursor.execute.call_args_list if "CREATE TABLE" in c[0][0]]
