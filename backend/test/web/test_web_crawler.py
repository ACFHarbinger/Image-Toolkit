...
from backend.src.web.crawlers.image_crawler import ImageCrawler  # noqa: E402
...
@patch("backend.src.web.crawlers.image_crawler.QObject", MockQObject)
@patch("backend.src.web.crawlers.image_crawler.WebCrawler", MockWebCrawler)
@patch("backend.src.web.crawlers.image_crawler.QtABCMeta", MockWebCrawler.QtABCMeta)
class TestImageCrawler:
    ...
    @patch("backend.src.web.crawlers.image_crawler.os.path.exists", side_effect=[True, True, False])
    @patch("backend.src.web.crawlers.image_crawler.os.path.splitext", return_value=("/tmp/test", ".jpg"))
    ...
    @patch(
        "backend.src.web.crawlers.image_crawler.os.path.join", side_effect=lambda *args: "/".join(args)
    )
    @patch(
        "backend.src.web.crawlers.image_crawler.os.path.basename", return_value="image_from_url.jpg?q=1"
    )
    @patch(
        "backend.src.web.crawlers.image_crawler.os.path.splitext",
        return_value=("image_from_url", ".jpg"),
    )
    @patch("backend.src.web.crawlers.image_crawler.os.path.exists", return_value=False)
    ...
    def test_download_image_from_url_failure(self, crawler, mock_requests):
        ...
        with patch("backend.src.web.crawlers.image_crawler.print") as mock_print:
