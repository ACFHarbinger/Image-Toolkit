...
//
// Catch2 tests for base::image — load_image_batch and scan_files.
...
TEST_CASE("scan_files: returns matching extensions", "[images][scan_files]") {
    ...
    auto results = base::image::scan_files(dir.string(), {".png", ".jpg"}, false);
...
TEST_CASE("scan_files: recursive flag includes subdirectory files", "[images][scan_files]") {
    ...
    auto flat = base::image::scan_files(dir.string(), {".png"}, false);
    auto rec  = base::image::scan_files(dir.string(), {".png"}, true);
...
TEST_CASE("scan_files: empty directory returns empty list", "[images][scan_files]") {
    ...
    auto results = base::image::scan_files(dir.string(), {".png"}, true);
    CHECK(results.empty());
    fs::remove_all(dir);
}
...
TEST_CASE("scan_files: results are sorted", "[images][scan_files]") {
    ...
    auto results = base::image::scan_files(dir.string(), {".png"}, false);
