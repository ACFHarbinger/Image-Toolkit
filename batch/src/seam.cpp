...
std::vector<std::vector<int>> seam_batch_impl(const std::vector<batch::ZonePair>& pairs, float transition_penalty, float edge_weight) {
    ...
    float    transition_penalty  = 0.0f)
{
    ...
    std::vector<batch::ZonePair> pairs;
    pairs.reserve(N);
