...
//
// Catch2 tests for batch::video — extract_video_thumbnails_batch.
// Phase 3 skeleton.
// ---------------------------------------------------------------------------

#include <catch2/catch_test_macros.hpp>

#include "batch/video/video_batch.hpp"
...
TEST_CASE("extract_thumbnails: missing file returns error", "[video]") {
    auto result = batch::video::extract_thumbnails(
...
TEST_CASE("extract_thumbnails: empty timestamp list returns no frames", "[video]") {
    ...
    batch::video::VideoThumbResult r;
    r.path = "test.mp4";
    r.frames = {};
    r.error  = "";
...
TEST_CASE("extract_thumbnails: path is preserved in result", "[video]") {
    auto result = batch::video::extract_thumbnails("my_video.mp4", {}, 64, 64);
