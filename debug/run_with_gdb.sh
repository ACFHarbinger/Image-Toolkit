...
# line above) with debug/telemetry_analyzer.py to see exactly what
...
echo "    with: python debug/telemetry_analyzer.py)"
...
if [ ${#hs_err_files[@]} -gt 0 ]; then
    ...
    echo "  python debug/resolve_qt_offset.py --hs-err $OUT_DIR/hs_err_pid<PID>.log"
fi
shopt -u nullglob
