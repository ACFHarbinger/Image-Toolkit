...
def test_merge_images_horizontal(sample_images, output_dir):
    ...
    result = ImageMerger.merge_images(image_paths, output_path, direction='horizontal')
...
def test_merge_images_horizontal_with_spacing(sample_images, output_dir):
    ...
    result = ImageMerger.merge_images(image_paths, output_path, direction='horizontal', spacing=spacing)
    
    assert os.path.exists(output_path)
    assert isinstance(result, Image.Image)
    
    original_images = [Image.open(img) for img in image_paths]
    total_width = sum(img.width for img in original_images) + (spacing * (len(original_images) - 1))
    max_height = max(img.height for img in original_images)
    assert result.size == (total_width, max_height)


def test_merge_images_vertical(sample_images, output_dir):
    ...
    result = ImageMerger.merge_images(image_paths, output_path, direction='vertical')
...
def test_merge_images_vertical_with_spacing(sample_images, output_dir):
    ...
    result = ImageMerger.merge_images(image_paths, output_path, direction='vertical', spacing=spacing)
    ...
    original_images = [Image.open(img) for img in image_paths]
    max_width = max(img.width for img in original_images)
    total_height = sum(img.height for img in original_images) + (spacing * (len(original_images) - 1))
    
    assert result.size == (max_width, total_height)


def test_merge_images_grid(sample_images, output_dir):
    ...
    result = ImageMerger.merge_images(image_paths, output_path, direction='grid', grid_size=grid_size)
...
def test_merge_images_grid_with_spacing(sample_images, output_dir):
    ...
    result = ImageMerger.merge_images(image_paths, output_path, direction='grid', grid_size=grid_size, spacing=spacing)
...
def test_merge_images_grid_too_many_images(sample_images, output_dir):
    ...
    with pytest.raises(ValueError, match="More images provided than the grid slots can hold."):
        ImageMerger.merge_images(image_paths, output_path, 'grid', grid_size)


def test_merge_images_function(sample_images, output_dir):
    ...
    result_horizontal = ImageMerger.merge_images(image_paths, output_horizontal, 'horizontal')
    ...
    result_vertical = ImageMerger.merge_images(image_paths, output_vertical, 'vertical')
    ...
    result_grid = ImageMerger.merge_images(image_paths, output_grid, 'grid', grid_size=(2, 2))
    assert os.path.exists(output_grid)
    assert isinstance(result_grid, Image.Image)


def test_merge_images_invalid_direction(sample_images, output_dir):
...
def test_merge_images_grid_no_size(sample_images, output_dir):
    """Test merge_images grid direction without grid_size"""
    temp_dir, image_paths = sample_images
    output_path = os.path.join(output_dir, "grid_no_size.png")
    
    # Updated to reflect the ValueError raised by the ImageMerger class logic
    with pytest.raises(ValueError, match="grid_size must be provided for grid merging"):
        ImageMerger.merge_images(image_paths, output_path, 'grid')  # No grid_size provided


def test_merge_directory_images(sample_images, output_dir):
...
def test_merge_directory_images_multiple_formats(sample_images, output_dir):
...
def test_merge_directory_images_empty_directory(tmp_path):
...
def test_merge_directory_images_nonexistent_format(sample_images, output_dir):
...
def test_output_directory_creation(sample_images, output_dir):
...
def test_image_content_preservation(sample_images, output_dir):
    ...
    result = ImageMerger.merge_images([image_paths[0], image_paths[1]], output_path, direction='horizontal')
...
@pytest.mark.parametrize("direction,grid_size,expected_success", [
...
def test_merge_images_parameterized(sample_images, output_dir, direction, grid_size, expected_success):
    """Parameterized test for different merge scenarios"""
    temp_dir, image_paths = sample_images
    output_path = os.path.join(output_dir, f"param_test_{direction}.png")
    
    if expected_success:
        result = ImageMerger.merge_images(image_paths, output_path, direction, grid_size)
        assert os.path.exists(output_path)
        assert isinstance(result, Image.Image)
    else:
        if direction == 'grid' and grid_size is None:
            ...
            with pytest.raises(ValueError, match="grid_size must be provided for grid merging"):
                ImageMerger.merge_images(image_paths, output_path, direction, grid_size)
...
def test_single_image_merge(sample_images, output_dir):
    ...
    result = ImageMerger.merge_images(single_image, grid_output, 'grid', grid_size=(1, 1))
