...
from devtool.mcp import McpServer
...
class TestMcpProtocol:
    def test_initialize(self, tmp_path):
        ...
        result = resp["result"]
        assert result["serverInfo"]["name"] == "devtool"
...
class TestMcpTools:
    ...
    def test_list_artifacts(self, tmp_path):
        ...
        resp = _rpc(
            server,
            "tools/call",
            {"name": "list_artifacts", "arguments": {"entry_point": "devtool.plugins.telemetry_workbench:plugin"}},
        )
        assert resp["result"]["isError"] is False
