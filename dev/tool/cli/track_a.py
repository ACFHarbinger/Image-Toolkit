...
Kept in a separate module so the canonical CLI (cli/parser.py) stays a thin
...
def cmd_repro(args: Any) -> int:
    ...
    if not cmd and scenario_name:
        ...
        if sc:
            ...
            print(f"Unknown scenario '{scenario_name}'. Use 'tool repro --list-scenarios' to list.", file=sys.stderr)
            return 2

    if not cmd:
        print("repro requires a command or scenario (tool repro --scenario <NAME> OR tool repro -- CMD ARGS...)", file=sys.stderr)
        return 2
    ...
    for line in summary.splitlines():
        inv.append_note(line, author="tool.repro")
