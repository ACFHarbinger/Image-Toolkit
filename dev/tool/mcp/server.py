...
No third-party deps: the server is stdlib-only so it runs anywhere tool
imports.
"""
...
class McpServer:
    ...
    def serve_http(self, host: str = "127.0.0.1", port: int = 8000) -> None:
        """Optional localhost HTTP transport (tool serve --mcp)."""
        ...
        print(f"tool mcp listening on http://{host}:{httpd.server_port}", flush=True)
        try:
            httpd.serve_forever()
