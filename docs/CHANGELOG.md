...
Implements P0 and P1 of [moon/roadmaps/unified_database.md](../moon/roadmaps/unified_database.md) (merging the Listings subtabs' SQLCipher store and the Database tabs' PostgreSQL+pgvector index into one encrypted `~/.image-toolkit/library.db`; Postgres to be dropped entirely). 41 new tests in `backend/test/database/`; UI unchanged this session — tab ports land in P2/P3.
