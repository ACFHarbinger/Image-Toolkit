...
**Started implementing the Manga Colorization & Animation feature area** (`moon/roadmaps/manga_colorization_animation.md`, Milestone #6). This session ships the first working end-to-end slice: MCA.4 (Levin scribble colorizer), MCA.8 (layered canvas editor), and MCA.13 (Manga Colorization Tab) — closing issues #186, #190, and #195.
...
**Roadmap status**: `moon/roadmaps/manga_colorization_animation.md` MCA-1 phase partially shipped (MCA.4, MCA.8, MCA.13); MCA.1–MCA.3 (text/bubble removal, line art extraction, Gabor features) and the rest of MCA-2 onward remain open, tracked on the existing issues (#183–#185, #187–#189, #191–#200).
...
- `moon/roadmaps/architecture.md` §5.17: giants table added (matching the 24-file and 8-file tables' format), pain-point/Option-B/Recommendation prose updated from present to past tense, Effort×Impact Matrix's two remaining §5.17B cells collapsed into one "✅ shipped" entry, priority downgraded from High to Medium now that Option B is fully closed (Options A/D remain open). `moon/ROADMAP.md`: new item A.22 (the 3-giant split epic), marked Done.
...
- `moon/roadmaps/architecture.md` §5.17: `gui/src/` LoC table replaced with the 24-file shipped/commit-hash table (matching the `backend/src/` table's format); pain-point paragraph, Option A allow-list note, and Effort×Impact Matrix updated to reflect only the 3 giants remaining. `moon/ROADMAP.md`: new item A.21 (the 24-file split epic), marked Done.
...
- `moon/roadmaps/architecture.md`: §5.11 and §5.14 marked ✅ Shipped (A+B+C+D); §5.17's `backend/src/` LoC table replaced with a shipped/commit-hash table; Effort×Impact Matrix and Option A's "existing offenders" text updated from "35" to "27 remaining in `gui/src/`"; Mermaid nodes `s511`/`s514` → `:::done`, `s517` → `:::active`. `moon/ROADMAP.md`: new items A.18 (module graph), A.19 (`AppConfig`), A.20 (the 8-file split epic), all marked Done.
...
- **`moon/roadmaps/architecture.md` §5.17 "File Size Limit Enforcement (<500 LoC)" added, marked high priority.** Lists the worst offenders from a fresh count (27 `gui/src` files, 8 `backend/src` files over the 500-code-line threshold; `stitch_tab.py` at 5,032 and `extractor_tab.py` at 3,268 the worst two), with lettered Options A-E (CI-enforced gate, package-directory splits via mixins, dedup-before-split, auto-generated dashboard, blast-radius triage) and a Recommendation. Added to the Table of Contents, the Mermaid flowchart (`s517`, `:::planned`), the Effort×Impact matrix, and the Anchor Index.
...
- Full write-up: `.agent/cache/hugin_field_notes.md`. `moon/roadmaps/asp.md` §0.5 and `moon/roadmaps/new_features.md` §4.16 updated with status and implementation details.
