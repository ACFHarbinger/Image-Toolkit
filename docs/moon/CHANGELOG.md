...
**Started implementing the Manga Colorization & Animation feature area** (`docs/moon/roadmaps/manga_colorization_animation.md`, Milestone #6). This session ships the first working end-to-end slice: MCA.4 (Levin scribble colorizer), MCA.8 (layered canvas editor), and MCA.13 (Manga Colorization Tab) — closing issues #186, #190, and #195.
...
**Roadmap status**: `docs/moon/roadmaps/manga_colorization_animation.md` MCA-1 phase partially shipped (MCA.4, MCA.8, MCA.13); MCA.1–MCA.3 (text/bubble removal, line art extraction, Gabor features) and the rest of MCA-2 onward remain open, tracked on the existing issues (#183–#185, #187–#189, #191–#200).
...
- `docs/moon/roadmaps/asp.md` §0.1 already referenced the correct `backend/benchmark/managers/rating_manager.py` path — the stale `rate_coherence.py` cross-reference the task description flagged had already been corrected in a prior session; nothing left to update there.
...
- `docs/moon/roadmaps/asp.md` §0.3 and Phase 4 updated with full findings. No ASP pipeline behavior changed this round — both items are benchmark coverage / measurement tooling, not pipeline algorithm changes, consistent with the anti-goals against new gates/heuristics without a verified payoff.
...
- `docs/moon/roadmaps/asp.md` updated: new "Full-corpus checkpoint" section, Phase 2.6 marked fixed/confirmed, Phase 4 breakdown refined.
...
- **No code shipped** — closing this item as already satisfied rather than re-implementing something that exists. `docs/moon/roadmaps/asp.md` §3.3 corrected; posted as an update on GitHub issue #27.
...
- No code changes. `.agent/cache/asp_phase4_fallback_class_analysis_2026-07-27.md` and `docs/moon/roadmaps/asp.md` Phase 4 section updated with this finding; posted as a follow-up comment on GitHub issue #26.
...
- **No new code shipped** — this is diagnostic per Phase 4's instruction, not a measured code change. `ASP_JOINT_GAIN_SOLVE` stays default OFF. Full write-up: `.agent/cache/asp_phase4_fallback_class_analysis_2026-07-27.md`. `docs/moon/roadmaps/asp.md` Phase 4 section updated with all four findings.
...
- `.agent/cache/overmix_field_notes.md` and `docs/moon/roadmaps/asp.md` §1.2 updated with the full write-up and conclusion.
...
- `.agent/cache/asp_dense_band_scan_gate_2026-07-27.md` and `docs/moon/roadmaps/asp.md` §3.1 updated with this re-verification; posted as a follow-up comment on GitHub issue #26.
...
- `docs/moon/roadmaps/asp.md` §3.1 updated with this follow-up's outcome; flags `ASP_JOINT_GAIN_SOLVE`'s own disposition as needing a fresh re-measurement against the current ToonOut-inclusive baseline in a future session (out of scope here — this follow-up was gate-design only).
...
- No code changes — this is a research-currency update. Findings appended to `research/Image_Stitching_Research.md` §21 as a dated addendum; `docs/moon/roadmaps/asp.md` §1.1 marked done with the summary.
...
- `docs/moon/roadmaps/asp.md` §0.4 updated.
...
- Full write-up in `.agent/cache/asp_toonout_fix_2026-07-27.md`. `docs/moon/roadmaps/asp.md` §3.4 marked done.
...
- Full write-up in `.agent/cache/asp_joint_gain_solve_postmortem_2026-07-27.md` per the roadmap's ground rules. `docs/moon/roadmaps/asp.md` §3.1 updated with the mixed-result status.
...
- Full write-up: `.agent/cache/hugin_field_notes.md`. `docs/moon/roadmaps/asp.md` §0.5 and `docs/moon/roadmaps/new_features.md` §4.16 updated with status and implementation details.
...
- Full write-up: `.agent/cache/overmix_field_notes.md`. `docs/moon/roadmaps/asp.md` §0.3 and §1.2 updated with status and pointers.
...
- Full write-up in `.agent/cache/asp_phase_aware_select_postmortem_2026-07-27.md`. `docs/moon/roadmaps/asp.md` §2.4 updated to a short pointer.
...
- Full write-up in `.agent/cache/asp_bg_average_postmortem_2026-07-27.md` per the roadmap's ground rules. `docs/moon/roadmaps/asp.md` §2.5 updated to a short pointer.
...
- Full write-up in `.agent/cache/asp_graphcut_postmortem_2026-07-27.md` per the roadmap's ground rules (failures get a post-mortem there, not accreted detail in the roadmap itself). `docs/moon/roadmaps/asp.md` §1.3/§3.2 updated to a short pointer.
...
- All edits are surgical status corrections — no prose rewrites, no reorganization, `docs/moon/roadmaps/asp.md` itself untouched (already current).
...
- Updated `docs/moon/roadmaps/asp.md` §2.6 and the ground-rules warning to reflect "likely fixed, re-verify at scale" rather than "unresolved, do not run" — condensed the accreted per-session freeze narrative (S218/S219) into one summary now that there's a confirmed result.
...
- Full write-up and the keep/revert reasoning in `docs/moon/roadmaps/asp.md` §2.1. Both `ASP_HOLD_AVERAGE` and `ASP_PHASE_COMPOSITE` stay **default OFF** — the roadmap's ground rules require a human visual pass before any item is "done," and none has run against this data yet. These full-corpus numbers are the case for prioritizing that pass (Phase 0.1), not a decision to flip the defaults.
...
**Part 2 — ASP roadmap: merged missing items from the archived 3,596-line roadmap into `docs/moon/roadmaps/asp.md`:**
...
- **Docs**: all six `.agent/cache/` ASP analyses archived to `archive/agent/cache/`;
  `docs/moon/roadmaps/asp.md` (3,596 lines) and its `docs/roadmaps/` mirror deleted;
...
- Roadmap: `docs/moon/roadmaps/asp.md` / `docs/roadmaps/asp.md` §4.6 marked ✅; Effort × Impact Matrix pending-items table updated (S204, 2026-07-02).
...
*No new code changes. Full 97-test benchmark completed overnight (~3 hours, PID 16528 nohup). JSON: `backend/benchmark/output/anime_stitch_20260623_234305.json`. Documentation updated across `.agent/cache/` and `docs/moon/roadmaps/asp.md`.*
...
- `docs/moon/roadmaps/asp.md` — Phase 4 header, §3.32 confirmation, Document History updated to 97-test
...
| **New roadmap section §1.14** (`docs/moon/roadmaps/asp.md`) | Added as a new section with Option A (Bhattacharyya, shipped) and Option B (pipeline gate, future). |
