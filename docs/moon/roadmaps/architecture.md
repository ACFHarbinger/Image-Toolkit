...
flowchart LR
    ...
    subgraph Quality ["📐 Code Quality"]
        ...
        s517["§5.17 File Size Limit\nEnforcement (<500 LoC)"]:::refactor:::done
    end
...
- See: [`docs/moon/roadmaps/development_tool.md`](development_tool.md) — Track B Phase 8 (Distributed Observability) and Phase 4 (Causal Discovery).
...
- See: [`docs/moon/roadmaps/development_tool.md`](development_tool.md) — Track B Phase 3 (CV Diagnostics, §3.1).

**Recommendation:** A + B immediately. C as a quality-of-life follow-on. D if log analysis at scale is needed. F + G when building out the Visual Analytics dashboard (see development-tool roadmap). Skip E unless the app becomes multi-user.
...
- **Added — `docker` + `docker-compose`** (directory `/docker`): confirmed live — `infra/global/docker/Dockerfile` + `infra/global/docker/docker-compose.yml` back the Django/Celery/Postgres(pgvector)/Redis backend stack, added 2026-07-13. Previously untracked.
...
- This is a full-scope project defined in detail in [`docs/moon/roadmaps/development_tool.md`](development_tool.md) — Track B Phase 1 (Interactive Meta-Graph), covering SCIP indexing, Cosmograph rendering, SBEB edge bundling, Dependency Structure Matrix, and CPG-based semantic code analysis.
- Recommendation for §5.12: Options A+B cover immediate documentation needs; Option E is Track B Phase 1 of the development-tool roadmap and should be tracked there.

**Recommendation:** A + B immediately; they have zero infrastructure cost. C as a onboarding-focused one-pager. D once A is done and there is something worth generating docs from. E tracked separately in the development-tool roadmap.

---

## 5.13 Decorator Library for Cross-Cutting Concerns (`backend/src/utils/decorators.py`)

**Partial — Options A, C, E shipped; B not confirmed; D superseded. See ROADMAP.md item A.6.** Confirmed: `log_call()` (Option C) exists at `backend/src/utils/decorators/function_calls.py` (the module became a package rather than a single file, but is still importable via `backend/src/utils/decorators` per Option E); `@lazy_load` (Option A) shipped per ROADMAP 4.2/§5.8. `@require_path` (Option B) not found anywhere in the codebase — still open, delegated 2026-09-13 as issue #624. `@worker_method` (Option D) is superseded by `BaseQThreadWorker`'s built-in three-tier handler (§5.9/§5.15B, already shipped) — the option text itself says to choose one or the other.
...
**Partial — Option A shipped; C not yet, B not done. See ROADMAP.md item A.9.** Confirmed: `backend/test/models/test_wrapper_contracts.py` exists with mock-based interface contract tests (Option A). Option C (a shared `ModelWrapperContractMixin`) is not yet factored out — a comment in the test file itself still describes it as future work ("will..."), so individual per-wrapper contract test classes exist without the shared mixin; delegated 2026-09-13 as issue #625. Option B (GPU-gated smoke tests with real weights) not found — still open, not yet delegated (needs a GPU CI runner decision first). Note also: the "827-test suite" figure below is stale post-S200-trim; re-verified 2026-07-27 at 675 tests (see ROADMAP.md item 3.13).
...
**Recommendation:** ✅ A shipped (issue #626) — CI-enforced gate (`loc-gate` job in `ci.yml`) rejects any new file exceeding 500 code lines. ✅ D shipped (issue #626) — `loc-dashboard` job auto-regenerates `docs/loc_report.md` on every push to main. E before every individual split decided under B (this is how the giants were sequenced under issue #122 — `settings_window.py` first as the smallest, `stitch_tab.py` last as the largest and most central). C wherever a section of this document already targets the duplication driving a file's size.
