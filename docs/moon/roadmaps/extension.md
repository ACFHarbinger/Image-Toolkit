...
**Status: ✅ Done (2026-08-05, issue #102).** Modes A+B shipped S208 (2026-07-04); grid-preview page (mode C) shipped this issue.

**Shipped as:** `extension/src/gallery/gallery.{html,ts}`, a new tab page opened via a "🔲 Grid preview…" popup button (`options.html`/`options.ts::openGridPreview()`). The popup asks the active tab's content script for every detected image via a new `collect_page_images` message (`content.ts`, `shared/pageMedia.ts::collectImageDetails()` — a non-downloading sibling of the existing `collectImages()`, refactored to share the same ≥64px/dedup/URL-scheme filtering and additionally reporting each image's width/height), stashes the list under `storage.local.galleryData` (a tab can't take constructor args), and opens `gallery.html`. The grid page renders one cell per image with a **min-dimension** number filter, a **URL-contains** text filter, and **format chips** (derived from each URL's extension / `data:image/...` MIME, built dynamically from what's actually on the page) — all applied client-side, no re-scan needed. Each cell has its own **download button** and **status badge** (idle/downloading/✓ done/✕ error with the failure message on hover), driven by a new `download_gallery_item` message (`background.ts`) that awaits a single `api.downloads.download()` call and reports success/failure — `downloadImage()` was changed from `Promise<void>` to `Promise<{ok, error}>` to support this while staying backward-compatible with its fire-and-forget callers (context menu, `download_batch`, turbo capture). "Download selected" runs the checked, filter-visible items through a 4-way concurrency pool for bulk progress. **Duplicate-check badges reuse the §7.16C client pHash pre-check as-is, no new dedup logic**: a "Check duplicates" button runs `computePhashFromUrl()` + `snapshotBestMatch()` (`shared/clientPhash.ts`, unchanged) per visible item against the existing cached `storage.local` snapshot, rendering "new" / "dup? d=N" / "?" (hash unavailable, e.g. CORS-blocked fetch) badges — same hint-not-authority framing §7.16C already documented, and the same manual "Refresh snapshot" control in options still owns keeping the cache current. This is exactly the integration §7.16C's own status note flagged as its natural next step.
...
- **C — Grid-preview page**: a full tab page listing every detected image with size/format/URL filters and per-item dup-check badges (§7.6/§7.16C integration), plus per-item download progress/status. See "Shipped as" above.

**Verification:** `npm run typecheck` clean for every new/touched file (pre-existing, unrelated `shared/api.ts`/`shared/dupTabs.ts` `@types/chrome` overload errors reproduce identically on an unmodified checkout — confirmed by stashing this change and rerunning — and are a toolchain drift issue, not something this issue touches). `npm run build:chrome` bundles `gallery.ts` cleanly (verified with a locally pinned `typescript@5.6.3`, since the repo's pinned `typescript@7.0.2` currently crashes `ts-loader` itself regardless of this change — also pre-existing and reproduced on an unmodified checkout). No test framework is configured for this package (no jest/vitest in `package.json`, no `*.test.ts` files) — verification is typecheck + build, matching how §7.1–§7.16C were verified. **Not done**: retry-on-error for individual failed downloads (the status badge shows the error but there's no one-click re-attempt — a small, natural follow-up); an automatic/alarm-based pHash snapshot refresh (already noted as future work in §7.16C, unchanged by this issue).
...
**Approach (selected):**
- **A — Background removal: ✅ Done (issue #93, 2026-08-05).** context item **"Remove background"** → `POST /api/extension/cv/bg-remove` `{url|data_b64}` → app runs BiRefNet (`birefnet_wrapper.py`), returns/saves transparent PNG (`<name>_nobg.png`) into the active folder profile. Long-running: job-id + polling (or SSE) using the app's existing task queue; extension shows a progress badge and notification on completion.
  **Shipped as:** `CvBgRemoveView`/`CvJobStatusView` (`extension_api/views.py`) + `cv_bg_remove_task` (new `extension_api/tasks.py`, autodiscovered by the project's existing Celery app — `api/celery.py` — the same queue `tasks/views.py`'s `CoreTaskView` already uses for other async work, per the roadmap's own "existing task queue" framing, not a second mechanism). The view synchronously resolves `{url|data_b64}` and does a cheap `cv2.imdecode` probe first (400 on garbage input instead of a wasted queued job), then `.delay()`s the real BiRefNet `get_mask()` call and returns `{job_id, status: "processing"}` (202). `GET /api/extension/cv/status/<job_id>/` wraps Celery's own `AsyncResult` (state/result/error). Extension side: `cvBgRemove()`/`pollCvJob()` (`extension/src/shared/bridge.ts`) + a "Remove background" context-menu item (`background.ts`'s `runBgRemove()`/`runCvJob()`) that shows a toolbar badge while polling, then routes the base64 PNG result through the existing `downloadImage()` path (folder-profile resolution, filename templating, optional sidecar all apply exactly as they do for any other capture) and fires a completion notification. **Deviation:** HTTP transport only — job-id+poll doesn't map onto native messaging's single-request/response call without a second design pass (the host would need its own polling loop or a push-style follow-up message); native-messaging parity for this feature is a reasonable, separate follow-on, not attempted here. Tests: `extension_api/tests.py::TestCvBgRemove` (mocked `BiRefNetWrapper.get_mask`, real Celery task execution via `CELERY_TASK_ALWAYS_EAGER`).
- **B — Upscale before save: ✅ Done (issue #94, 2026-08-05).** context item **"Upscale & save"** → `POST /api/extension/cv/upscale` `{url|data_b64, scale: 2|4}` → Real-ESRGAN (anime_6B, shared upscaler module CG.2); auto-suggested when the §7.11 extractor reports the best candidate is small (< configurable threshold, e.g. 600px).
  **Shipped as:** `CvUpscaleView` + `cv_upscale_task`, same job-id+poll pattern as A. **Deviation, verified before writing any code:** the anime_6B `RRDBNet` architecture (`esrgan_wrapper.py`) is hardcoded to two 2x nearest-upsample stages — a fixed native 4x forward pass regardless of the `ESRGANWrapper(scale=...)` constructor argument, which only feeds the wrapper's own tile-stitching math (constructing with `scale=2` would size output tiles for 2x while the network still emits 4x tiles, corrupting the stitch). So `scale=2` is honored by always running the real 4x pass and then a single Lanczos downsample to net 2x, not a second, cheaper model path — same visual-quality ceiling as 4x, just resized. `scale` outside `{2, 4}` falls back to 4. Extension side: `cvUpscale()` (`bridge.ts`) + "Upscale & save" menu item (`background.ts`), same `downloadImage()` hand-off as A. The §7.11-extractor auto-suggestion (small-image heuristic) is **not implemented** — out of scope for this pass, a pure extension-side UX addition layered on top of the now-working endpoint. Tests: `extension_api/tests.py::TestCvUpscale` (mocked `ESRGANWrapper.upscale`, covers both `scale=4` and the `scale=2` downsample path).
- **C — Auto-tag on save: ✅ Done (issue #95, 2026-08-05).** §7.7 ingest gains `auto_tag: bool` — WD14 (`wd_tagger_wrapper.py`, §4.4) tags computed server-side at ingest; tags stored in DB + optional sidecar; threshold + review-queue semantics reuse `tag_with_review()`.
  **Shipped as:** `bridge_handlers.handle_ingest` now checks `payload.get("auto_tag")`; when set, calls `WDTaggerWrapper().tag_with_review(dest_path)` synchronously after the file is saved and adds `{"tags": [...], "review_tags": [...]}` to both the provenance sidecar JSON and the 201 response body. **Deviation:** "tags stored in DB" is not implemented — §7.7 ingest does not write to any database at all yet (see this section's own "Remaining: embedding/DB indexing at ingest" note, still open); tags are stored in the sidecar only, ready to be picked up whenever that DB-indexing work lands. Tagging failures (missing `onnxruntime`, model download error, ...) are caught and reported as `{"tags": {"error": ...}}` rather than failing the ingest — the image is already saved on disk by that point, so losing it over a tagging failure would be strictly worse. No extension-side UI change was needed (the flag is a simple `true`/`false` on the existing `ingest()` call — wiring an options-page toggle for it is a small, separate follow-on). Tests: `extension_api/tests.py::TestIngest.test_auto_tag_adds_tags_to_sidecar_and_response` / `test_auto_tag_false_by_default_no_tags_key` / `test_auto_tag_failure_does_not_fail_ingest`.
- **D — OCR extraction + translation:** context item **"Extract text"** → `POST /api/extension/cv/ocr` `{url|data_b64, translate_to?: str}`; app runs OCR (manga-ocr for anime-style text, tesseract otherwise — new small app-side capability) and optional translation (local model, e.g. argos-translate/NLLB via the app; explicit non-cloud default). Result shown in a copyable popup panel with per-block layout; "copy all" + "save as .txt sidecar" actions. **Not started** — genuinely new app-side capability (no existing OCR/translation wrapper to wire behind a thin endpoint, unlike A/B/C), correctly out of scope for this pass.
...
- **B — GIF/animation frame extractor: ✅ Done (issue #97, 2026-08-05).** **Shipped as:** `extension/src/frames/frames.{html,ts}`, a new grid-preview tab opened by a new "Extract frames…" image context-menu item (`background.ts`), following #102's `galleryData` hand-off pattern (`framesData` in `storage.local` carries `{imageUrl, pageUrl}` since a new tab can't take constructor args). The page fetches the image itself, sniffs GIF/PNG(APNG)/WebP from magic bytes (not the often-wrong `Content-Type` header), and decodes every frame with the WebCodecs `ImageDecoder` API — a large scrubbable preview (with Play/Pause using each frame's real duration) plus a per-frame grid with checkboxes, mirroring `gallery.ts`'s established "detected items → grid → per-item action" shape. Selected/all frames save as a numbered PNG sequence via the existing `download_image` message. **Deviation from the original plan:** the `omggif`/`upng` JS-fallback decoder path is *not* bundled — `ImageDecoder` already covers every build target that matters here (chrome/edge/brave all ship it); only the Firefox build lacks it, and gets a clear "not supported in this browser" message instead. This keeps the feature dependency-free per this session's established bias; adding a fallback decoder for Firefox parity is a contained, separately-scoped follow-up. Original plan: context item **"Extract frames…"** on GIF/APNG/animated-WebP → decode in an extension page (`ImageDecoder` WebCodecs API where available; `omggif`/`upng` fallbacks) → frame grid preview with scrubber → save selected/all frames or send-to-app.
...
- **D — Video clip → GIF/WebP: ✅ Done (issue #99, 2026-08-05).** **Shipped as:** two new video context-menu items, "Record 5s clip (WebM)" and "Record 5s clip → Animated WebP" (`background.ts`, `videoClip.ts`), both drawing the target `<video>` onto an off-DOM canvas via `requestAnimationFrame`. WebM export feeds `canvas.captureStream()` into a `MediaRecorder`, exactly the roadmap's literal MVP. WebP export samples the same draw loop at 10fps, encodes each frame independently with `canvas.toBlob("image/webp")` (the only *animated*-WebP-adjacent thing any browser API actually offers — no browser exposes an animated-WebP *encoder*), and muxes the frames into one real animated WebP container by hand (`shared/webpMux.ts`, a few dozen lines of RIFF/VP8X/ANIM/ANMF chunk-writing per the published container spec — not an image codec, so no new dependency). **Deviation from the original plan:** true GIF export and the app-side ffmpeg conversion path are *not* built — the roadmap's own text frames ffmpeg conversion as "optional, app-side," and new backend/bridge work is out of scope for this pass; a fixed 5s duration is used instead of a user-configurable duration/format panel, mirroring §7.15A's already-established fixed 5-frame burst rather than inventing a new UI pattern for this one feature. Both are natural, separately-scoped follow-ups. Original plan: popup action **"Record clip"** → `MediaRecorder` on `captureStream()` of the target `<video>` for a user-set duration (≤ 30s) → WebM; optional app-side conversion to GIF/WebP via the bridge (ffmpeg already in the app stack) with palette optimization.
