...
| R4.3 | Live theme toggle is broken: found during #594 D12 (2026-09-13), pre-existing (not caused by #594 — `_theme.py`'s toggle logic is unchanged by the composition). **Still open, 2026-09-13 — reopened, do not mark done without a real live screenshot check.** Two rounds of fixes so far (`refresh_component_styles()`/`ThemedQss` in #620, then a `ThemeColor` re-resolving extras marker) each fixed a real, narrow secondary bug, but neither fixed the actual symptom: toggling still only recolors the header bar. Confirmed live via `MainWindow.grab()` screenshots (both directions) and direct instrumentation: `apply_stylesheet(app, qss)` really does set genuinely different content on the correct `QApplication` singleton every toggle (styleSheet() length changes to match), and a sampled `QTabBar` has zero local stylesheet override — yet the app-wide base cascade (`dark.qss`/`light.qss`'s `QWidget`/`QPushButton`/`QTabBar` rules, which cover the majority of the app's visual surface) does not visibly update. Ruled out: KDE/Breeze native style interference (forced Fusion — no change), the `PYTEST_CURRENT_TEST` branch mixup (confirmed the `app` branch runs), frozen `qss()` extras (real but narrow, fixed separately). A full-tree `style().unpolish()+polish()` sweep measurably helps but doesn't fix the majority — real lead, not yet run to ground. Full investigation notes and a live-screenshot debugging technique (`self.grab().save(path)` gated behind an env var) posted on the reopened issue. | Cursor, Claude (2 fix attempts, still open) | ui-arch-54 (#620) |
