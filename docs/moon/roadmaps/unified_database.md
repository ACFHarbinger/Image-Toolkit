...
- The `embedding` column and `hybrid_search_secure()` are dead weight (GUI writes empty or byte-of-title placeholder vectors). Recommendations bypass the DB entirely via `CRE/` (BGE-M3 hybrid, plain SQLite `rec_engine.db`).
...
**Verification note — 2026-08-22:** the rebuilt `base` extension resolves to
the compiled `.so`, exposes `base.database`, and completed a temporary
SQLCipher create/insert/query/close round trip. The remaining GUI acceptance
check—open the Database tab and verify listings load and save—is still
pending: the available display already had another active `just python`
session, which was deliberately not driven.
...
*Scale benchmark shipped 2026-08-22 (development_tool.md §12.4's rescoped target, issue #403): `backend/benchmark/bench_search_repo_scale.py` measures `filter_media()`/`filter_entities()` (and the FTS5 `search_media_text`/`search_entities_text`) at 10k/100k rows. The search box and the `sort_key='tags'` path are the two shapes that scale linearly with row count (~28→239 ms and ~30→220 ms respectively); the type/status/role combos stay cheap and flat-ish. Nothing exceeds ~260 ms at 100k rows — comfortably inside the per-keystroke budget this path exists to hit.*
...
*Shipped 2026-08-01 (issue #65): image-side semantic search, listings-side semantic search (BGE-M3), and — reversing a prior round's "too large" conclusion — full CRE storage absorption. See below for what's still genuinely minor/deferred (hnswlib ANN index, multi-group semantic prefilter).*
...
- Found the real, working BGE-M3 embedder already in this codebase: `submodules/CRE/src/data/embedder.py`'s `Embedder.embed_dense()` (1024-dim, `FlagEmbedding`, already a locked dependency) — reused rather than adding a new model, same "use what already runs in-process" principle as the image side's `OpenClipEmbedder`.
- Fixed a genuine **pre-existing bug** found along the way: `gui/src/helpers/database/recommendation_worker.py`'s `_RE_DIR` pointed at `<repo_root>/CRE`, which doesn't exist — the submodule actually lives at `<repo_root>/submodules/CRE`. The existing recommendation feature was silently broken (`ImportError` on every use) in this checkout before this fix.
...
- Tests: `backend/test/database/test_recommendation_encrypted_store.py` (15 tests, mirrors `CRE`'s own `tests/test_store.py` coverage plus a table-collision check); `CRE`'s own suite (14 tests) unaffected.
...
- **Listings semantic search**: BGE-M3 embeddings for `media_items`/`entities` stored under `model='bge-m3'`; `CRE`'s store gains a `LibraryBackend` so `rec_engine.db` is absorbed and the dead placeholder-embedding code in `listings_common.py` (`save_*_entry_to_db` byte-of-title vectors) is deleted.
...
- **Migration**: self-heals automatically on every login as of `a528409b` (2026-08-03) — `session.ensure_schema()` now calls `tag_categories.ensure_category_id_column()` *before* applying `schema.sql` (a pre-DB.11 `tags` table lacking `category_id` otherwise makes schema.sql's own `CREATE INDEX idx_tags_category ON tags(category_id)` fail outright with "no such column: category_id", which is exactly what broke listings load/save on the live DB before this fix — the original design wrongly assumed `CREATE TABLE IF NOT EXISTS` would retrofit an existing table, and required an undocumented manual `migrations upgrade-tag-categories` CLI run that nobody was told to do), then backfills via `migrate_legacy_type_column()` after `seed()` populates `tag_categories`. `backend/migrations/upgrade_tag_categories.py` still exists as an explicit backup-first CLI run, sharing the same functions.
...
| ✅ P4 | DB.7 semantic search & CBIR | Full original scope shipped: image + listings semantic search, and full CRE storage absorption (`EncryptedSQLiteStore`) | done |
