...
//      documentation portal shared with the MkDocs Material build (which
//      stays CI-validation-only, see .github/workflows/docs.yml). Sources
...
//      outside docs/ (AGENTS.md, CONTRIBUTING, the root README) and so
//      aren't part of mkdocs's docs_dir. Hand-curated rather than a full
//      repo walk, to keep the nav deliberate rather than noisy.
//
...
// for markdown_extensions / mkdocstrings config we don't care about here.
// js-yaml can't register a wildcard tag, so strip any such tag down to a
// plain quoted string before parsing instead.
...
const EXTRA_SECTIONS = [
  {
    ...
    children: [
      ...
      { title: "Coding Assistant Instructions", source: ".agent/AGENTS.md" },
 ...
 * landing page — "/" itself is reserved for the interactive engineering hub
 * HomeView, see src/router.ts). */
function slugify(mdPath) {
  const noExt = mdPath.replace(/\.md$/, "").replace(/\.ipynb$/, "");
...
const banner =
  ...
  'import type { NavNode, SearchEntry } from "./interfaces/types";\n\n';
const body =
