...
const md = new MarkdownIt({
  ...
  highlight(str, lang) {
    if (lang === "mermaid") {
      mermaidCounter += 1;
      return `<div class="mermaid" id="mermaid-${mermaidCounter}">${md.utils.escapeHtml(str)}</div>`;
    }
...
/** Strip mkdocs-only front-matter-ish `{: #anchor }` attr_list suffixes markdown-it doesn't know. */
