...
const state = ref<"loading" | "ok" | "notfound" | "notebook">("loading");
const html = ref("");
const title = ref("");
...
const REPO_EDIT_BASE = "https://github.com/ACFHarbinger/Image-Toolkit/edit/main/";
const REPO_BLOB_BASE = "https://github.com/ACFHarbinger/Image-Toolkit/blob/main/";
...
async function load() {
  ...
  // Notebooks (docs/notebooks/*.ipynb) aren't bundled as raw markdown — the
  // MkDocs build renders them via mkdocs-jupyter instead. Here they get a
  // lightweight pointer card rather than a client-side .ipynb JSON parser.
  if (entry.source.endsWith(".ipynb")) {
    title.value = entry.title;
    state.value = "notebook";
...
<template>
  <div class="doc-page">
    ...
    <div v-else-if="state === 'notebook'" class="doc-content">
      <h1>{{ title }}</h1>
      <p class="notebook-note">
        This is a Jupyter notebook — rendered with outputs by the MkDocs build
        (<code>mkdocs-jupyter</code>), and viewable raw on GitHub.
      </p>
      <a class="btn btn-primary" :href="REPO_BLOB_BASE + editSource" target="_blank" rel="noopener noreferrer">
        View {{ editSource.split("/").pop() }} on GitHub ↗
      </a>
    </div>
