...
const Modal: React.FC<ModalProps> = ({ isVisible, content, type, onClose }) => {
  ...
  const getStyle = (): ModalStyle => {
    switch (type) {
      ...
      case 'info':
        return { icon: AlertCircle, color: 'bg-blue-500', title: 'Information' };
      case 'custom':
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm transition-opacity duration-300">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-lg w-full m-4 transform transition-all duration-300 scale-100 dark:text-white">
        ...
        <header className={`flex items-center justify-between p-4 ${color} rounded-t-xl text-white`}>
        ...
        <footer className="p-4 border-t border-gray-200 dark:border-gray-700 flex justify-end">
          <button onClick={onClose} className="px-4 py-2 text-sm font-semibold text-white bg-violet-600 rounded-md shadow-md hover:bg-violet-700 transition-colors">
            Close
          </button>
        </footer>
      </div>
