...
export const PropertyComparisonDialog: React.FC<PropertyComparisonDialogProps> = ({
  isOpen,
  onClose,
  propertyData,
}) => {
  ...
  propertyData.forEach(item => Object.keys(item).forEach(k => allKeys.add(k)));
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ overflowY: 'auto', flex: 1, border: '1px solid #202225' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
            ...
            <tbody>
              {propertyData.length === 0 ? (
                <tr><td colSpan={3} style={{ padding: '10px', textAlign: 'center' }}>No Images Selected</td></tr>
              ) : (
                sectionKeys.flatMap((key, keyIdx) => {
                  const bgColor = keyIdx % 2 === 0 ? '#2c2f33' : '#23272a';
                  return propertyData.map((item, itemIdx) => {
                    const imgName = item["File Name"] || item["Path"]?.split(/[/\\]/).pop() || "Unknown";
                    const val = String(item[key] || 'N/A');
                    return (
                      <tr key={`${key}-${itemIdx}`} style={{ backgroundColor: bgColor }}>
                        <td style={{ padding: '6px 8px', fontWeight: 'bold' }}>{key}</td>
                        <td style={{ padding: '6px 8px' }}>{imgName}</td>
                        <td style={{ padding: '6px 8px', textAlign: 'center' }}>{val}</td>
                      </tr>
                    );
                  });
                })
