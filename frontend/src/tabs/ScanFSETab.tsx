...
const ScanFSETab = forwardRef<ScanFSETabHandle, ScanFSETabProps>(({ showModal, onAddImagesToDb }, ref) => {
  ...
  const handleDirectoryChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const initialImagesState: ScannedImage[] = await Promise.all(imageFiles.map(async (file) => {
  ...
  const handleWindowClose = useCallback((idToClose: number) => {
    setOpenWindows(prev => prev.filter(w => w.id !== idToClose));
  }, []);

  const handleWindowMinimize = useCallback((idToToggle: number) => {
    setOpenWindows(prev => 
      prev.map(w => 
  ...
  const handleWindowMaximize = useCallback((idToToggle: number) => {
    setOpenWindows(prev => 
      prev.map(w => 
  ...
  const handleBringToFront = useCallback((idToBringFront: number) => {
    setOpenWindows(prev => {
      ...
      // Reassign IDs to match z-index order
  ...
  const PageDropdown = ({ totalPages, currentPage, onPageChange }: { totalPages: number, currentPage: number, onPageChange: (p: number) => void }) => {
  ...
  return (
    <div className="flex flex-col h-full p-6 dark:bg-gray-900/50">
      ...
      <input
        type="file"
        // @ts-ignore: webkitdirectory is a non-standard attribute but required here
                ...
                <img 
                  ...
                  onError={(e) => {
                    ...
                    if (target.parentElement) {
                        target.parentElement.style.backgroundColor = '#6d28d9'; 
                        target.parentElement.textContent = 'File Error'; 
                    }
                  }}
                />
                {img.selected && (
