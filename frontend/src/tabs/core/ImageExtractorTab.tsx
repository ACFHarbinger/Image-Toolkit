...
const isFileVideo = (file: File) => {
    return file.type.startsWith('video/') || /\.(mp4|mkv|webm|avi|mov|m4v)$/i.test(file.name);
};
...
export const ImageExtractorTab: React.FC = () => {
  ...
  const onSourceDirectoryChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validFiles = files.filter(file => 
      isFileVideo(file) || file.type === 'image/gif'
    );
  ...
  const onExtractDirectoryChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    if (!event.target.files || event.target.files.length === 0) return;
  ...
  const handleExtractRange = (type: 'frames' | 'video' | 'gif') => {
    ...
    alert(`Mock: Extracting ${type} from ${formatTime(startTime)} to ${formatTime(endTime)}`);
  ...
  const isCurrentPathMkv = currentVideoPath ? /\.mkv$/i.test(currentVideoPath) : false;
    ...
    <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-4 space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="border p-4 rounded bg-white dark:bg-gray-800 shadow-sm">
          ...
          <div className="flex gap-2">
            ...
            >
              {isLoading ? <RefreshCcw className="animate-spin" size={16} /> : <FolderOpen size={16} />}
              Browse
            </button>
          </div>
          <p className="text-xs text-gray-500 mt-1">Scans for Video (mp4, mkv, etc) & GIFs</p>
        </div>
        ...
        <div className="border p-4 rounded bg-white dark:bg-gray-800 shadow-sm">
          ...
          <p className="text-xs text-gray-500 mt-1">Scans for any Image or Video to populate gallery</p>
        </div>
      </div>

      {/* 2. Source Media Gallery */}
      <div className="border p-4 rounded bg-white dark:bg-gray-800 shadow-sm">
        <h3 className="font-bold mb-2">Available Media ({sourceMedia.length})</h3>
        <MarqueeScrollArea>
        ...
        <div className="border p-4 rounded bg-white dark:bg-gray-800 shadow-sm transition-all">
          <div className="flex justify-between items-center mb-2 border-b pb-2 dark:border-gray-700">
            ...
            <div className="flex gap-4 items-center text-sm">
              ...
              >
                {useInternalPlayer ? <Monitor size={14} /> : <FolderOpen size={14} />}
              ...
              <label className="flex items-center gap-2">
                ...
                >
                  {PLAYER_SIZES.map((s, i) => <option key={i} value={i}>{s.label}</option>)}
                </select>
              </label>
          ...
          {useInternalPlayer ? (
            <div className="flex flex-col items-center bg-black rounded overflow-hidden relative">
              ...
              <video 
                ...
                onError={(e) => {
                    ...
                    let message = "The format is not supported by this browser or the file is corrupted.";

                    if (error) {
                        if (error.code === error.MEDIA_ERR_SRC_NOT_SUPPORTED) {
                            message = "Error: Media source (likely HEVC/H.265 codec) is not natively supported by your browser/OS.";
                        } else if (error.code === error.MEDIA_ERR_DECODE) {
                            message = "Error: Decoding problem. The video codec (likely HEVC/H.265) is missing or unsupported on your system.";
                        }
                    }

                    // Provide advice to the user
                    setVideoError(message + " If this is an MKV file, try using the 'External Player' option below.");
                }}
              >
                ...
                <span style={{ display: 'none' }} dangerouslySetInnerHTML={{ __html: `<video src="${currentVideoPath}"></video>` }} />
              </video>
              ...
              <div className="w-full bg-gray-900 text-white p-2 flex items-center gap-4">
                <button onClick={togglePlayback} className="hover:text-blue-400">
            ...
            <div className="h-64 flex flex-col items-center justify-center bg-gray-100 dark:bg-gray-900 rounded border border-dashed border-gray-400 text-gray-500">
              ...
              <p className="text-xs italic mt-1">Use slider to sync extraction timestamps.</p>
              <input 
                type="range" 
          ...
          <div className="flex flex-wrap gap-6 items-end">
            ...
            <label className="flex flex-col text-sm w-20">
              ...
              <input 
                ...
                value={gifFps} onChange={(e) => setGifFps(Number(e.target.value))}
                className="border rounded p-1 mt-1 dark:bg-gray-700"
              />
            </label>
      ...
      <div className="flex-1 flex flex-col border rounded bg-white dark:bg-gray-800 shadow-sm overflow-hidden min-h-[400px]">
        <div className="p-4 border-b dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900/50">
          <h3 className="font-bold">Extraction Gallery ({resultsGallery.items.length})</h3>
        ...
        <div className="flex-1 relative">
          <MarqueeScrollArea onSelectionChanged={(set, isCtrl) => resultsGallery.actions.selectBatch(set, isCtrl)}>
            <div className="flex flex-wrap content-start p-2 gap-2">
              {resultsGallery.paginatedItems.map((item, idx) => (
                <ClickableLabel
                  ...
                  isVideo={item.isVideo}
                  isSelected={currentVideoPath === item.path}
                  onPathClicked={(p) => resultsGallery.actions.selectItem(p, true)}
                />
              ))}
              {resultsGallery.items.length === 0 && (
                <div className="w-full h-full flex flex-col items-center justify-center text-gray-400 mt-20">
                  ...
                  <p>Extracted frames (or scanned output files) will appear here.</p>
                </div>
              )}
            </div>
          </MarqueeScrollArea>
        ...
        <div className="p-2 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 flex justify-between items-center text-sm">
          <div className="flex items-center gap-2">
            ...
            <select 
              ...
              onChange={(e) => resultsGallery.pagination.setItemsPerPage(Number(e.target.value))}
            >
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono">
              Page {resultsGallery.pagination.currentPage + 1} / {resultsGallery.pagination.totalPages || 1}
            </span>

            <button 
              onClick={resultsGallery.pagination.nextPage}
              disabled={resultsGallery.pagination.currentPage >= resultsGallery.pagination.totalPages - 1}
              className="px-3 py-1 border rounded hover:bg-gray-200 dark:hover:bg-gray-600 disabled:opacity-50"
            >
              Next
            </button>
