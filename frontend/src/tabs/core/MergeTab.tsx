...
import { LayoutGrid, FolderOpen, FileOutput, CheckSquare, Image as ImageIcon, ChevronLeft, ChevronRight } from 'lucide-react';
...
const MergeTab = forwardRef<MergeTabHandle, MergeTabProps>(({ showModal }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    getData: () => ({
      action: 'merge',
      config: { scanDir, outputDir, outputFilename, direction, spacing, alignMode, gridRows, gridCols, duration },
  ...
  const handleRunMerge = () => {
    if (selected.items.length < 2) return showModal("Select at least 2 images to merge.", "error");
  ...
  return (
    <div className="flex flex-col h-full p-4 gap-4 bg-gray-50 dark:bg-gray-900 overflow-hidden">
      ...
      />
      <input type="file" ref={outputDirRef} onChange={e => {if(e.target.files) setOutputDir("Selected Output")}} className="hidden" 
        // @ts-ignore
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 flex-shrink-0">
        <h3 className="font-bold text-gray-800 dark:text-gray-100 mb-4 flex items-center gap-2"><LayoutGrid size={18} className="text-violet-500"/> Merge Settings</h3>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            {/* Input/Output Group */}
            <div className="space-y-2">
                <div className="flex gap-2">
                    <input type="text" placeholder="Input Directory..." readOnly value={scanDir} className="flex-1 p-2 border rounded text-sm dark:bg-gray-700 dark:border-gray-600"/>
                    <button onClick={handleBrowseInput} className="px-3 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 rounded"><FolderOpen size={16}/></button>
                </div>
                <div className="flex gap-2">
                    <input type="text" placeholder="Output Directory (Optional)..." readOnly value={outputDir} className="flex-1 p-2 border rounded text-sm dark:bg-gray-700 dark:border-gray-600"/>
                    <button onClick={handleBrowseOutput} className="px-3 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 rounded"><FileOutput size={16}/></button>
                </div>
                <input type="text" placeholder="Output Filename (auto extension)" value={outputFilename} onChange={e => setOutputFilename(e.target.value)} className="w-full p-2 border rounded text-sm dark:bg-gray-700 dark:border-gray-600"/>
            </div>

            {/* Merge Config */}
            <div className="grid grid-cols-2 gap-2">
                <select value={direction} onChange={e => setDirection(e.target.value)} className="p-2 border rounded text-sm dark:bg-gray-700 dark:border-gray-600 col-span-2">
                ...
                {direction === 'grid' && (
                    <>
                        <label className="text-xs">Rows: <input type="number" value={gridRows} onChange={e => setGridRows(Number(e.target.value))} className="w-12 p-1 border rounded dark:bg-gray-700"/></label>
                        <label className="text-xs">Cols: <input type="number" value={gridCols} onChange={e => setGridCols(Number(e.target.value))} className="w-12 p-1 border rounded dark:bg-gray-700"/></label>
                    </>
                )}
                
                {direction === 'gif' ? (
                    <label className="text-xs col-span-2">Duration (ms): <input type="number" value={duration} onChange={e => setDuration(Number(e.target.value))} className="w-16 p-1 border rounded dark:bg-gray-700"/></label>
                ) : (
                    <>
                        <label className="text-xs">Spacing: <input type="number" value={spacing} onChange={e => setSpacing(Number(e.target.value))} className="w-12 p-1 border rounded dark:bg-gray-700"/></label>
                        <select value={alignMode} onChange={e => setAlignMode(e.target.value)} className="p-1 border rounded text-xs dark:bg-gray-700 dark:border-gray-600">
      ...
      <div className="flex-1 flex flex-col min-h-0 gap-4">
            ...
            <div className="flex-1 relative min-h-0">
                <MarqueeScrollArea onSelectionChanged={(set, isCtrl) => found.actions.selectBatch(set, isCtrl)}>
                    <div className="flex flex-wrap content-start p-2 gap-2">
                        ...
                        {found.items.length === 0 && <div className="w-full h-full flex items-center justify-center text-gray-400">No images found.</div>}
                    </div>
                </MarqueeScrollArea>
            </div>
        ...
        <div className="flex flex-col border rounded-lg bg-white dark:bg-gray-800 shadow-sm flex-shrink-0" style={{ minHeight: '150px' }}>
            <div className="p-2 border-b dark:border-gray-700 flex justify-between items-center bg-indigo-50 dark:bg-gray-900/50">
                <span className="font-bold text-sm text-indigo-700 dark:text-indigo-400 flex items-center gap-2"><CheckSquare size={16}/> Selected for Merge ({selected.items.length})</span>
      ...
      <div className="pb-2 flex-shrink-0">
        <button onClick={handleRunMerge} disabled={selected.items.length < 2} className="w-full py-3 bg-gradient-to-r from-violet-600 to-indigo-600 text-white rounded shadow-md hover:from-violet-700 hover:to-indigo-700 font-bold disabled:opacity-50">Run Merge</button>
      </div>
    </div>
  );
});
