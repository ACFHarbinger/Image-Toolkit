...
const FloatingImageWindow: React.FC<FloatingWindowProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <span
          className={`${isMaximized ? "text-white" : "dark:text-gray-200"}`}
        >
        ...
        <div className="flex space-x-2 window-controls">
          ...
          >
            {isMaximized ? (
              <Minimize2 size={14} className="dark:text-white" />
            ) : (
              <Maximize2 size={14} className="dark:text-white" />
            )}
          </button>
          <button
...
const ScanMetadataTab = forwardRef<ScanMetadataTabHandle, ScanMetadataTabProps>(
  ({ showModal, onAddImagesToDb }, ref) => {
    ...
    const handleOutputDirectoryChange = (
    ...
    const handleDirectoryChange = async (
      ...
      // Revoke old URLs
      found.items.forEach(
        (img) => img.thumbnail && URL.revokeObjectURL(img.thumbnail),
      );
      ...
      setIsLoading(false);
      showModal(
        `Scan complete! Found ${newGalleryItems.length} images.`,
        "success",
        2000,
      );
    };
    ...
    const handleWindowClose = useCallback(
      (id: number) => setOpenWindows((p) => p.filter((w) => w.id !== id)),
      [],
    );
    ...
    const viewSelectedImages = async () => {
      ...
      if (targets.length === 0) {
        showModal(
          "Please select one or more images to view from the Selected Gallery.",
          "error",
        );
        return;
      }
    ...
    const handleAddSelectedToDb = () => {
      if (!outputDir) {
        showModal(
          "Please select an output directory before adding to DB.",
          "error",
        );
        return;
      }
      ...
      if (filesToAdd.length > 0) {
        ...
        showModal(
          `Adding ${filesToAdd.length} images to database and saving output metadata to ${outputDir}...`,
          "success",
        );
    ...
    const handleUpdateSelected = () => {
      if (selected.items.length > 0) {
        showModal(
          `Simulating update for ${selected.items.length} images.`,
          "info",
        );
      } else {
    ...
    return (
      ...
      <div className="flex flex-col h-full p-4 gap-4 dark:bg-gray-900/50">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div
            className="flex flex-col border rounded-lg bg-white dark:bg-gray-800 shadow-sm"
          ...
          >
            <div className="p-2 border-b dark:border-gray-700 flex justify-between items-center bg-indigo-50 dark:bg-gray-900/50">
              ...
              {selected.selectedPaths.size > 0 && (
                <button
                  onClick={actions.removeSelectedTargets}
                ...
                <div className="flex flex-wrap content-start p-2 gap-2">
                  {selected.paginatedItems.map((item, idx) => (
                    <ClickableLabel
                      ...
                      } // Highlight inside selected gallery
                    />
                  ))}
