...
import { Search, CheckSquare, Image as ImageIcon, ChevronLeft, ChevronRight } from 'lucide-react';
...
const SearchTab = forwardRef<SearchTabHandle, SearchTabProps>(({ showModal }, ref) => {
  ...
  const MOCK_TAGS = ['sunset', 'beach', 'mountain', 'city', 'night', 'portrait', 'landscape', '4k', 'hd', 'abstract', 'nature', 'people'];
  const [selectedTags, setSelectedTags] = useState<Set<string>>(new Set());

  useImperativeHandle(ref, () => ({
    getData: () => ({
      action: 'search',
      criteria: { group, subgroup, filename, formats: Array.from(formats), tags: Array.from(selectedTags) },
  ...
  const handleSearch = () => {
    ...
    const mockResults: GalleryItem[] = Array.from({ length: 125 }).map((_, i) => ({
    ...
    <div className="flex flex-col h-full p-4 gap-4 bg-gray-50 dark:bg-gray-900 overflow-hidden">
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 flex-shrink-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <input type="text" placeholder="Group Name (Optional)" value={group} onChange={e => setGroup(e.target.value)} className="p-2 border rounded text-sm dark:bg-gray-700 dark:border-gray-600"/>
            <input type="text" placeholder="Subgroup Name (Optional)" value={subgroup} onChange={e => setSubgroup(e.target.value)} className="p-2 border rounded text-sm dark:bg-gray-700 dark:border-gray-600"/>
            <input type="text" placeholder="Filename Pattern (*.jpg)" value={filename} onChange={e => setFilename(e.target.value)} className="p-2 border rounded text-sm dark:bg-gray-700 dark:border-gray-600"/>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex-1">
                <span className="text-xs font-semibold text-gray-500 mb-2 block">Formats</span>
            ...
            <div className="flex-[2]">
                <div className="flex justify-between items-center mb-2">
                    ...
                    <button onClick={() => setSelectedTags(new Set())} className="text-[10px] text-red-500 hover:underline">Clear Tags</button>
                </div>
                <div className="h-20 overflow-y-auto border rounded p-2 bg-gray-50 dark:bg-gray-700/50 dark:border-gray-600">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                        {MOCK_TAGS.map(tag => (
                            <label key={tag} className="flex items-center gap-1 text-xs cursor-pointer select-none">
                                <input type="checkbox" checked={selectedTags.has(tag)} onChange={() => toggleTag(tag)} className="rounded text-violet-500"/>
                                {tag}
                            </label>
                        ))}
                    </div>
      ...
      <div className="flex-1 flex flex-col min-h-0 gap-4">
        ...
        <div className="flex-1 flex flex-col min-h-0 border rounded-lg bg-white dark:bg-gray-800 shadow-sm">
            <div className="p-2 border-b dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900/50">
                <span className="font-bold text-sm flex items-center gap-2"><ImageIcon size={16}/> Search Results ({found.items.length})</span>
            ...
            <div className="flex-1 relative min-h-0">
                <MarqueeScrollArea onSelectionChanged={(set, isCtrl) => found.actions.selectBatch(set, isCtrl)}>
                    <div className="flex flex-wrap content-start p-2 gap-2">
                        ...
                        {found.items.length === 0 && <div className="w-full h-full flex items-center justify-center text-gray-400">No results found.</div>}
                    </div>
                </MarqueeScrollArea>
            </div>
            <PaginationBar 
      ...
      <div className="flex gap-2 pb-2 flex-shrink-0">
        <button onClick={() => showModal(`Sending ${selected.items.length} items to Scan Tab`, 'success')} className="flex-1 py-2 bg-green-600 text-white rounded hover:bg-green-700 text-sm font-medium disabled:opacity-50" disabled={selected.items.length === 0}>Send to Scan Tab</button>
        <button onClick={() => showModal(`Sending ${selected.items.length} items to Merge Tab`, 'success')} className="flex-1 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm font-medium disabled:opacity-50" disabled={selected.items.length === 0}>Send to Merge Tab</button>
        <button onClick={() => showModal(`Sending ${selected.items.length} items to Delete Tab`, 'error')} className="flex-1 py-2 bg-red-600 text-white rounded hover:bg-red-700 text-sm font-medium disabled:opacity-50" disabled={selected.items.length === 0}>Send to Delete Tab</button>
      </div>
