  ...
  return (
    <div className="p-4 bg-white dark:bg-gray-800 rounded shadow text-gray-800 dark:text-gray-200 space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <FormRow label="Width:">
          <input type="number" value={width} onChange={(e) => setWidth(Number(e.target.value))} className="w-full border p-2 rounded dark:bg-gray-700 dark:border-gray-600" min={256} max={4096} />
        </FormRow>
        <FormRow label="Height:">
          <input type="number" value={height} onChange={(e) => setHeight(Number(e.target.value))} className="w-full border p-2 rounded dark:bg-gray-700 dark:border-gray-600" min={256} max={4096} />
        </FormRow>
        <FormRow label="Steps:">
          <input type="number" value={steps} onChange={(e) => setSteps(Number(e.target.value))} className="w-full border p-2 rounded dark:bg-gray-700 dark:border-gray-600" min={1} max={200} />
        </FormRow>
      </div>
      ...
      <div className="flex items-center gap-2 mb-4">
        <input 
          ...
          className="w-4 h-4" 
        />
        <span className="font-medium">Skip Layer Cfg (Recommended for SD3.5-M)</span>
      </div>
      ...
      <div className="border-t dark:border-gray-700 pt-4">
        ...
        <FormRow label="ControlNet Model:">
          <select 
            ...
            className="w-full border p-2 rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white"
          >
            {CN_MODELS.map(m => <option key={m} value={m}>{m}</option>)}
          </select>
        </FormRow>
      ...
      <div className="flex gap-2 pt-4">
        <button className="flex-1 bg-blue-600 text-white py-2 rounded hover:bg-blue-700 font-bold">Generate SD3.5 Image</button>
      </div>
    </div>
  );
};