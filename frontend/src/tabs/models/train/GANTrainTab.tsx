...
export const GANTrainTab: React.FC = () => {
  ...
  const startTraining = () => {
    ...
    setLogs((prev) => [
      ...prev,
      "Initializing training thread...",
      `Device: ${"cuda"}`,
    ]);

    // Simulate training progress
    setTimeout(
      () => setLogs((prev) => [...prev, "Starting training loop..."]),
      1000,
    );
  };
  ...
  useEffect(() => {
    ...
    if (isTraining) {
      interval = setInterval(() => {
        ...
        setPreviewUrl(
          `https://placehold.co/200x200/4c4c4c/ffffff?text=Epoch+${Math.floor(Math.random() * 50)}`,
        );
        setLogs((prev) => [
          ...prev,
          `Epoch progress... Loss_D: ${Math.random().toFixed(2)}, Loss_G: ${Math.random().toFixed(2)}`,
        ]);
