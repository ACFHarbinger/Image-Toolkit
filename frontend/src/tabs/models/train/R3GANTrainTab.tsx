...
const PRESETS = ["CIFAR10", "FFHQ-64", "FFHQ-256", "ImageNet-32", "ImageNet-64"];
  ...
  return (
    <div className="p-4 bg-white dark:bg-gray-800 rounded shadow text-gray-800 dark:text-gray-200 space-y-4">
      ...
      <FormRow label="Dataset (.zip):">
        <PathInput value={datasetZip} onChange={setDatasetZip} type="file" placeholder="Path to dataset zip" />
      </FormRow>
      ...
      <div className="flex flex-col gap-2 p-3 border rounded dark:border-gray-600 bg-gray-50 dark:bg-gray-700">
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" checked={mirror} onChange={(e) => setMirror(e.target.checked)} className="w-4 h-4" />
          <span>Mirror Data</span>
        </label>
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" checked={aug} onChange={(e) => setAug(e.target.checked)} className="w-4 h-4" />
          <span>Use Augmentation</span>
        </label>
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" checked={cond} onChange={(e) => setCond(e.target.checked)} className="w-4 h-4" />
      ...
      <div className="flex gap-2 pt-4">
        <button className="flex-1 bg-green-600 text-white py-2 rounded hover:bg-green-700 font-bold">Start R3GAN Training</button>
      </div>
    </div>
  );
};