...
const DriveSyncTab = forwardRef((props: DriveSyncTabProps, ref) => {
  ...
  const [localAction, setLocalAction] = useState<
    "upload" | "delete_local" | "ignore"
  >("upload");
  const [remoteAction, setRemoteAction] = useState<
    "download" | "delete_remote" | "ignore"
  >("download");
  ...
  const handleSync = () => {
    props.showModal(
      `Starting Sync [${dryRun ? "DRY RUN" : "LIVE"}]...\nLocal: ${localPath}\nRemote: ${remotePath}`,
      "info",
    );
  };
    ...
    <div className="p-6 flex flex-col gap-6">
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
        <h3 className="font-bold text-gray-800 dark:text-gray-100 mb-4 flex items-center gap-2">
          <Cloud size={18} className="text-violet-500" /> Cloud Sync
          Configuration
        </h3>
        ...
        <div className="space-y-4">
          ...
          {provider.includes("Service Account") ? (
            <div className="flex gap-2">
              <input
                type="text"
                placeholder="Path to service_account_key.json"
                value={keyFile}
            ...
            <>
              <div className="flex gap-2">
                <input
                  ...
                  value={clientSecrets}
                  onChange={(e) => setClientSecrets(e.target.value)}
                  className="flex-1 p-2 border rounded text-sm dark:bg-gray-700 dark:border-gray-600"
                />
                <button className="px-3 bg-gray-200 dark:bg-gray-700 rounded text-sm">
                  Browse
              ...
              <input
                type="text"
                placeholder="Path to store token.json"
                value={tokenFile}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="Remote folder (e.g. Backups/2024)"
                value={remotePath}
                onChange={(e) => setRemotePath(e.target.value)}
                className="w-full mt-1 p-2 border rounded text-sm dark:bg-gray-700 dark:border-gray-600"
              />
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            <p className="text-sm font-bold text-blue-500 mb-2">
              Files found ONLY Locally (Local Orphans):
            </p>
            <div className="flex gap-4">
              <label className="flex items-center gap-1 text-sm cursor-pointer">
                <input
                  type="radio"
                  name="local"
                  checked={localAction === "upload"}
                  onChange={() => setLocalAction("upload")}
                />
              ...
              <label className="flex items-center gap-1 text-sm cursor-pointer text-red-500">
                <input
                  type="radio"
                  name="local"
                  checked={localAction === "delete_local"}
                  onChange={() => setLocalAction("delete_local")}
                />
          ...
          <div>
            <p className="text-sm font-bold text-green-500 mb-2">
              Files found ONLY Remote (Remote Orphans):
            </p>
            <div className="flex gap-4">
              <label className="flex items-center gap-1 text-sm cursor-pointer">
                <input
                  type="radio"
                  name="remote"
                  checked={remoteAction === "download"}
                  onChange={() => setRemoteAction("download")}
                />
              ...
              <label className="flex items-center gap-1 text-sm cursor-pointer text-red-500">
                <input
                  type="radio"
                  name="remote"
                  checked={remoteAction === "delete_remote"}
                  onChange={() => setRemoteAction("delete_remote")}
                />
              ...
              <label className="flex items-center gap-1 text-sm cursor-pointer text-gray-500">
                <input
                  type="radio"
                  name="remote"
                  checked={remoteAction === "ignore"}
                  onChange={() => setRemoteAction("ignore")}
                />
                <ShieldAlert size={14} /> Do Nothing
              </label>
            </div>
      ...
      <button
        onClick={handleSync}
        className="w-full py-4 bg-gradient-to-r from-blue-600 to-cyan-600 text-white font-bold rounded-lg shadow-lg hover:from-blue-700 hover:to-cyan-700 flex items-center justify-center gap-2 text-lg"
      >
