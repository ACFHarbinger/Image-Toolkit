...
const WebRequestsTab = forwardRef((props: WebRequestsTabProps, ref) => {
  ...
  const handleRun = () => {
    ...
    if (requests.length === 0)
      return props.showModal("Add at least one request.", "error");
    ...
    <div className="p-6 flex flex-col gap-6 h-full">
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
        <h3 className="font-bold mb-4 flex items-center gap-2">
          <Network size={18} className="text-violet-500" /> Request
          Configuration
        </h3>
        <input
          type="text"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1 min-h-0">
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col">
          <h4 className="font-bold mb-2 text-sm text-gray-600 dark:text-gray-300">
            1. Request List (Sequence)
          </h4>
          ...
          <div className="flex gap-2 mb-2">
            ...
            <input
              type="text"
              placeholder="Data / Suffix"
              value={reqParam}
              onChange={(e) => setReqParam(e.target.value)}
          ...
          <div className="flex-1 border rounded bg-gray-50 dark:bg-gray-900 p-2 overflow-y-auto">
            {requests.map((r, i) => (
              <div
                key={i}
                className="flex justify-between items-center p-2 mb-1 bg-white dark:bg-gray-800 rounded shadow-sm text-sm"
              >
                <span>
                  {i + 1}. {r}
                </span>
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col">
          ...
          <div className="flex gap-2 mb-2">
            ...
            <input
              type="text"
              placeholder="Param (e.g. path)"
              value={actParam}
              onChange={(e) => setActParam(e.target.value)}
              className="w-24 p-2 border rounded text-sm dark:bg-gray-700 dark:border-gray-600"
            />
            <button
              onClick={addAction}
      ...
      <div className="bg-black text-green-400 font-mono text-xs p-4 rounded-lg h-32 overflow-y-auto shadow-inner">
        ...
        {logs.length === 0 && (
          <div className="text-gray-600">Ready to execute requests...</div>
        )}
      </div>
      ...
      <div className="flex-shrink-0">
        {!isRunning ? (
          <button
            onClick={handleRun}
            className="w-full py-3 bg-gradient-to-r from-violet-600 to-indigo-600 text-white font-bold rounded shadow-md hover:opacity-90 flex items-center justify-center gap-2"
          >
          ...
          <button
            onClick={() => {
              setIsRunning(false);
              setLogs((p) => [...p, "Cancelled."]);
            }}
            className="w-full py-3 bg-red-600 text-white font-bold rounded shadow-md hover:bg-red-700 flex items-center justify-center gap-2"
          >
            <Ban size={20} /> Cancel
          </button>
        )}
