...
class _InlineActionsMixin:
    ...
    def _rename_selected_file(self: "AbstractClassSingleGalleryHostProtocol") -> None:
    ...
    def _export_selection_as_paths(self: "AbstractClassSingleGalleryHostProtocol") -> None:
        """Write selected (or all visible) paths to a TXT file (Ctrl+E)."""
        paths = self.selected_files or self.gallery_image_paths
        if not paths:
            QMessageBox.information(cast(QWidget, self), "Export", "No files to export.")
            return
        ...
        try:
            ...
            QMessageBox.information(
                cast(QWidget, self), "Export", f"Exported {len(paths)} paths to:\n{dest}"
            )
        except OSError as exc:
            QMessageBox.critical(cast(QWidget, self), "Export Error", str(exc))

    def _copy_selection_to_folder(self: "AbstractClassSingleGalleryHostProtocol") -> None:
        ...
        if not paths:
            QMessageBox.information(cast(QWidget, self), "Copy to Folder", "No files to copy.")
            return
        ...
        QMessageBox.information(cast(QWidget, self), "Copy to Folder", msg)
