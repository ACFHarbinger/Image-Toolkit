...
class FlowLayout(QLayout):
    ...
    def _do_layout(self, rect: QRect, test_only: bool) -> int:
        ...
        # Phase 1: group items into wrapped rows. A spacer's sizeHint() is
        # (0, 0), so it never itself forces an early wrap.
        ...
        # Phase 2: expand addStretch() spacers to fill leftover row width
        # (right-anchoring whatever follows) and vertically center shorter
        # items against the row's tallest member.
        y = rect.y()
        total_height = 0
        for row in rows:
