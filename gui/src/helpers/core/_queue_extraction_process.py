"""Single-extraction pipeline executed in-process (or a pool child).

Split from ``queue_execution_worker`` (§5.17 Option B, #629) — pure code
motion, no logic change. :func:`run_extraction_in_process` keeps its
import path (re-exported by :mod:`queue_execution_worker`, which is also
the multiprocessing pickle path) and its behavior.
"""
