...
class CodecConversionWorker(BaseQThreadWorker):
    ...
    finished = Signal(tuple)  # (converted_count, message)
    ...
    def _emit_completion(self, converted_count: int, total_files: int, failures: List[str]) -> tuple:
        if self._cancelled:
    ...
    def _execute(self) -> object:
        try:
            ...
            return self._emit_completion(converted_count, total_files, failures)
        finally:
            if self._executor:
    ...
    def _convert_single_file(self, input_file: str, idx: int, total_files: int) -> bool:
        if self._cancelled or not os.path.exists(input_file):
            return False
