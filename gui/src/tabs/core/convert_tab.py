...
class FormatTab(AbstractClassTwoGalleries):
...
class ConvertTab(QWidget):
    ...
    def __init__(self, dropdown=True):
        ...
        self.format_tab = FormatTab(dropdown=dropdown)
        self.sampler_tab = SamplerTab()

        self._subtabs.addTab(self.format_tab, "Format")
        self._subtabs.addTab(self.sampler_tab, "Sampler")

        # Forward the QML signal from the inner FormatTab
        self.format_tab.qml_input_path_changed.connect(self.qml_input_path_changed)

        layout.addWidget(self._subtabs)

    # --- QML / external API forwarded to FormatTab ---

    def collect(self) -> dict:
        return self.format_tab.collect()

    def set_config(self, config: dict):
        return self.format_tab.set_config(config)

    def get_default_config(self) -> dict:
        return self.format_tab.get_default_config()
    ...
    def cancel_loading(self):
        ...
        self.sampler_tab.cancel_loading()

    def closeEvent(self, event):
        self.format_tab.close()
        self.sampler_tab.close()
        super().closeEvent(event)
