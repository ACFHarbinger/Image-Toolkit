...
class DeleteTab(AbstractClassTwoGalleries):
    ...
    def __init__(self, dropdown=True):
        ...
        if self.dropdown:
            ...
            for ext in SUPPORTED_IMG_FORMATS:
                ...
                btn.clicked.connect(
                    lambda checked, e=ext: self.toggle_extension(e, checked)
                )
            ...
            apply_shadow_effect(
                btn_add_all, color_hex="#000000", radius=8, x_offset=0, y_offset=3
            )
            ...
            apply_shadow_effect(
                btn_remove_all, color_hex="#000000", radius=8, x_offset=0, y_offset=3
            )
            ...
            self.extensions_field = OptionalField(
                "Target extensions", ext_container, start_open=False
            )
            ...
            settings_layout.addRow(
                "Target extensions (optional):", self.target_extensions
            )

        self.confirm_checkbox = QCheckBox(
            "Require confirmation before delete (recommended)"
        )
        ...
        apply_shadow_effect(
            self.btn_delete_files, color_hex="#000000", radius=8, x_offset=0, y_offset=3
        )
        ...
        apply_shadow_effect(
            self.btn_delete_directory,
            color_hex="#000000",
            radius=8,
            x_offset=0,
            y_offset=3,
        )
        self.btn_delete_directory.clicked.connect(
            lambda: self.start_deletion(mode="directory")
        )
    ...
    def cancel_scan(self):
        if self.scan_thread and self.scan_thread.isRunning():
            ...
            QMessageBox.information(
                self,
                "Scan Cancelled",
                "The image scanning process was manually cancelled.",
            )
            self._reset_scan_ui("Scan cancelled.")

    def _reset_scan_ui(self, status_message: str):
        # Restore button to Delete mode
        self.btn_delete_files.setText(
            f"Delete Selected Files ({len(self.selected_files)})"
        )
    ...
    def on_scan_finished(self, results: Dict[str, List[str]]):
        ...
        if not flattened_paths:
            ...
            QMessageBox.information(
                self, "No Matches", "No duplicate or similar images found."
            )
            return
    ...
    def get_image_properties(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            props["File Size"] = (
                f"{stat.st_size / (1024 * 1024):.2f} MB ({stat.st_size} bytes)"
            )
            props["Last Modified"] = datetime.fromtimestamp(stat.st_mtime).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
        except OSError:
            props["File Size"] = "N/A"
    ...
    def show_comparison_dialog(self):
        if not self.selected_files:
            QMessageBox.warning(
                self, "No Selection", "Please select at least one image to compare."
            )
            return
    ...
    def browse_directory(self):
        d = QFileDialog.getExistingDirectory(
            self, "Select Directory", self.last_browsed_dir
        )
        if d:
