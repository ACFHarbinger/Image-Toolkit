...
class MonitorDisplayGraphOpsController(TabBoundController):
    ...
    def _add_node(self: "MonitorDisplaySubTabHostProtocol"):
        if self._current_monitor_id is None:
            return
        all_exts = list(SUPPORTED_VIDEO_FORMATS) + [f".{e.lower().lstrip('.')}" for e in SUPPORTED_IMG_FORMATS]
        ext_str = " ".join(f"*{e}" for e in all_exts)
        paths, _ = QFileDialog.getOpenFileNames(
            self.tab,
            "Select Wallpaper File(s)",
            "",
    ...
    def _add_self_edge(self: "MonitorDisplaySubTabHostProtocol"):
        ...
        if nid is None:
            QMessageBox.information(self.tab, "No Node Selected", "Select a node first, then click 'Self-Edge'.")
            return
    ...
    def _add_edge(self: "MonitorDisplaySubTabHostProtocol"):
        ...
        if src_id is None:
            QMessageBox.information(self.tab, "No Node Selected", "Select the SOURCE node first, then click 'Connect'.")
            return
    ...
    def _set_start_node(self: "MonitorDisplaySubTabHostProtocol"):
        nid = self._selected_node_id()
        if nid is None:
            QMessageBox.information(self.tab, "No Node Selected", "Select a node first, then click '★ Set Start'.")
            return
    ...
    def _edit_node(self: "MonitorDisplaySubTabHostProtocol", node_id: str):
        ...
        dlg = NodeEditDialog(nd, parent=self.tab)
    ...
    def show_image_context_menu(self: "MonitorDisplaySubTabHostProtocol", global_pos: QPoint, path: str):
        ...
        view_action = QAction(view_text, self.tab)
        ...
        add_action = QAction("➕ Add to Graph Canvas", self.tab)
        add_action.triggered.connect(lambda: self.handle_thumbnail_double_click(path))
        menu.addAction(add_action)

        menu.addSeparator()
        delete_action = QAction("🗑️ Delete File (Permanent)", self.tab)
...
__all__ = ["MonitorDisplayGraphOpsController"]
