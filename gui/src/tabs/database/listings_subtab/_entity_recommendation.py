"""Vector-search recommendation dialog and worker dispatch for Entity
Listings -- mirrors ``_series_recommendation.py``.

``RecommendationWorker`` expects MediaItem-shaped entries (``title`` is
required; ``genres``/``type``/``status``/``rating``/``year``/``episodes``/
``review`` are read via ``.get(..., default)`` and simply stay absent for
entities). Entities carry ``name`` instead of ``title`` and have no
genre/type/status/rating/year/episodes concept, so each entity is passed
through with ``title`` aliased from ``name`` -- everything else about the
worker (embedding store, RRF fusion, id-keyed results) is unchanged and
shared with the same on-disk index as content recommendations.
"""

from __future__ import annotations

import contextlib

from PySide6.QtCore import Slot
from PySide6.QtWidgets import QDialog, QMessageBox

from gui.src.elements.database.dialog.recommendation_dialog import _RecommendationDialog
from gui.src.helpers.database.recommendation_worker import RecommendationWorker

from ._tab_bound import TabBoundController


class EntityListingsRecommendationController(TabBoundController):
    """Runs the recommendation worker and applies/clears its results."""

    def _on_recommend_entities(self) -> None:
        if not self.vault_manager or not self.vault_manager.raw_password:
            QMessageBox.information(
                self.tab,
                "Secure Access Required",
                "You must be logged in to get personalized recommendations.",
            )
            return

        dlg = _RecommendationDialog(self.tab, entity_mode=True)
        if dlg.exec() == QDialog.DialogCode.Accepted:
            self._run_recommendation(dlg.get_inputs())

    def _run_recommendation(self, inputs: dict) -> None:
        old = self._active_rec_worker
        if old is not None:
            with contextlib.suppress(Exception):
                old.finished.disconnect()
                old.error.disconnect()
                old.status.disconnect()
            if old.isRunning():
                old.requestInterruption()
                old.wait()

        entries = [{**e, "title": e.get("name", "")} for e in self._entities]
        worker = RecommendationWorker(
            entries=entries,
            all_entities=self._entities,
            inputs=inputs,
            top_k=50,
            parent=self.tab,
        )
        worker.finished.connect(self._on_recommendation_results)
        worker.error.connect(lambda e: QMessageBox.warning(self.tab, "Recommendation Error", e))
        worker.status.connect(lambda msg: self.stats_label.setText(f"🌟 {msg}"))
        self._active_rec_worker = worker
        self.stats_label.setText("🌟 Running recommendations…")
        worker.start()

    @Slot(object)
    def _on_recommendation_results(self, results) -> None:
        if results is None:  # failure/cancel — error path already reported
            return
        self._recommendation_results = results
        self.clear_rec_btn.show()
        self._rebuild_gallery()

    def _clear_recommendations(self) -> None:
        self._recommendation_results = None
        self.clear_rec_btn.hide()
        self._rebuild_gallery()


__all__ = ["EntityListingsRecommendationController"]
