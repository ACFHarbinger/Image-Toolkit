...
class CBIRTrainTelemetryController(TabBoundController):
    ...
    def _on_epoch(self, epoch: int, m: dict) -> None:
        ...
        self._recall_label.setText(f"Recall@1: {r1:.3f}   Recall@5: {r5:.3f}   Recall@10: {r10:.3f}")
        self._lbl_epoch_loss.setText(f"<b>Last loss</b><br/>{loss:.4f}")

        # Update best R@1 label
        current_best = getattr(self.tab, "_best_r1", 0.0)
        if r1 > current_best:
            self._best_r1 = r1
    ...
    def _on_done(self, status: str, msg: str) -> None:
        ...
        if status == "ok":
            QMessageBox.information(self.tab, "Training complete", msg)
        elif status == "cancel":
            QMessageBox.warning(self.tab, "Cancelled", msg)
        else:
            QMessageBox.critical(self.tab, "Training error", msg)
    ...
    def _on_index_done(self, status: str, msg: str) -> None:
        ...
        if status == "ok":
            QMessageBox.information(self.tab, "Index built", msg)
        else:
            QMessageBox.critical(self.tab, "Index build failed", msg)


# COMPAT(ui-arch-23): legacy mixin alias
_TelemetrySlotsMixin = CBIRTrainTelemetryController

__all__ = ["CBIRTrainTelemetryController", "_TelemetrySlotsMixin"]
