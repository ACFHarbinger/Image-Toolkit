...
class MainWindow(
    ...
    def _set_session_recovery_state(self, new_state) -> None:
        self.session_recovery._set_session_recovery_state(new_state)

    def _restore_when_ready(self, is_ready, action, **kwargs) -> None:
        self.session_recovery._restore_when_ready(is_ready, action, **kwargs)

    def _ensure_category(self, category: str) -> None:
        self.tab_registry._ensure_category(category)

    def _begin_classic_tab_construction(self) -> None:
        self.tab_registry._begin_classic_tab_construction()

    def _iter_live_tabs(self):
        return self.tab_registry._iter_live_tabs()

    def _apply_gallery_prefs_to_tab(self, tab, prefs: dict) -> None:
        self.startup_prefs._apply_gallery_prefs_to_tab(tab, prefs)

    def _apply_wallpaper_slideshow_prefs(self) -> None:
        self.startup_prefs._apply_wallpaper_slideshow_prefs()
