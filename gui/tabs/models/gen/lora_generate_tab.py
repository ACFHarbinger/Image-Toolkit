...
class LoRAGenerateTab(BaseGenerativeTab):
    ...
    def browse_input_image(self):
        fname, _ = QFileDialog.getOpenFileName(
            self, "Select Input Image", self.last_browsed_scan_dir, "Images (*.png *.jpg *.jpeg)"
        )
        if fname:
    ...
    def cancel_generation(self):
        model_id = self.model_selector.currentData()
        if model_id == "animegan_v2": GanWrapper.cancel_process()
        else: LoRATuner.cancel_process()
        self.cancel_btn.setEnabled(False)
    ...
    def set_config(self, config: dict):
        ...
        if "prompt" in config: self.prompt_edit.setText(config["prompt"])
        if "neg_prompt" in config: self.neg_prompt_edit.setText(config["neg_prompt"])
        if "lora_path" in config: self.lora_edit.setText(config["lora_path"])
        if "inference_steps" in config: self.steps_box.setValue(config["inference_steps"])
        if "guidance_scale" in config: self.guidance_box.setValue(config["guidance_scale"])
        if "gan_input_image" in config: self.input_image_edit.setText(config["gan_input_image"])
        ...
        try:
            if model_id == "animegan_v2":
                ...
                LoRATuner.generate_anime_image(
                    prompt=prompt, negative_prompt=neg_prompt, model_id=model_id,
                    output_filename=output_filename, steps=steps,
                    guidance_scale=guidance, lora_path=lora_path, batch_size=batch_size
                )
    ...
    def handle_generation_finished(self, status_type, message):
        ...
        if status_type == "success": QMessageBox.information(self, "Success", message)
        elif status_type == "cancel": QMessageBox.warning(self, "Result", message)
