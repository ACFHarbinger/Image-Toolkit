...
from PySide6.QtWidgets import (
    ...
    QFileDialog, QTextEdit, QGridLayout, QMessageBox,
)
...
class GANTrainTab(QWidget):
    ...
    def set_config(self, config):
        if "data_path" in config: self.txt_data_path.setText(config["data_path"])
        if "save_path" in config: self.txt_save_path.setText(config["save_path"])
        if "epochs" in config: self.spin_epochs.setValue(config["epochs"])
        if "batch_size" in config: self.spin_batch.setValue(config["batch_size"])
        if "lr" in config: self.spin_lr.setValue(config["lr"])