...
class LoRATrainTab(BaseGenerativeTab):
    ...
    def browse_dataset(self):
        directory = QFileDialog.getExistingDirectory(self, "Select Dataset Folder", self.last_browsed_scan_dir)
        if directory:
    ...
    def set_config(self, config: dict):
        ...
        if "dataset_folder" in config: self.data_dir_edit.setText(config["dataset_folder"])
        if "trigger_prompt" in config: self.prompt_edit.setText(config["trigger_prompt"])
        if "lora_rank" in config: self.rank_box.setValue(config["lora_rank"])
    ...
    def run_training(self, params, data_dir, model_id, rank, prompt, output_name):
        try:
            ...
            if is_cancelled:
                ...
                self.training_finished_signal.emit("success", "Training finished and weights saved.")
