...
class WebRequestsTab(QWidget):
    def __init__(self):
        ...
        self.request_param_input.setPlaceholderText(
            "POST Data (key:val, k2:v2) or URL Suffix"
        )
        ...
        self.action_param_input.setPlaceholderText(
            "Parameter (e.g., file path for Save)"
        )
    ...
    def add_action(self):
        ...
        if "Save" in action_text and not param:
            QMessageBox.warning(
                self,
                "Missing Parameter",
                "Save actions require a file path or directory as a parameter.",
            )
            return
    ...
    def start_requests(self):
        ...
        if self.request_list_widget.count() == 0:
            QMessageBox.warning(
                self, "No Requests", "Please add at least one request to run."
            )
            return
        ...
        if self.action_list_widget.count() == 0:
            QMessageBox.warning(
                self,
                "No Actions",
                "Please add at least one action to perform on the response.",
            )
            return
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", "Failed to import WebRequestsWorker. Check console."
            )
        ...
        if self.worker and self.worker.isRunning():
            try:
                ...
                QMessageBox.information(
                    self, "Cancelled", "The request sequence has been stopped."
                )
            except Exception as e:
    ...
    def set_config(self, config: dict):
        ...
        try:
            ...
            for req in requests:
                ...
                if param:
                    display_text += (
                        f" | {'Data' if req_type == 'POST' else 'Suffix'}: {param}"
                    )
                self.request_list_widget.addItem(display_text)
