...
class TestDirectoryImportTransaction:
    def test_cancelled_dialog_makes_no_changes(self, q_app):
        ...
        with patch(
            "gui.src.database.series_listings_subtab._directory_import._DirectoryImportDialog",
            return_value=dlg,
        ), patch(
            "gui.src.database.series_listings_subtab._directory_import.get_library_db"
        ) as mock_get_db:
    ...
    def test_locked_vault_shows_warning_and_saves_nothing(self, q_app):
        ...
        with patch(
            ...
            "gui.src.database.series_listings_subtab._directory_import.QMessageBox.warning"
        ) as mock_warn:
            tab._on_import_from_directory()
    ...
    def test_batch_commits_in_one_transaction(self, q_app):
        ...
        with patch(
            ...
            "gui.src.database.series_listings_subtab._directory_import.MediaRepo",
            return_value=mock_media_repo,
        ), patch(
            "gui.src.database.series_listings_subtab._directory_import.QMessageBox.information"
        ):
            tab._on_import_from_directory()
    ...
    def test_save_failure_rolls_back_and_does_not_add_entries(self, q_app):
        ...
        with patch(
            ...
            "gui.src.database.series_listings_subtab._directory_import.QMessageBox.critical"
        ) as mock_crit:
            tab._on_import_from_directory()
