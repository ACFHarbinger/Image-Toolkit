"""SimilarityTab virtual dual-gallery migration tests (GUI/UX §2.1).

The found/selected card grids are replaced by a ``VirtualDualGallery``;
selection syncs with the dual and the triage _select_paths/_deselect_paths
push their mutations back into the selected panel.
"""
...
def test_toggle_selection_syncs_to_dual(q_app, tmp_path):
...
def test_select_paths_push_to_dual(q_app, tmp_path):
    tab = _make_tab(q_app, tmp_path, n=6)
    tab._select_paths([tab.found_files[4]])
    assert tab.dual.count_selected() == 1
    tab._deselect_paths([tab.found_files[4]])
    assert tab.dual.count_selected() == 0
    tab.close()


def test_clear_galleries_clears_dual(q_app, tmp_path):
    tab = _make_tab(q_app, tmp_path, n=6)
    tab.toggle_selection(tab.found_files[0])
    tab.clear_galleries()
    assert tab.dual.count_found() == 0
    assert tab.dual.count_selected() == 0
    assert tab.found_files == []
    tab.close()
