...
class TestMaybeOfferAutoListings:
    def test_empty_touched_groups_is_noop(self, q_app):
        tab = _make_tab()
        with patch(
            "gui.src.tabs.database.scan_metadata_tab._auto_listings.get_library_db"
        ) as mock_get_db:
    ...
    def test_group_already_linked_is_skipped(self, q_app):
        ...
        with patch(
            ...
            "gui.src.tabs.database.scan_metadata_tab._auto_listings.ImageRepo",
            return_value=mock_image_repo,
        ), patch(
            "gui.src.tabs.database.scan_metadata_tab._auto_listings.MediaRepo",
            return_value=mock_media_repo,
        ), patch(
            "gui.src.tabs.database.scan_metadata_tab._auto_listings._AutoListingsReviewDialog"
        ) as mock_dialog_cls:
