...
class TestSettingsWindowLogs:
    ...
    def test_import_credential(self, q_app, tmp_path):
        ...
        with (
            patch("gui.src.windows.settings_window.API_DIR", mock_api),
            patch("gui.src.windows.settings_window.QFileDialog.getOpenFileName", return_value=(str(dummy_import_file), "JSON")),
            patch("gui.src.windows.settings_window.QInputDialog.getText", return_value=("imported_alias", True)),
            patch.object(QMessageBox, "information") as mock_info,
            ...
            assert mock_vault.api_credentials["imported_alias"] == {"api_key": "imported_secret"}
