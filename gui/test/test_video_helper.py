...
class TestVideoScannerWorker:
    def test_run(self, q_app, tmp_path):
        ...
        with (
            patch(
                "gui.src.helpers.video.video_scan_worker.concurrent.futures.ProcessPoolExecutor"
            ) as MockExecutor,
            ...
            with patch(
                "gui.src.helpers.video.video_scan_worker.concurrent.futures.wait",
                return_value=mock_wait_res,
            ):
    ...
    def test_run_rust_multiprocessing(self, q_app, tmp_path):
        ...
        mock_buf = b"\xff" * 400
        mock_batch_result = [(str(v1), mock_buf, 10, 10), (str(v2), mock_buf, 10, 10)]
