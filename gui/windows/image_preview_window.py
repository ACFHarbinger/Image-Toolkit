...
class ImagePreviewWindow(QDialog):
    ...
    def _initial_scale_and_layout(self):
        ...
        target_width = min(
            current_size.width() + 100, self.max_width
        )  # 100 is padding + arrows
        target_height = min(
    ...
    def update_image_display(self):
        ...
        if self.is_animated and self.current_movie:
            ...
            self.image_label.setFixedSize(
                scaled_pixmap.size()
            )  # Set fixed size to allow scrolling
        ...
        self.setWindowTitle(
            f"Full-Size Image Preview: {os.path.basename(self.image_path)} [{zoom_percent}%]{nav_info}"
        )
        ...
        if self.parent_tab and hasattr(self.parent_tab, "open_preview_windows"):
            ...
            if other_windows_count > 0:
                ...
                close_all_action = QAction(
                    f"Close All {other_windows_count} Other Previews", self
                )
    ...
    def _close_all_other_previews(self):
        ...
        if self.parent_tab and hasattr(self.parent_tab, "open_preview_windows"):
            windows_to_close = [
                w
                for w in list(self.parent_tab.open_preview_windows)
            ...
            QMessageBox.information(
                self.parent_tab,
                "Previews Closed",
                f"Closed {len(windows_to_close)} other preview windows.",
            )
