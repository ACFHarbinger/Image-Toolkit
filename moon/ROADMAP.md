...
| # | Item | Effort | Roadmap Link |
|---|------|--------|--------------|
| 2.1 | **[ASP] ✅ TOML config file for pipeline constants** — `load_asp_config()` + `get_asp()` in `backend/src/anim/config.py`; `_CONFIG_SCHEMA` (14 keys); `validate_asp_config()` strict/warning modes; `asp_config.toml` loaded on startup (§1.8A/B, S27/S42) | Done | [asp.md §1.8](roadmaps/asp.md#18-asp-pipeline-configuration-file) |
