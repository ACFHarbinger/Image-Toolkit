...
| 1.7 | **[Perf] ✅ Rust DynamicImage move semantics** — take ownership in `apply_ar_transform` (`image_converter.rs`) and `fast_resize` (`image_merger.rs`); no-op path returns owned image, eliminating ~30 MB clone for 4K RGBA on convert+merge | Done | [performance.md §3.6](roadmaps/performance.md#36-dynamicimage-move-semantics-in-rust) |
