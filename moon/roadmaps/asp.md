...
### §4.9 Post-Composite Seam Band Smoothing ✅ [Priority: Medium — seam_visibility; promoted to default-ON S166]
...
- `_SEAM_SMOOTH_PX: int = int(os.environ.get("ASP_SEAM_SMOOTH_PX", "0"))` (default OFF S164)
...
- **S166**: Promoted to default-ON (4px) — default changed to `"4"` in `pipeline.py` and `SEAM_SMOOTH_PX=4` in constants. Safe to enable now that §4.10 pre-seam gain equalization reduces inter-frame luminance steps before the seam search. Set `ASP_SEAM_SMOOTH_PX=0` to disable.

---

### §4.10 Pre-Seam Global Gain Equalization ✅ [Priority: High — strip_banding dominant failure]

**Problem**: GraphCut seam finding (§4.2, default-ON since S161) assigns frame ownership globally but does not equalise inter-frame luminance before the seam search. The pairwise `_blocks_gain_compensate()` (§4.1) was wired only in the DP fallback path, so most composites went through GraphCut with no gain correction — producing strip_banding_score 31–41 and seam_visibility 119+ on the worst failing tests.

**Implementation** *(shipped S165)*:
- `_equalize_warped_gains(warped_frames, block_size=32)` in `compositing.py`:
  - Sequential pairwise `_blocks_gain_compensate()`: frame 0 is reference; each frame corrected to match its predecessor
  - Only corrects pixels where BOTH adjacent frames have valid content (`max(axis=2) > 0`)
  - Black/transparent fill border regions left untouched
- Called inside `_composite_foreground` immediately before the GraphCut block (`if _GRAPHCUT_SEAM ...`), so it runs regardless of which seam path is active
- `_GLOBAL_GAIN_COMP: bool = os.environ.get("ASP_GLOBAL_GAIN_COMP", "1") != "0"` (default ON)
- Also enabled `_BLOCKS_GAIN_COMP` and `_BLOCKS_LUM_COMP` by default ("1") so DP path also benefits
- Exported in `__all__`: `_equalize_warped_gains`, `_GLOBAL_GAIN_COMP`
- 5 tests in `TestEqualizeWarpedGains` (`test_compositing.py`); 1410 passed
...
*Items marked ✅ are fully shipped and removed from pending rows. Matrix last updated: S167 (2026-06-24).*
...
|---|---|---|---|---|
| **Low (<1d)** | — | — | ✅§4.3 Wave correction (detail.waveCorrect) · ✅§4.5 Canvas-space DpSeamFinder (S162) · ✅§4.8 SeamVisGate (S163) · ✅§4.9 Seam band smoothing (default-ON S166) | — |
| **Medium (1d–1w)** | — | — | ✅§4.1 BlocksGainCompensator (strip_banding fix) · ✅§4.4 BlocksChannelsCompensator · ✅§4.10 Pre-seam global gain eq (S165) · ✅§3.33 Feathered GC boundary (S164) · §4.6 MultiBand confidence weighting | ✅§4.2 GraphCut global seam (default-ON S161) · ✅§4.7 dy_cv gate (SCANS fallback S161) · ✅§4.8 SeamVisGate (S163) |
