...
*Added 2026-06-22 after reverse-engineering `cv2.detail` (OpenCV 4.13.0) and contrasting with ASP. These items address the benchmark failures where ASP underperforms simple stitch (strip_banding_score: 97.9% worse; ghosting: 93.8% worse; seam_visibility: 88.5% worse).*
...
**Implementation**: After Stage 10 render (warped frames already in canvas space), call `GraphCutSeamFinder.find(warped_images_as_umats, corners, masks)`. Returns per-image binary ownership masks in canvas space. Replace current pairwise seam loop with mask-based compositing. `ASP_GRAPHCUT_SEAM=1` flag. Fallback to pairwise DP when GraphCut fails or `num_bands < 2`. Requires `warped_frames` and `warped_masks` as `cv2.UMat` lists — minor refactor of Stage 11 input preparation.

**Expected impact**: Seam routing quality improvement from eliminating pairwise-DP local optima. Particularly valuable for sequences with sparse background corridors where multiple boundaries compete.
...
**Implementation**: After Stage 10, call `DpSeamFinder("COLOR_GRAD").find(warped_as_umats, corners, masks_as_umats)`. Convert resulting binary masks to seam paths by finding the boundary row per column. `ASP_DP_CANVAS_SEAM=1` flag. Step between §4.2 (GraphCut) and current pairwise DP in the feature hierarchy.
...
|---|---|---|---|---|
| **Low (<1d)** | — | — | ✅§4.3 Wave correction (detail.waveCorrect) · §4.5 Canvas-space DpSeamFinder | — |
| **Medium (1d–1w)** | — | — | ✅§4.1 BlocksGainCompensator (strip_banding fix) · ✅§4.4 BlocksChannelsCompensator · §4.6 MultiBand confidence weighting | §4.2 GraphCut global seam (detail_GraphCutSeamFinder, ghosting fix) |
