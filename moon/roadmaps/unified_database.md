...
- **Re-investigated once more, reframed as a storage-location question rather than an algorithm-porting one**: storage location is already effectively consolidated (`RecommendationWorker` already points `rec_engine.db` at `IMAGE_TOOLKIT_DIR`, the same directory `library.db` lives in — not a separate directory). The real, concrete, previously-unstated gap: **`rec_engine.db` is confirmed plaintext SQLite** (`submodules/Recommendation-Engine/src/data/store.py`'s `SQLiteStore` uses the stdlib `sqlite3` directly, no SQLCipher anywhere in that module or its dependencies) — sitting next to the encrypted `library.db`, storing real listing content (titles, genres, tags, resolved entity names, review text). This is a genuine violation of DB.1 locked-in decision #2 ("everything is encrypted at rest ... no plaintext sidecar DBs"). Closing it has exactly two paths, both already-known blockers: (1) add a SQLCipher-capable driver to `Recommendation-Engine`'s own dependencies — this is *precisely* the "second libsqlcipher loaded in-process alongside the JPype JVM" class that has caused three prior SIGSEGVs in this app (see project memory), not safe to do blind; or (2) route its SQL through the already-linked `base.database.Database` instead of raw `sqlite3` — which is the algorithm-porting rewrite already correctly ruled too large. No third, smaller option exists. Flagging this precisely (rather than leaving it as a vague "too large") so a future round has a concrete starting point.
...
*Started 2026-08-01 (issue #66): DB.8a/8b/8c/8d are all fully shipped, including cross-tab navigation and Entity Recon integration. Only real tag-chip widgets (tracked separately as #127) remain — see below.*
...
- **DB.8c Unified tag vocabulary**: `TagRepo.merge_tags()` already existed (shipped with DB.3, tested, but never exposed on the facade or reachable from any GUI control). Exposed as `UnifiedImageDatabase.merge_tags()`; the Maintenance panel's tag table gained a "🔀 Merge Into…" right-click action — pick a destination tag from a dialog, confirm, every image/media reference to the source tag is repointed and the source is deleted. Later in the same round: the listings detail panel's Genres/Tags fields gained real autocomplete from the shared vocabulary, wiring up `TagCompleter` (`gui/src/helpers/database/tag_completer.py` — built for issue #127, sat dead/unused until now) — one vocabulary, both directions (an image tag suggests on the listings side and vice versa, since `tags` is one unified table post-migration). The Maintenance tag table also gained a "🔍 Search Images with this Tag" right-click action (dispatches through the existing `search_tab_ref` cross-tab pattern, same as `SearchTab`'s own "Send To..." actions).
...
- Tests: `test_media_group_links`, `test_media_group_fuzzy_suggestions`, `test_entity_image_links` (`test_unified_repos.py`), `test_merge_tags_facade` (`test_unified_facade.py`); `gui/test/core/test_detail_panel_links.py` (16), `gui/test/core/test_tag_vocabulary_and_search.py` (12), `gui/test/database/test_scan_metadata_auto_listings.py` (6), `gui/test/web/test_entity_recon_library_link.py` (7), `gui/test/core/test_directory_import_transaction.py` (5).

**Not done yet** (explicitly deferred, not silently skipped):
- **Real tag-chip widgets in listings**: the roadmap's "tag chips ... autocomplete" phrasing presupposes a per-tag chip UI in the listings detail panel; today the Genres/Tags fields are still plain comma-separated `QLineEdit`s (now autocompleting, but not chips) — a real `TagChipWidget` is tracked separately as issue #127, out of scope here.
...
| 🔄 P5 | DB.8 cross-domain features | Links, tag vocabulary, cross-tab navigation, both auto-listings halves, and Entity Recon integration all live; only tag-chip widgets (tracked separately as #127) pending | ~1w |
...
| **`rec_engine.db` is plaintext, violating DB.1's "no plaintext sidecars" decision** (confirmed 2026-08-01, DB.7) | Not yet closed — both fixes are already-known-blocked (SQLCipher driver = JVM-SIGSEGV risk class; route through `base.database` = the already-ruled-too-large algorithm rewrite). Flagged here so it isn't silently forgotten between rounds. |
