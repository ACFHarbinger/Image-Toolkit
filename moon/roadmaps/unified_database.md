...
*Started 2026-08-01 (issue #66): DB.8a/8b DAL + detail-panel UI, DB.8c's tag-merge/autocomplete/search-by-tag, and a scoped-down DB.8d are all shipped and tested. Only Entity Recon integration and two specific cross-tab-navigation gaps remain — see below.*
...
- **DB.8d Auto-create listings from scans (scoped down)**: rather than the original spec's series/episode parsing from a video-directory import (heuristic-heavy, deferred), this ships the more tractable half — after a Scan & Tag batch upsert touches one or more image groups, any group with **zero** linked `media_items` and no whole-word-fuzzy-matching existing title gets offered in one batched review dialog (checkable rows, editable title, "create new" vs. "link to existing" per row where a fuzzy match *does* exist) — confirm once, one transaction creates/links everything checked. Wired as a live hook in `scan_metadata_tab/_upsert_ops.py` (`_auto_listings.py`, new `_AutoListingsMixin`), not a separate Maintenance action — the upsert flow already exposed the group names touched by a batch cleanly.
- Tests: `test_media_group_links`, `test_media_group_fuzzy_suggestions`, `test_entity_image_links` (`test_unified_repos.py`), `test_merge_tags_facade` (`test_unified_facade.py`); `gui/test/core/test_detail_panel_links.py` (11), `gui/test/core/test_tag_vocabulary_and_search.py` (7), `gui/test/database/test_scan_metadata_auto_listings.py` (6).

**Not done yet** (explicitly deferred, not silently skipped):
- **Two cross-tab-navigation gaps, same root cause**: DB.8a's "View Images" jump (Content Listings → Search tab, pre-filtered by group) and DB.8c's "Search Listings with this Tag" (Maintenance → Listings). Both investigated — `ListingsTab`/`DatabaseTab` have no reference to each other anywhere; the only existing cross-tab pattern (`DatabaseTab.search_tab_ref`/`merge_tab_ref`/etc.) only connects tabs *within* the Library category. This is a real architecture change (new tab-reference plumbing through `MainWindow`'s wiring), not "hooking into existing plumbing" — left for a dedicated round. ("Search Images with this Tag", *within* the Library category, shipped fine — see above.)
...
- **DB.8d, full series/episode parsing** from a video-directory scan (the original roadmap text) — still not started; the scoped-down group-level version above ships the more tractable half.

## 🔄 DB.9 Data Browser Tab (Raw Tables + ER View)

*Started 2026-08-01 (issue #67): the table-grid slice, then FK-cell navigation + the reverse-references panel, are both shipped and tested. Only the ER/crow's-foot graphics view and gated edit mode remain — see below.*
...
- Tests: `backend/test/database/test_browser_repo.py` (16 tests total: the original 10 plus 6 for the new FK/reverse-reference methods); `gui/test/database/test_data_browser_tab.py` (15 GUI tests total: the original 8 plus 7 for FK-click/reverse-refs navigation).

**Not done yet** (explicitly deferred, not silently skipped):
- **Schema/ER view**: the `QGraphicsScene` crow's-foot relationship-diagram view — a much larger, separate piece (layout algorithm, FK-edge rendering, domain clustering) than the table grid + navigation above; not attempted.
...
| 🔄 P5 | DB.8 cross-domain features | Links, tag vocabulary (merge/autocomplete/search), and scoped auto-listings all live; Entity Recon integration + two cross-tab-navigation gaps still pending | ~1w |
| 🔄 P6 | DB.9 data browser + DB.10 cleanup/backups/docs | Table-grid browser + FK navigation + reverse-refs live (DB.10 already done); ER view, edit mode still pending | ~1w |
