...
*Started 2026-08-01 (issue #66): every DB.8a/8b/8c/8d piece attempted so far has shipped, including cross-tab navigation and Entity Recon integration. Only real tag-chip widgets (tracked as #127) and full series/episode-based auto-listings remain — see below.*
...
*Started 2026-08-01 (issue #67): the table-grid slice, FK-cell navigation + reverse-references panel, and the ER/crow's-foot schema view are all shipped and tested. Only the gated edit mode (v2) and per-column filter UI remain — see below.*
...
- Tests: `backend/test/database/test_browser_repo.py` (16 tests: the original 10 plus 6 for the FK/reverse-reference methods); `gui/test/database/test_data_browser_tab.py` (20 GUI tests: the original 8, plus 7 for FK-click/reverse-refs navigation, plus 5 for the schema view).
...
| 🔄 P5 | DB.8 cross-domain features | Links, tag vocabulary, cross-tab navigation, scoped auto-listings, and Entity Recon integration all live; only tag-chip widgets (#127) and full series/episode auto-listings pending | ~1w |
| 🔄 P6 | DB.9 data browser + DB.10 cleanup/backups/docs | Table-grid browser + FK navigation + reverse-refs + ER schema view all live (DB.10 already done); only gated edit mode + per-column filters pending | ~1w |
